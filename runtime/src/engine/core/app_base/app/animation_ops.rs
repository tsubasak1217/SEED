...
use crate::engine::components::{
    ...
    ComponentKind, ModelAnimDrive, ModelComponent,
};
...
impl App {
    ...
    pub(super) fn update_animations(&mut self, dt: f32) {
        ...
        for (owner, slot_entity) in jobs {
            ...
            let cref: AnimClipRef = match world
            ...
            match cref.kind {
                ...
                AnimClipKind::Keyframe => {
                    ...
                    let clip: Arc<AnimationClip> = match world
                        .get::<AnimatorComponent>(slot_entity)
                    ...
                    for track in &clip.tracks {
                        let Some(binding) = animation::resolve_binding(
                            &track.target.component,
                            &track.target.property,
                        ) else {
                            continue;
                        };
                        if binding.expected_value_type() != track.value_type {
                            continue;
                        }
    ...
    fn init_animators(&mut self) {
        ...
        for slot_entity in entities {
                ...
                match AnimationClip::load(&cref.path) {
                    Ok(clip) => {
                        ...
                        for (ti, track) in clip.tracks.iter().enumerate() {
                            match animation::resolve_binding(
                                &track.target.component,
                                &track.target.property,
                            ) {
                                Some(binding)
                                ...
                                {
                                    eprintln!(
                                        "[SEED anim] {}: track {ti} ({}/{}) の値型が不一致のため無視します",
                                        cref.path, track.target.component, track.target.property
                                    );
                                }
                                None => {
                                    eprintln!(
                                        "[SEED anim] {}: track {ti} の未対応プロパティ {}/{} を無視します",
                                        cref.path, track.target.component, track.target.property
                                    );
                                }
                                _ => {}
            ...
            if let Some(a) = scene.world.get_mut::<AnimatorComponent>(slot_entity) {
                ...
                let default_playable = !a.default_clip.is_empty()
                    && a.clips.iter().any(|c| {
                        c.name == a.default_clip
                            && (matches!(c.kind, AnimClipKind::Model)
    ...
    /// 安全側で常に再初期化する）。
    pub(super) fn handle_set_animator_clips(
        &mut self,
        actor_dfs_id: u32,
        slot_idx: u32,
        json: &str,
    ...
    pub(super) fn handle_anim_preview(&mut self, actor_dfs_id: u32, clip_path: &str, time: f32) {
            ...
            None => match AnimationClip::load(clip_path) {
                Ok(c) => {
                    ...
                    self.anim_preview_cache
                        .insert(clip_path.to_string(), arc.clone());
                    arc
                }
                Err(err) => {
        ...
        let Some(owner) = find_actor_by_dfs(actors, wl, actor_dfs_id, &mut counter) else {
            return;
        };
        ...
        for track in &clip.tracks {
            ...
            let Some(target_actor) = animation::resolve_actor_path(owner, &track.target.actor_path)
            else {
                continue;
            };
            if let Some(value) = animation::sample_track(track, sample_time) {
    ...
    pub(super) fn handle_anim_preview_stop(&mut self, actor_dfs_id: u32) {
        let Some(saved) = self.anim_preview_saved.remove(&actor_dfs_id) else {
            return;
        };
        if saved.is_empty() {
            return;
