...
impl App {
    ...
    pub(super) fn update_camera_snap_anim(&mut self, delta_time: f32) {
        ...
        let (rot, target_yaw, target_pitch, done) = {
            let Some(anim) = &mut self.camera_snap_anim else {
                return;
            };
            ...
            let rot = if done {
                ...
                anim.start_rot.slerp(anim.target_rot, t_smooth)
            };
    ...
    pub(super) fn apply_camera_data(&mut self, cam: &DebugCameraData) {
        ...
        self.camera.base.transform.position =
            Vector3::new(cam.position[0], cam.position[1], cam.position[2]);
