...
impl App {
    ...
    pub(super) fn on_cursor_moved(&mut self, position: PhysicalPosition<f64>) {
        ...
        // ── ロジック配置モード中 ────────────────────────────────────
        // 基準点はフレームループが毎フレーム解決するので、ここでは
        // カーソル座標を記録するだけでよい（上で記録済み）。
        // 半径ドラッグの「クリックかドラッグか」だけは画面上の移動量で決まるので、
        // 基準点の解決（GPU 読み戻し）を待たずにここで判定する。
        // 選択ホバー・矩形選択・ギズモドラッグへは一切流さない（排他）。
        if self.placement_mode_active() {
            self.update_placement_drag_flag(cx, cy);
            return;
        }
