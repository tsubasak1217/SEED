...
use crate::engine::core::app_base::undo::{
    ActorDfsSelectionCommand, ActorGroupTransformCommand, CanvasTransformCommand, CompositeCommand,
    MultiActorDragTransformCommand, MultiTransformCommand, SelectionCommand,
};
use crate::engine::methods::gizmo_interact::{
    mat4x4_inv, mat4x4_mul, screen_to_ray_ortho, update_drag,
};

use super::{
    App, CANVAS_WORLD_SCALE, RuntimeMode, camera_scene_gizmo, canvas_anchor_offset_for_dfs,
    collect_canvas_actors_in_rect, collect_child_actor_mc_starts, collect_mcs_in_world_line,
    collect_transform_only_in_rect, find_actor_by_dfs, find_parent_actor_of_dfs,
    get_3d_canvas_world_mat, warp_cursor_to_local, world_to_screen,
};
...
impl App {
    ...
    pub(super) fn on_cursor_moved(&mut self, position: PhysicalPosition<f64>) {
                    ...
                    if let Some(scene) = &self.scene {
                        if let Some(ws) = self.window.as_ref().map(|w| w.inner_size()) {
                            ...
                            if self.actor_edit_canvas_wls.contains(&wl) || self.edit_view_is_2d() {
                                ...
                                let all_mcs =
                                    collect_mcs_in_world_line(&scene.actors, &scene.world, wl);
                                ...
                                for (dfs_id, icon_mat) in cam_gizmo_mats_rect {
                                    let world_pos =
                                        [icon_mat[0][3], icon_mat[1][3], icon_mat[2][3]];
                                    let in_rect = world_to_screen(
                                        world_pos, &view.data, &proj.data, vp_w, vp_h,
                                    )
                                    .map(|(sx, sy)| {
                                        sx >= sx_min && sx <= sx_max && sy >= sy_min && sy <= sy_max
                                    })
                                    .unwrap_or(false);
        ...
        let new_mat_opt = if let Some(drag) = &self.drag.gizmo_drag {
            if let Some(ws) = self.window.as_ref().map(|w| w.inner_size()) {
                ...
                let use_ss_drag = self.canvas_screen_space_overlay
                    || !in_editor_drag
        ...
        if let Some(new_mat) = new_mat_opt {
            if let Some(drag) = &self.drag.gizmo_drag {
                ...
                let drag_ctx_2d = self
                    .drag
                    .canvas_transform_drag_start
                    .as_ref()
                    .map(|&(dfs, _)| dfs)
                ...
                if let Some(scene) = &mut self.scene {
                    ...
                    if let Some(mc) =
                        mc_entity.and_then(|e| scene.world.get_mut::<ModelComponent>(e))
                    {
                    ...
                    if mc_entity.is_some() && !self.drag.drag_root_starts.is_empty() {
                        if let Some(entity) = drag_actor_entity {
                            if let Some(actor_new_mat) = self
                                .drag
                                .drag_root_starts
                                .first()
                        ...
                        for (slot_i, start_mats) in &extra_starts {
                            ...
                            if let Some(entity) = slot_entity {
                                if let Some(mc) = scene.world.get_mut::<ModelComponent>(entity) {
                                    for (m, start) in
                                        mc.instance_mats.iter_mut().zip(start_mats.iter())
                                    {
                                        *m = mat4x4_mul(delta, *start);
                                    }
                                    mc.mark_batch_dirty();
                    ...
                    if self.drag.drag_root_starts.is_empty() {
                        ...
                        if self.drag.canvas_transform_drag_start.is_some() {
                            ...
                            if let Some((drag_dfs, ref start_ct)) =
                                self.drag.canvas_transform_drag_start.clone()
                            {
                                let entity = {
                                    let mut c = 0u32;
                                ...
                                if let Some(entity) = entity {
                                    ...
                                    if let Some(ct) = scene.world.get_mut::<CanvasTransform>(entity)
                                    {
                                        if let Some(ctw) = parent_ctw {
                                            ...
                                            let wy = new_mat[1][3];
                                            let wz = new_mat[2][3];
                                            let cx = ctw_inv[0][0] * wx
                                                + ctw_inv[0][1] * wy
                                                + ctw_inv[0][2] * wz
                                                + ctw_inv[0][3];
                                            let cy = ctw_inv[1][0] * wx
                                                + ctw_inv[1][1] * wy
                                                + ctw_inv[1][2] * wz
                                                + ctw_inv[1][3];
                                            ...
                                            let use_ss_c = drag_ctx_2d.is_some()
                                                || self.canvas_screen_space_overlay
                                                || !in_editor_c
                                            ...
                                            if let Some(ctx2d) = &drag_ctx_2d {
                                                ...
                                                let dx =
                                                    new_mat[0][3] - ctx2d.parent_canvas_origin[0];
                                                let dy =
                                                    new_mat[1][3] - ctx2d.parent_canvas_origin[1];
                                                let (sin_p, cos_p) =
                                                    ctx2d.parent_world_rot.sin_cos();
                                                ...
                                                if ctx2d.sm_transform {
                                                    let sx = if ctx2d.cumul_scale[0].abs()
                                                        > f32::EPSILON
                                                    {
                                                        ctx2d.cumul_scale[0]
                                                    } else {
                                                    ...
                                                    let sy = if ctx2d.cumul_scale[1].abs()
                                                        > f32::EPSILON
                                                    {
                                                        ctx2d.cumul_scale[1]
                                                    } else {
                                                ...
                                                let pos_inv_scale = if use_ss_c {
                                                    1.0
                                                } else {
                                                    1.0 / CANVAS_WORLD_SCALE
                                                };
                                                let y_inv_sign =
                                                    if use_ss_c { 1.0f32 } else { -1.0 };
                                                ct.position[0] =
                                                    new_mat[0][3] * pos_inv_scale - anchor_off[0];
                                                ct.position[1] =
                                                    new_mat[1][3] * pos_inv_scale * y_inv_sign
                                                        - anchor_off[1];
                                            }
                                            match self.tool_mode {
                                                crate::engine::core::app_base::ipc::ToolMode::Rotate => {
                        ...
                        } else if let Some((drag_dfs, ref start_tf)) =
                            self.drag.actor_transform_drag_start.clone()
                        {
                    ...
                    if !self.drag.multi_actor_drag_starts.is_empty() {
                        ...
                        for (other_dfs, start_mat) in &multi_starts {
                            let mut c = 0u32;
                            if let Some(actor) =
                                find_actor_by_dfs(&scene.actors, wl, *other_dfs, &mut c)
                            {
                                ...
                                if let Some(tf) =
                                    scene.world.get_mut::<ActorTransform>(actor_entity)
                                {
            ...
            if let Some(drag) = self.try_gizmo_hit_and_start(cx, cy) {
                ...
                if let Some(scene) = self.scene.as_ref() {
                    ...
                    if let Some(mc) = mc_entity.and_then(|e| scene.world.get::<ModelComponent>(e)) {
                        ...
                        self.drag.drag_root_starts = roots
                            .iter()
                    ...
                    if let Some(dfs) = selected_dfs {
                        ...
                        if let Some(actor) =
                            find_actor_by_dfs(&scene.actors, wl, dfs as u32, &mut c)
                        {
                            self.drag.actor_extra_mc_drag_starts = actor
                                .slots()
                                .iter()
                                ...
                                find_actor_by_dfs(&scene.actors, wl, dfs as u32, &mut c)
                            {
                                ...
                                if actor.is_2d() {
                                    ...
                                    let old_ct = scene
                                        ...
                                        .cloned()
                                        .unwrap_or_default();
                                    self.drag.canvas_transform_drag_start =
                                        Some((dfs as u32, old_ct));
                                } else {
                                    let old_tf = scene
                                        .world
                                        ...
                                        .cloned()
                                        .unwrap_or_default();
                                    self.drag.actor_transform_drag_start =
                                        Some((dfs as u32, old_tf));
                                }
                            }
                        }
                    }
                    ...
                    if self.selected_actor_dfs_ids.len() > 1 {
                        for &other_dfs in &self.selected_actor_dfs_ids {
                            if Some(other_dfs) == selected_dfs {
                                continue;
                            }
                            let mut c = 0u32;
                            if let Some(actor) =
                                find_actor_by_dfs(&scene.actors, wl, other_dfs as u32, &mut c)
                            {
                                let start_mat = actor
                                    .mc_entity()
                                ...
                                self.drag
                                    .multi_actor_drag_starts
                                    .push((other_dfs as u32, start_mat));
                            }
                        }
                    }
                }
    ...
    pub(super) fn handle_lmb_release(&mut self) {
        ...
        } else if self.drag.gizmo_drag.is_none()
            && self.drag.lmb_press_pos.is_some()
            && (self.mode == RuntimeMode::Edit || self.paused)
        {
            if let Some((cx, cy)) = self.last_cursor_pos {
        ...
        if self.drag.gizmo_drag.is_some() {
            ...
            if let Some((canvas_drag_dfs, old_ct)) = self.drag.canvas_transform_drag_start.take() {
                ...
                let new_ct_opt = self.scene.as_ref().and_then(|s| {
                    let mut c = 0u32;
                    find_actor_by_dfs(&s.actors, wl, canvas_drag_dfs, &mut c).and_then(|a| {
                        s.world
                            .get::<crate::engine::components::CanvasTransform>(a.entity)
                            .cloned()
                    })
                });
            ...
            if let Some((dfs_id, old_transform)) = self.drag.actor_transform_drag_start.take() {
                ...
                if let Some(new_transform) = new_transform_opt {
                    let delta =
                        mat4x4_mul(new_transform.to_mat4(), mat4x4_inv(old_transform.to_mat4()));
                    let mut child_transforms: Vec<(
                        u32,
                        ActorTransform,
                        ActorTransform,
                        [[f32; 4]; 4],
                    ...
                    for (child_dfs, start_mat) in child_drag_starts {
                        ...
                        if let Some(scene) = &mut self.scene {
                            ...
                            if let Some(child_entity) = child_entity {
                                ...
                                if let Some(tf) =
                                    scene.world.get_mut::<ActorTransform>(child_entity)
                                {
                                    *tf = new_child_tf.clone();
                                }
                                if let Some(mc) =
                                    scene.world.get_mut::<ModelComponent>(child_entity)
                                {
                                    if let Some(m) = mc.instance_mats.first_mut() {
                                        *m = new_mc_mat;
                                    }
                    ...
                    if old_transform != new_transform || !child_transforms.is_empty() {
                        self.undo_history
                            .record(Box::new(ActorGroupTransformCommand {
                                wl,
                                dfs_id,
                ...
                if let Some(mc) =
                    mc_entity.and_then(|e| self.scene.as_ref()?.world.get::<ModelComponent>(e))
                {
                ...
                let extra_slot_transforms: Vec<(usize, u32, [[f32; 4]; 4], [[f32; 4]; 4])> =
                    extra_mc_starts
                        .iter()
                        .flat_map(|(slot_i, start_mats)| {
                            let cur_mats: Vec<[[f32; 4]; 4]> = selected_dfs_end
                                .and_then(|dfs| {
                                    let mut c = 0u32;
                                    self.scene.as_ref().and_then(|s| {
                                        find_actor_by_dfs(&s.actors, wl_end, dfs as u32, &mut c)
                ...
                if self.actor_virtual_selected_idx.is_some() {
                    ...
                    let child_drag_starts = std::mem::take(&mut self.drag.actor_child_drag_starts);
                    let (old_tf, new_tf, child_transforms) = if let Some(&(_, old_mat, new_mat)) =
                        transforms.first()
                    {
                        ...
                        for (child_dfs, start_mat) in child_drag_starts {
                            if let Some(scene) = &mut self.scene {
                                ...
                                if let Some(child_entity) = child_entity_opt {
                                    ...
                                    let new_child_tf =
                                        ActorTransform::from_mat4(&mat4x4_mul(delta, start_mat));
                                    if let Some(tf) =
                                        scene.world.get_mut::<ActorTransform>(child_entity)
                                    {
                                        *tf = new_child_tf.clone();
                    ...
                    if !transforms.is_empty() {
                        self.undo_history
                            .record(Box::new(MultiTransformCommand { transforms }));
                        primary_recorded = true;
                        if let Some(ipc) = &self.ipc {
                            ipc.send("SCENE_MODIFIED");
                        }
        ...
        if !self.drag.multi_actor_drag_starts.is_empty() {
            ...
            if !drag_transforms.is_empty() {
                let multi_cmd: Box<dyn crate::engine::core::app_base::undo::Command> =
                    Box::new(MultiActorDragTransformCommand {
                        wl,
                        transforms: drag_transforms,
                    });
                if primary_recorded {
