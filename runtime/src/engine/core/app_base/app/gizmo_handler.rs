...
use crate::engine::core::app_base::ipc::{GizmoSpace, ToolMode};
use crate::engine::methods::gizmo_interact::{
    GIZMO_SCREEN_RADIUS_RATIO, GizmoDrag, GizmoPart, hit_test_gizmo, hit_test_gizmo_canvas,
    hit_test_gizmo_oriented, screen_to_ray, screen_to_ray_ortho, screen_to_ray_ortho3d, start_drag,
    start_drag_oriented,
};

use super::{
    App, RuntimeMode, canvas_anchor_offset_for_dfs, find_actor_by_dfs, find_parent_actor_of_dfs,
    get_3d_canvas_world_mat, selection_centroid,
};
...
impl App {
    ...
    pub(super) fn selected_primary_actor_is_2d(&self) -> bool {
        let scene = if let Some(s) = self.scene.as_ref() {
            s
        } else {
            return false;
        };
    ...
    pub(super) fn selected_primary_is_viewport_root_canvas(&self) -> bool {
        ...
        let mut c = 0u32;
        let Some(actor) = find_actor_by_dfs(&scene.actors, wl, dfs as u32, &mut c) else {
            return false;
        };
    ...
    pub(super) fn editor_3d_gizmo_radius(&self, gizmo_pos: [f32; 3]) -> f32 {
        if self.camera.is_ortho() {
            ...
            let d = [
                gizmo_pos[0] - cam_pos.x,
                gizmo_pos[1] - cam_pos.y,
                gizmo_pos[2] - cam_pos.z,
            ];
    ...
    pub(super) fn try_gizmo_hit_and_start(&self, cx: f32, cy: f32) -> Option<GizmoDrag> {
        ...
        if let Some([ax, ay, az]) = self.selected_canvas_child_axes() {
            let part =
                hit_test_gizmo_canvas(ray_o, ray_d, gizmo_pos, radius, self.tool_mode, ax, ay, az)?;
