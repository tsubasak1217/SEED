...
use crate::engine::components::{
    AspectRatioAxis, CanvasComponent, CanvasTransform, CanvasViewportRef, Collider2dComponent,
    ComponentKind, GravityMode, ModelComponent, Transform as ActorTransform,
};
use crate::engine::core::app_base::undo::{ActorGroupTransformCommand, TransformCommand};
...
use super::{
    App, RuntimeMode, apply_delta_to_actor_children, canvas_anchor_offset_for_dfs,
    find_actor_by_dfs, find_actor_by_dfs_mut, find_parent_actor_of_dfs, get_3d_canvas_world_mat,
};
...
impl App {
    ...
    pub(super) fn apply_set_transform(
        &mut self,
        id: u32,
        px: f32,
        py: f32,
                ...
                if let Some(mc) = scene.world.get_mut::<ModelComponent>(slot_entity) {
                    ...
                    for (i, &old) in old_mats.iter().enumerate() {
                        if let Some(new) = mc.instance_mats.get(i).copied() {
                            if new != old {
                                all_changes.push((i as u32, old, new));
                            }
                        }
                    }
                }
    ...
    pub(super) fn handle_set_collider2d_aspect_ratio(
        ...
        let aspect_axis = if axis == "height" {
            AspectRatioAxis::Height
        } else {
            AspectRatioAxis::Width
        };
