...
use crate::engine::components::{GROUP_ID_BASE, GroupMeta, ModelComponent};
...
use super::{App, find_actor_by_dfs, release_window_clamp};
...
impl App {
            ...
            match cmd {
                ...
                IpcCommand::Undo => {
                    ...
                    if let Some((structural, sel)) = result {
                        // アクターツリー再構築
                        if let Some((wl, actors_data)) =
                            self.undo_history.peek_undone_actor_rebuild()
                        {
                            self.rebuild_actors_for_wl(wl, actors_data);
                        }
                        // コンポーネントスロット再構築
                        if let Some((wl, actor_dfs_id, slots_data)) =
                            self.undo_history.peek_undone_component_rebuild()
                        {
                            self.rebuild_actor_slots(wl, actor_dfs_id, slots_data);
                            self.send_actor_components(
                                actor_dfs_id,
                                self.actor_virtual_selected_slot_idx,
                            );
                        }
                        ...
                        if let Some((dfs_ids, primary)) =
                            self.undo_history.peek_undone_actor_dfs_selection()
                        {
                ...
                IpcCommand::Redo => {
                    ...
                    if let Some((structural, sel)) = result {
                        // アクターツリー再構築
                        if let Some((wl, actors_data)) =
                            self.undo_history.peek_redone_actor_rebuild()
                        {
                            self.rebuild_actors_for_wl(wl, actors_data);
                        }
                        // コンポーネントスロット再構築
                        if let Some((wl, actor_dfs_id, slots_data)) =
                            self.undo_history.peek_redone_component_rebuild()
                        {
                            self.rebuild_actor_slots(wl, actor_dfs_id, slots_data);
                            self.send_actor_components(
                                actor_dfs_id,
                        ...
                        // アクター DFS 選択の復元（ActorDfsSelectionCommand）
                        if let Some((dfs_ids, primary)) =
                            self.undo_history.peek_redone_actor_dfs_selection()
                        {
                            self.selected_actor_dfs_ids = dfs_ids;
                            self.actor_virtual_selected_idx = primary;
                            self.selected_instances.clear();
                ...
                IpcCommand::Rename { idx, name } => {
                    if let Some(scene) = &mut self.scene {
                        if let Some(mc) = scene.find_component_in_world_line_mut::<ModelComponent>(
                        ...
                        if let Some(mc) = scene.find_component_in_world_line_mut::<ModelComponent>(
                            ...
                            for child in children {
                                if child >= GROUP_ID_BASE {
                                    if let Some(g) =
                                        mc.group_meta.iter_mut().find(|g| g.id == child)
                                    {
                                        g.parent = Some(gid);
                                    }
                ...
                IpcCommand::BeginTransformDrag {
                    ...
                    if !is_actor {
                        ...
                        if let Some(old_mat) = old_mat {
                            self.inspector_transform_drag =
                                Some(super::InspectorTransformDrag::Instance {
                                    idx: target_id,
                                    old_mat,
                                });
                        }
                        ...
                        if wl != 0 {
                            ...
                            let snapshot = self.scene.as_ref().and_then(|s| {
                                find_actor_by_dfs(&s.actors, wl, target_id, &mut c).map(|actor| {
                                    ...
                                    super::collect_child_actor_old_states(
                                        actor,
                                        &s.world,
                                        &mut child_dfs,
                                        &mut child_old_states,
                                    );
                            ...
                            if let Some((old_mats, old_tf, child_old_states)) = snapshot {
                                if !old_mats.is_empty() {
                                    self.inspector_transform_drag =
                                        Some(super::InspectorTransformDrag::ActorGroup {
                                            dfs_id: target_id,
                                            old_mats,
                                            old_tf,
                                            child_old_states,
                                    ...
                                    if let Some(old_ct) = old_ct {
                                        self.inspector_transform_drag =
                                            Some(super::InspectorTransformDrag::CanvasActor {
                                                wl,
                                                dfs_id: target_id,
                                                old_ct,
                                            });
                                    ...
                                    self.inspector_transform_drag =
                                        Some(super::InspectorTransformDrag::Actor {
                                            wl,
                                            dfs_id: target_id,
                                            old_tf,
                                        });
                            ...
                            let old_tf = self.scene.as_ref().and_then(|s| {
                                let mut c = 0u32;
                                find_actor_by_dfs(&s.actors, wl, target_id, &mut c).and_then(|a| {
                                    s.world
                                        .get::<crate::engine::components::Transform>(a.entity)
                                        .cloned()
                                })
                            });
                        ...
                        match drag {
                            super::InspectorTransformDrag::Instance { idx, old_mat } => {
                                if let Some(mc) = self.scene.as_ref().and_then(|s| {
                                    s.find_component_in_world_line::<ModelComponent>(wl)
                                }) {
                            ...
                            super::InspectorTransformDrag::Actor {
                                wl: drag_wl,
                                dfs_id,
                                old_tf,
                            } => {
                                {
                                    if let Some(scene) = &self.scene {
                                        let mut c = 0u32;
                                        let new_tf_opt = find_actor_by_dfs(
                                            &scene.actors,
                                            drag_wl,
                                            dfs_id,
                                            &mut c,
                                        ...
                                        .and_then(|a| {
                                            scene
                                                .world
                                                .get::<crate::engine::components::Transform>(
                                                    a.entity,
                                                )
                                                .cloned()
                                        });
                            ...
                            super::InspectorTransformDrag::ActorGroup {
                                dfs_id,
                                old_mats,
                                old_tf,
                                child_old_states,
                                ...
                                let child_transforms: Vec<(
                                    u32,
                                    crate::engine::components::Transform,
                                    crate::engine::components::Transform,
                                    [[f32; 4]; 4],
                                    [[f32; 4]; 4],
                                )>;
                            ...
                            super::InspectorTransformDrag::CanvasActor {
                                wl: drag_wl,
                                dfs_id,
                                old_ct,
                            } => {
                                ...
                                let new_ct_opt = self.scene.as_ref().and_then(|s| {
                                    let mut c = 0u32;
                                    find_actor_by_dfs(&s.actors, drag_wl, dfs_id, &mut c).and_then(
                                        |a| {
                                            s.world
                                                .get::<crate::engine::components::CanvasTransform>(
                                                    a.entity,
                                                )
                                                .cloned()
                                        },
                ...
                IpcCommand::SetCameraFov(deg) => {
                    self.camera.base.projection.fov_y_rad = deg * std::f32::consts::PI / 180.0;
                }
                ...
                IpcCommand::SetEditViewMode { is_2d } => {
                    ...
                    if self.mode == RuntimeMode::Edit {
                        ...
                        if tab_changed {
                            self.leave_current_tab_physics();
                        }
                    ...
                    match result {
                        Some(Ok((mut new_scene, cam_data))) => {
                            ...
                            if let Some(old_scene) = self.scene.take() {
                                for actor in
                                    old_scene.actors.into_iter().filter(|a| a.world_line > 0)
                                {
                            ...
                            if let Some(ipc) = &self.ipc {
                                ...
                                let (pos, euler_x, euler_y, euler_z, fov, far, spd) =
                                    self.cam_state_tuple();
                    ...
                    if self.draw_ctx.is_some() {
                        ...
                        let main_scene = self.scene.get_or_insert_with(|| Scene::new("main"));
                        {
                            fn collect_entities(
                                ...
                                out: &mut Vec<crate::engine::ecs::Entity>,
                            ) {
                ...
                IpcCommand::SetCanvasTransform {
                    ...
                    self.handle_set_canvas_transform(
                        dfs_id, px, py, rotation, sx, sy, pivot_x, pivot_y,
                    );
                }
                ...
                IpcCommand::SetMaterialOverride {
                    actor_dfs_id,
                    slot_idx,
                    mat_slot,
                    json,
                ...
                IpcCommand::SetJointAttachField {
                    actor_dfs_id,
                    slot_idx,
                    key,
                    value,
                ...
                IpcCommand::SetCanvasTransformScaleMode {
                    actor_dfs_id,
                    scale_transform,
                    scale_size,
                    keep_aspect,
                    ...
                    self.handle_set_canvas_transform_scale_mode(
                        actor_dfs_id,
                        scale_transform,
                        scale_size,
                        keep_aspect,
                ...
                IpcCommand::SetCanvasGravityMode {
                    actor_dfs_id,
                    slot_idx,
                    mode,
                } => {
                ...
                IpcCommand::SetCollider2dAspectRatio {
                    actor_dfs_id,
                    slot_idx,
                    keep,
                    axis,
                ...
                IpcCommand::SetCameraComponentClearColor {
                    actor_dfs_id,
                    slot_idx,
                    r,
                    g,
                ...
                IpcCommand::SetCameraComponentScalingMode {
                    actor_dfs_id,
                    slot_idx,
                    mode,
                } => {
                ...
                IpcCommand::SetCameraComponentTargetSize {
                    actor_dfs_id,
                    slot_idx,
                    width,
                    height,
                ...
                IpcCommand::SetCameraComponentProjection {
                    actor_dfs_id,
                    slot_idx,
                    mode,
                } => {
                ...
                IpcCommand::SetActorActive { dfs_id, active } => {
                    ...
                    if let Some(scene) = self.scene.as_mut() {
                        let mut c = 0u32;
                        if let Some(actor) =
                            super::find_actor_by_dfs_mut(&mut scene.actors, wl, dfs_id, &mut c)
                        {
                            actor.active = active;
                            if let Some(ipc) = &self.ipc {
                ...
                IpcCommand::SetSlotEnabled {
                    ...
                    if let Some(scene) = self.scene.as_mut() {
                        let mut c = 0u32;
                        if let Some(actor) = super::find_actor_by_dfs_mut(
                            &mut scene.actors,
                            wl,
                            actor_dfs_id,
                            &mut c,
    ...
    fn handle_set_component_value(
        ...
        match kind {
                ...
                match key {
                    "fov" | "fov_y_deg" => {
                        if let Ok(v) = value.parse::<f32>() {
                            self.handle_set_camera_fov(actor_dfs_id, slot_idx, v);
                        }
                    }
                    "near" => {
                        if let Ok(v) = value.parse::<f32>() {
                            self.handle_set_camera_near(actor_dfs_id, slot_idx, v);
                        }
                    }
                    "far" => {
                        if let Ok(v) = value.parse::<f32>() {
                            self.handle_set_camera_far(actor_dfs_id, slot_idx, v);
                        }
                    }
                    "is_main" => {
                    ...
                    "clear_color" => {
                        ...
                        let parts: Vec<f32> = value
                            ...
                            .collect();
                        if parts.len() == 4 {
                            self.handle_set_camera_clear_color(
                                actor_dfs_id,
                                slot_idx,
                                parts[0],
                                parts[1],
            ...
            ComponentKind::Sprite => match key {
                "texture_path" | "path" => {
                        ...
                        let h = {
                            ...
                            find_actor_by_dfs(&scene.actors, wl, actor_dfs_id, &mut c)
                                ...
                                .and_then(|s| {
                                    scene
                                        .world
                                        .get::<crate::engine::components::SpriteComponent>(s.entity)
                                })
                                .map(|s| s.height)
                                .unwrap_or(100.0)
                        };
            ...
            ComponentKind::Canvas => match key {
                "width" => {
                    if let Ok(v) = value.parse::<f32>() {
                        let h = {
                            let scene = self.scene.as_ref().unwrap();
                            ...
                            find_actor_by_dfs(&scene.actors, wl, actor_dfs_id, &mut c)
                                ...
                                .and_then(|s| {
                                    scene
                                        .world
                                        .get::<crate::engine::components::CanvasComponent>(s.entity)
                                })
                                .map(|s| s.height)
                                .unwrap_or(1080.0)
                        };
    ...
    fn handle_set_plugin_field(
        ...
        use crate::engine::components::{ComponentKind, PluginComponent};
