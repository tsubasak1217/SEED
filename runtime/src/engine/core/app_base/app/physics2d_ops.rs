...
impl App {
        ...
        contexts
            ...
            .map(|c| {
                (
                    c.actor_entity,
                    [
                        c.body_pos_px[0] + win_w * 0.5,
                        c.body_pos_px[1] + win_h * 0.5,
                    ],
                )
            })
            .collect()
    ...
    pub(super) fn start_physics_2d(&mut self) {
        ...
        for ctx in &contexts {
            ...
            let Some(collider) = scene.world.get::<Collider2dComponent>(slot_entity) else {
                continue;
            };
        ...
        if *PHYS_LOG_ENABLED {
            eprintln!(
                ...
                contexts
                    .iter()
                    .filter(|c| c.collider_slot_entity.is_some())
                    .count(),
                self.active_world_line,
                force_kinematic,
            );
        ...
        if force_kinematic {
            thread.send(PhysicsCommand2d::SetGravity {
                gravity: [0.0, 0.0],
            });
        }
    ...
    pub(super) fn update_physics_2d(&mut self) {
        ...
        let Some(_) = &self.physics_thread_2d else {
            return;
        };
        ...
        let new_drag_entity_id: Option<u64> = if self.drag.gizmo_drag.is_some() {
            ...
            match &self.inspector_transform_drag {
                Some(InspectorTransformDrag::CanvasActor { dfs_id, .. }) => {
        ...
        if new_drag_entity_id != self.dragging_physics_2d_entity_id {
            if let Some(old_id) = self.dragging_physics_2d_entity_id {
                ...
                let final_position = contexts.iter().find(|c| c.dfs_id == old_id).map(|c| {
                    (
                        [
                            c.body_pos_px[0] / PIXELS_PER_METER,
                            c.body_pos_px[1] / PIXELS_PER_METER,
                        ],
                        c.rot_rad,
                    )
                });
            ...
            if let Some(new_id) = new_drag_entity_id {
                let ecs_start_pos = contexts.iter().find(|c| c.dfs_id == new_id).map(|c| {
                    (
                        [
                            c.body_pos_px[0] / PIXELS_PER_METER,
                            c.body_pos_px[1] / PIXELS_PER_METER,
        ...
        if let Some(ref result) = result {
            ...
            if should_apply_transforms {
                if let Some(scene) = &mut self.scene {
                    ...
                    for (entity_id, new_pos, new_rot) in &result.transform_updates {
                        if Some(*entity_id) == self.dragging_physics_2d_entity_id {
                            continue;
                        }
            ...
            self.store_edit_physics_rest_speeds_2d(
            ...
            if let (Some(thread), Some(scene)) = (&self.physics_thread_2d, &self.scene) {
                for ctx in &contexts {
                    ...
                    if collider.use_rigidbody {
                        ...
                        // 旧ボディを削除して新位置で再登録する
                        thread.send(PhysicsCommand2d::RemoveObject {
                            entity_id: ctx.dfs_id,
                        });
                        thread.send(PhysicsCommand2d::AddObject(PhysicsObject2d {
                            entity_id: ctx.dfs_id,
    ...
    fn apply_drag_pushback_2d(&mut self, drag_id: u64, ctx: &Actor2dPhysicsCtx) {
        ...
        //    reply チャンネルパターン）
        let overlapping = {
            let Some(thread) = &self.physics_thread_2d else {
                return;
            };
        ...
        // ── めり込みあり: 最終有効位置に押し戻す ─────────────────────────────
        let Some((last_pos, last_rot)) = self.drag_collider_last_valid_pos_2d else {
            return;
        };
...
fn compute_canvas_local_transform(
    ...
    let new_body_px = [new_pos[0] * PIXELS_PER_METER, new_pos[1] * PIXELS_PER_METER];
    ...
    let (sin_p, cos_p) = ctx.parent_world_rot.sin_cos();
    let eff_pos_local = [cos_p * dx + sin_p * dy, -sin_p * dx + cos_p * dy];
...
impl App {
    ...
    pub(super) fn compute_scene_gravity_2d(&self) -> [f32; 2] {
        ...
        for actor in scene
            .actors
            .iter()
            .filter(|a| a.world_line == wl && !a.is_2d())
        {
            let Some(slot) = actor
                .slots()
                .iter()
            ...
            let Some(cc) = scene.world.get::<CanvasComponent>(slot.entity) else {
                continue;
            };
