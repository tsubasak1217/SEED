...
fn billboard_quad(center: [f32; 3], cam_pos: [f32; 3], radius: f32) -> Option<[[f32; 4]; 4]> {
    if radius < DEGENERATE_SIZE_EPS {
        return None;
    }
    // 視線方向（中心 − カメラ）。縮退時は Z+ を仮の視線とする。
    let dir =
        v_normalize(v_sub(center, cam_pos), AXIS_VIEW_PARALLEL_EPS).unwrap_or([0.0, 0.0, 1.0]);
...
fn axis_billboard_quad(
    ...
    if half_len < DEGENERATE_SIZE_EPS || half_wid < DEGENERATE_SIZE_EPS {
        return None;
    }
    let dir =
        v_normalize(v_sub(center, cam_pos), AXIS_VIEW_PARALLEL_EPS).unwrap_or([0.0, 0.0, 1.0]);
    ...
    match shape {
        ...
        ColliderShapeData::Box { half_extents } => {
            ...
            for (u_dir, hu, v_dir, hv) in planes {
                if hu < DEGENERATE_SIZE_EPS || hv < DEGENERATE_SIZE_EPS {
                    continue;
                }
        ...
        ColliderShapeData::Sphere { radius } => {
            ...
            if let Some(m) = billboard_quad(pos, cam_pos, r) {
                out.push(m);
            }
        }
        ...
        ColliderShapeData::Capsule {
            ...
            if let Some(m) = axis_billboard_quad(pos, cam_pos, axis, hh + r, r) {
                out.push(m);
            }
        }
        ...
        ColliderShapeData::Cylinder {
            ...
            let hh = half_height * scale[1].abs();
            let axis = rot([0.0, 1.0, 0.0]);
            if let Some(m) = axis_billboard_quad(pos, cam_pos, axis, hh, r) {
                out.push(m);
            }
        }
        ...
        ColliderShapeData::ConvexHull { vertices } => {
            ...
            if let Some(m) = billboard_quad(pos, cam_pos, max_r2.sqrt()) {
                out.push(m);
            }
        }
