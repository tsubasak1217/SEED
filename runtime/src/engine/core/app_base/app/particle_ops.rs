...
impl App {
    ...
    pub(super) fn handle_set_particle_field(
        ...
        match key {
            ...
            "shape" => if let Some(s) = ParticleShape::from_tag(value, pe.shape.model_path()) {
                pe.shape = s;
            },
            // shape_model_path: 現在 Model 形状ならパスを差し替え、Model 以外なら
            // Model 形状へ切り替えつつパスを設定する。
            "shape_model_path" => {
                pe.shape = ParticleShape::Model { path: value.to_string() };
            },
            // ─── 出現範囲（スポーン体積） ─────────────────────────
            "spawn_volume" => if let Some(v) = SpawnVolume::from_tag(
                value, pe.spawn_volume.box_half_extents(), pe.spawn_volume.sphere_radius(),
            ) {
                pe.spawn_volume = v;
            },
            "spawn_box_x" => if let Ok(v) = value.parse::<f32>() {
                let mut half = pe.spawn_volume.box_half_extents();
                half[0] = v.max(0.0);
                pe.spawn_volume = SpawnVolume::Box { half_extents: half };
            },
            "spawn_box_y" => if let Ok(v) = value.parse::<f32>() {
                let mut half = pe.spawn_volume.box_half_extents();
                half[1] = v.max(0.0);
                pe.spawn_volume = SpawnVolume::Box { half_extents: half };
            },
            "spawn_box_z" => if let Ok(v) = value.parse::<f32>() {
                let mut half = pe.spawn_volume.box_half_extents();
                half[2] = v.max(0.0);
                pe.spawn_volume = SpawnVolume::Box { half_extents: half };
            },
            "spawn_sphere_radius" => if let Ok(v) = value.parse::<f32>() {
                pe.spawn_volume = SpawnVolume::Sphere { radius: v.max(0.0) };
            },
            // ─── 放出制御 ───────────────────────────────────────
            "emit_mode" => if let Some(m) = EmitMode::from_tag(value, pe.emit_mode.count_total()) {
                pe.emit_mode = m;
            },
            "emit_count_total" => if let Ok(v) = value.parse::<u32>() {
                pe.emit_mode = EmitMode::Count { total: v };
            },
            "initial_delay"      => if let Ok(v) = value.parse::<f32>() { pe.initial_delay = v.max(0.0); },
            "prewarm_time"       => if let Ok(v) = value.parse::<f32>() { pe.prewarm_time = v.max(0.0); },
            "emit_interval"      => if let Ok(v) = value.parse::<f32>() { pe.emit_interval = v.max(0.0); },
            "particles_per_emit" => if let Ok(v) = value.parse::<u32>() { pe.particles_per_emit = v; },
            // ─── 寿命 ───────────────────────────────────────────
            ...
            "direction_randomness" => if let Ok(v) = value.parse::<f32>() {
    ...
    pub(super) fn handle_set_particle_curve(
        ...
        let ok = match curve_id {
            "speed" => match serde_json::from_str(json) {
                Ok(c)  => { pe.speed_curve = c; true }
                Err(e) => { eprintln!("[SEED] SET_PARTICLE_CURVE(speed): json パース失敗（{e}）: {json}"); false }
            },
