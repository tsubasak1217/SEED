...
static ICON_BYTES: &[u8] = include_bytes!(
...
impl IconOverlay {
    ...
    pub fn build(
        ...
        if positions_screen.is_empty() { return None; }
        ...
        for &(sx, sy) in positions_screen {
            ...
            // 三角形 1: TL → TR → BR
            verts.push(IconOverlayVertex { position: tl, uv: [0.0, 0.0] });
            verts.push(IconOverlayVertex { position: tr, uv: [1.0, 0.0] });
            verts.push(IconOverlayVertex { position: br, uv: [1.0, 1.0] });
            // 三角形 2: TL → BR → BL
            verts.push(IconOverlayVertex { position: tl, uv: [0.0, 0.0] });
            verts.push(IconOverlayVertex { position: br, uv: [1.0, 1.0] });
            verts.push(IconOverlayVertex { position: bl, uv: [0.0, 1.0] });
        }
        ...
        Some(GpuIconOverlayBatch { vertex_buf, vertex_count })
    }
