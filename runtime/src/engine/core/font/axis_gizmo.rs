...
const AXES: [AxisDef; 3] = [
    AxisDef { col: 0, color_pos: Color::GIZMO_X.to_array(), color_neg: [0.55, 0.14, 0.14, 0.70], label: "X" },
    AxisDef { col: 1, color_pos: Color::GIZMO_Y.to_array(), color_neg: [0.28, 0.44, 0.00, 0.70], label: "Y" },
    AxisDef { col: 2, color_pos: Color::GIZMO_Z.to_array(), color_neg: [0.10, 0.25, 0.55, 0.70], label: "Z" },
];
...
impl AxisGizmo {
    pub fn new(
        ...
        let font_system = FontSystem::new(device, surface_format, depth_format, FontConfig::default())
    ...
    pub fn build(
        ...
        // cam_z 降順（奥→手前）でソート
        endpoints.sort_by(|a, b| b.cam_z.partial_cmp(&a.cam_z)
        ...
        for ep in &endpoints {
            ...
            push_circle_px(&mut geo, ep.tip_x, ep.tip_y,
                dot_r, screen_w, screen_h, color);
        }

        // 中心ドット（最前面）
        push_circle_px(&mut geo, cx, cy, CTR_RADIUS, screen_w, screen_h,
            [0.92, 0.92, 0.92, 1.0]);
        ...
        for ep in endpoints.iter().filter(|e| e.is_pos) {
            ...
            text_batch.add_text_screen(
                ax.label, lx, ly, LABEL_SIZE,
                [1.0, 1.0, 1.0, 1.0], &glyphs,
                screen_w, screen_h,
            );
        }
        ...
        GpuAxisGizmoBatch { geo_buf, geo_count, text }
    }
...
fn push_thick_line_px(
    ...
    verts.push(AxisGizmoVertex { position: pts[0], color });
    verts.push(AxisGizmoVertex { position: pts[1], color });
    verts.push(AxisGizmoVertex { position: pts[2], color });
    verts.push(AxisGizmoVertex { position: pts[0], color });
    verts.push(AxisGizmoVertex { position: pts[2], color });
    verts.push(AxisGizmoVertex { position: pts[3], color });
}
...
fn push_circle_px(
    ...
    for i in 0..n {
        ...
        verts.push(AxisGizmoVertex { position: center, color });
        verts.push(AxisGizmoVertex { position: p0,     color });
        verts.push(AxisGizmoVertex { position: p1,     color });
    }
}
