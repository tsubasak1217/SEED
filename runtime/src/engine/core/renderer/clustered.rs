...
pub fn light_bounding_sphere(light: &GpuLight) -> Option<([f32; 3], f32)> {
    ...
    let radius = if light.kind == LIGHT_KIND_RECT {
        let half_diag = (light.rect_half_width * light.rect_half_width
            + light.rect_half_height * light.rect_half_height)
            .sqrt();
...
mod tests {
    ...
    use crate::engine::core::renderer::lighting::{
        LIGHT_KIND_POINT, LIGHT_KIND_RECT, LIGHT_KIND_SPOT,
    };
    ...
    fn wgsl_const(src: &str, name: &str) -> String {
        let decl = src
            ...
            .unwrap_or_else(|| {
                panic!("cluster_common.wgsl に const {name} の宣言が見つかりません")
            });
        decl.split('=')
            .nth(1)
    ...
    fn wgsl_constants_match_rust() {
        ...
        assert_eq!(
            wgsl_const_u32(src, "MAX_LIGHTS_PER_CLUSTER"),
            MAX_LIGHTS_PER_CLUSTER
        );

        // マージンは f32 リテラル。
        let margin: f32 = wgsl_const(src, "CLUSTER_LIGHT_BOUND_MARGIN")
            .parse()
            .unwrap();
        ...
        assert_eq!(
            wgsl_const_u32(src, "LIGHT_KIND_DIRECTIONAL"),
        ...
        assert_eq!(
            CLUSTER_COUNT,
            CLUSTER_TILES_X * CLUSTER_TILES_Y * CLUSTER_SLICES_Z
        );
    }
    ...
    fn wgsl_workgroup_size_matches_rust() {
        ...
        let inner = decl
            .split('(')
            .nth(1)
            .expect("workgroup_size の '(' がありません")
            .split(')')
            .next()
            .expect("workgroup_size の ')' がありません");
        let dims: Vec<u32> = inner
            ...
            .map(|s| {
                s.trim()
                    .parse::<u32>()
                    .expect("workgroup_size が整数ではありません")
            })
            .collect();
    ...
    fn wgsl_gpu_light_struct_mirrors_light_common() {
        ...
        fn fields(src: &str) -> Vec<String> {
            let start = src
                .find("struct GpuLight {")
                .expect("struct GpuLight の宣言が見つかりません");
            let body = &src[start..];
            let end = body
                .find('}')
                .expect("struct GpuLight の '}' が見つかりません");
            body[..end]
                .lines()
    ...
    fn z_slice_is_inverse_of_slice_depth() {
        ...
        for k in 0..CLUSTER_SLICES_Z {
            ...
            let mid = (d0 * d1).sqrt();
            assert_eq!(
                cluster_z_slice(mid, near, far),
                k,
                "スライス {k} の内部点が別スライスへ落ちた"
            );
            // 深度は単調増加。
    ...
    fn light_bounds_are_conservative() {
        ...
        assert!(
            light_bounding_sphere(&dir).is_none(),
            "平行光はクラスタに入れない"
        );
        ...
        assert!(
            r >= 10.0,
            "点光源の半径が range を下回っている（影響範囲を切り落とす）"
        );
        ...
        let rect = GpuLight::rect(
            ...
            10.0,
            6.0,
            8.0, // 幅 6 高 8 → 半幅 3 半高 4 → 半対角 5
        );
        let (_, rr) = light_bounding_sphere(&rect).unwrap();
        assert!(
            rr >= 15.0,
            "Rect の半径が range + 半対角（15）を下回っている: {rr}"
        );
    ...
    fn light_at_view_center_is_collected() {
        ...
        for z in 0..CLUSTER_SLICES_Z {
            for y in 0..CLUSTER_TILES_Y {
                for x in 0..CLUSTER_TILES_X {
                    let (mn, mx) = cluster_view_aabb(x, y, z, near, far, tan_x, tan_y);
                    if sphere_aabb_intersects(c, r, mn, mx) {
                        hits += 1;
                    }
                }
            }
    ...
    fn params_layout() {
        ...
        assert_eq!(
            ...
            112,
            "ClusterParams は 112 バイト（16 の倍数）"
        );
