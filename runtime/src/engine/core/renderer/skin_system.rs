...
use crate::engine::core::loader::model::{Model, AnimationOutputs, Interpolation};
...
impl SkinComputeSystem {
    ...
    pub fn new(
        ...
        // ── バインドポーズ TRS ────────────────────────────────
        let bind_t: Vec<[f32; 4]> = model.nodes.iter()
            .map(|n| [n.translation[0], n.translation[1], n.translation[2], 0.0])
            .collect();
        let bind_r: Vec<[f32; 4]> = model.nodes.iter()
            .map(|n| n.rotation)
            .collect();
        let bind_s: Vec<[f32; 4]> = model.nodes.iter()
        ...
        for ch in &anim.channels {
            ...
            let (prop_type, val_offset) = match &s.outputs {
                AnimationOutputs::Translations(v) => {
                    ...
                    for t in v { trans_vals.push([t[0], t[1], t[2], 0.0]); }
                    (0u32, off)
                }
                AnimationOutputs::Rotations(v) => {
                    let off = rot_vals.len() as u32;
                    for r in v { rot_vals.push(*r); }
                    (1u32, off)
                }
                AnimationOutputs::Scales(v) => {
                    let off = scale_vals.len() as u32;
                    for sc in v { scale_vals.push([sc[0], sc[1], sc[2], 1.0]); }
                    (2u32, off)
                }
            ...
            gpu_channels.push(GpuChannelInfo {
                ...
                prop_type, ts_offset, ts_count, val_offset, interp,
                _pad: [0; 2],
            });
        }
        ...
        if timestamps.is_empty()   { timestamps.push(0.0); }
        if trans_vals.is_empty()   { trans_vals.push([0.0; 4]); }
        if rot_vals.is_empty()     { rot_vals.push([0.0, 0.0, 0.0, 1.0]); }
        if scale_vals.is_empty()   { scale_vals.push([1.0, 1.0, 1.0, 1.0]); }
        if gpu_channels.is_empty() { gpu_channels.push(bytemuck::Zeroable::zeroed()); }

        // BFS 順序 + 親インデックス
        let (bfs_order, parent_indices) = compute_bfs_order(model);

        // ジョイントノード + インバースバインド行列（CPU 行優先 → GPU 列優先）
        let joint_nodes: Vec<u32> = skin.joints.iter()
            .take(MAX_JOINTS)
            .map(|j| j.node_index as u32)
            .collect();
        let ibm_data: Vec<[[f32; 4]; 4]> = skin.joints.iter()
            .take(MAX_JOINTS)
        ...
        let static_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0,  resource: bind_t_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1,  resource: bind_r_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2,  resource: bind_s_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 3,  resource: chan_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 4,  resource: ts_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 5,  resource: trans_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 6,  resource: rot_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 7,  resource: scale_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 8,  resource: bfs_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 9,  resource: par_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 10, resource: jnode_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 11, resource: ibm_buf.as_entire_binding() },
            ],
        });
        ...
        for lod in 0..NUM_LODS {
            ...
            let pf_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: at_buf.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 1, resource: pm_buf.as_entire_binding() },
                ],
            });
            ...
            let out_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[wgpu::BindGroupEntry { binding: 0, resource: jm_buf.as_entire_binding() }],
            });
    ...
    pub fn upload_lod_times(
        ...
        queue.write_buffer(&self.lod_params_bufs[lod], 0, bytemuck::bytes_of(&GpuSkinParams {
...
pub fn compute_bfs_order(model: &Model) -> (Vec<u32>, Vec<i32>) {
    ...
    let mut queue: std::collections::VecDeque<usize> =
        model.root_nodes.iter().copied().collect();
    ...
    for i in 0..n {
        if !visited.contains(&(i as u32)) { bfs.push(i as u32); }
    }
