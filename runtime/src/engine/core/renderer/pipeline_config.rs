...
fn default_cull() -> String {
    "Back".into()
}
fn default_front_face() -> String {
    "Ccw".into()
}
fn default_true() -> bool {
    true
}
fn default_depth_compare() -> String {
    "Less".into()
}
...
impl<'d> RenderPipelineBuilder<'d> {
    ...
    pub fn new(
        ...
        let cfg: PipelineConfig = toml::from_str(toml_src).expect("invalid pipeline TOML");
        Self {
            device,
            cfg,
            surface_format,
        ...
        F: Fn(&str) -> &'static str,
    {
        ...
        let num_groups = cfg
            .num_bind_groups
            .unwrap_or_else(|| reflected.keys().max().copied().map_or(0, |m| m + 1));
        ...
        // ── 6. 頂点バッファレイアウト ──────────────────────────
        let vertex_buffers: Vec<wgpu::VertexBufferLayout<'static>> = cfg
            .vertex_slots
            .iter()
        ...
        let fragment: Option<wgpu::FragmentState<'_>> = match (&cfg.fragment_entry, &color_targets)
        {
        ...
        let front_face = if cfg.front_face == "Cw" {
            wgpu::FrontFace::Cw
        } else {
            wgpu::FrontFace::Ccw
        };
...
fn reflect_bgls(device: &wgpu::Device, src: &str) -> BTreeMap<u32, wgpu::BindGroupLayout> {
    let module =
    ...
    for (_, var) in module.global_variables.iter() {
        ...
        let Some((wgpu_ty, visibility)) = to_binding_type(&ty.inner, var.space) else {
            continue;
        };
    ...
    match ty {
        ...
        naga::TypeInner::Image {
            ...
            let binding_ty = match class {
                ...
                ImageClass::Depth { .. } => wgpu::BindingType::Texture {
        ...
        naga::TypeInner::AccelerationStructure { .. } => Some((
...
pub fn vertex_buffer_layout(name: &str) -> wgpu::VertexBufferLayout<'static> {
    use wgpu::{VertexAttribute as VA, VertexBufferLayout, VertexFormat as VF, VertexStepMode};
    ...
    static SMOOTH_NORMAL_ATTRS: &[VA] = &[VA {
        format: VF::Float32x3,
        offset: 0,
        shader_location: 8,
    }];
    ...
    match name {
        "mesh_vertex" => VertexBufferLayout {
            array_stride: 72,
            step_mode: VertexStepMode::Vertex,
            attributes: MESH_ATTRS,
        },
        "skin_vertex" => VertexBufferLayout {
            array_stride: 24,
            step_mode: VertexStepMode::Vertex,
            attributes: SKIN_ATTRS,
        },
        "smooth_normal" => VertexBufferLayout {
            array_stride: 12,
            step_mode: VertexStepMode::Vertex,
            attributes: SMOOTH_NORMAL_ATTRS,
        },
        "color_vertex" => VertexBufferLayout {
            array_stride: 28,
            step_mode: VertexStepMode::Vertex,
            attributes: COLOR_ATTRS,
        },
        "pos3" => VertexBufferLayout {
            array_stride: 12,
            step_mode: VertexStepMode::Vertex,
            attributes: POS3_ATTRS,
        },
        "gizmo_vertex" => VertexBufferLayout {
            array_stride: 48,
            step_mode: VertexStepMode::Vertex,
            attributes: GIZMO_ATTRS,
        },
        "sprite_vertex" => VertexBufferLayout {
            array_stride: 16,
            step_mode: VertexStepMode::Vertex,
            attributes: SPRITE_ATTRS,
        },
        // 80 bytes / インスタンス・step_mode=Instance（Phase R6）
        "sprite_instance" => VertexBufferLayout {
            array_stride: 80,
