...
// ============================================================
//  距離ソート逐次グラブ（Phase RT-Translucency: 半透明同士の屈折）
//
//  距離ソートは奥→手前順に描画するため、手前のガラスを描く直前に scene_hdr を
//  refract ピラミッド mip0 へ再コピーすると、そのガラスが「すでに描かれた奥のガラス」を
//  屈折で拾える（不透明のみのコピー 1 回では奥のガラスが背景に含まれない問題を解消）。
//
//  ・コスト上限: 再グラブは 1 フレーム最大 REFRACT_REGRAB_MAX 回。手前（カメラに近い）優先。
//  ・mip0 のみ更新（すりガラスの深いミップは初回グラブのまま妥協。粗い屈折は細部が見えず
//    1 フレーム内の差は知覚困難。深いミップの再生成＝ダウンサンプル＋ブラーは毎回だと高コスト）。
//  ・距離ソートモード限定（WBOIT は順序が無いため対象外）。
//  ・レンダーパスを跨ぐコピーになるため、呼び出し側（frame_renderer）が regrab_before 境界で
//    メインパスを分割（drop → copy → 再 begin）する。本モジュールはプランと 1 アイテム描画を提供。
// ============================================================

/// 1 フレーム最大の逐次グラブ回数（コスト上限）。手前のガラス最大 4 枚だけ再グラブする。
pub const REFRACT_REGRAB_MAX: usize = 4;

/// CPU 側の「屈折するマテリアルか」判定の ior しきい値マージン。
/// refract_common.wgsl の IOR_EPSILON（1.0e-4）と一致させること
/// （シェーダの material_refracts と CPU のグラブ判定を揃える）。
const REFRACT_IOR_EPSILON: f32 = 1.0e-4;

/// 距離ソート描画プラン（背面→前面の描画順＋逐次グラブ境界）。
pub struct SortedPlan {
    /// 背面→前面（dist_sq 降順）にソート済みの描画アイテム。
    items: Vec<TransparentItem>,
    /// この添字（items のインデックス）を描く**直前**に scene_hdr→refract mip0 の再グラブを行う。
    /// 昇順・重複なし。空なら再グラブ不要（単一パスで描ける＝従来経路）。
    pub regrab_before: Vec<usize>,
}
...
/// 逐次グラブ境界（描画添字）を選ぶ純関数（テスト可能に分離）。
///
/// `refracts`: 背面→前面（描画順）に並んだ「そのアイテムが屈折するか」のフラグ列。
/// 描画順は back→front なので**末尾ほど手前**。末尾側（手前）から屈折アイテムを最大 `max` 件拾い、
/// その描画添字を昇順で返す。各添字を描く直前に再グラブすれば、手前のガラスが奥のガラスを拾える。
fn select_regrab_indices(refracts: &[bool], max: usize) -> Vec<usize> {
    if max == 0 { return Vec::new(); }
    let mut picked: Vec<usize> = Vec::new();
    // 末尾（手前）から屈折アイテムを max 件。
    for i in (0..refracts.len()).rev() {
        if refracts[i] {
            picked.push(i);
            if picked.len() >= max { break; }
        }
    }
...
/// 距離ソート描画をプランする。items を背面→前面にソートし、`regrab_enabled` のとき
/// 屈折するアイテム（material_refracts = ior>1 || transmission>0）のうち手前の最大
/// REFRACT_REGRAB_MAX 件の描画直前を再グラブ境界に選ぶ。
///
/// `regrab_enabled` は「屈折背景ピラミッドがこのフレームに存在するか」（refract_active）。
/// false（forward／非 deferred／半透明なし）のとき regrab_before は常に空＝従来の単一パス経路。
pub fn plan_sorted(
    ...
    let mut items = gather_items(models, camera_pos);
    // 背面→前面（遠い順）。アルファブレンドは描画順に依存するため（draw_sorted と同一）。
    items.sort_by(|a, b| b.dist_sq.partial_cmp(&a.dist_sq).unwrap_or(std::cmp::Ordering::Equal));

    let regrab_before = if regrab_enabled && !items.is_empty() {
        // 各アイテムが屈折するか（material_refracts）を CPU 側で判定する。
        let refracts: Vec<bool> = items.iter().map(|it| {
            let (gpu, _batch) = &models[it.model_idx];
            let prim = &gpu.meshes[it.mesh_idx].primitives[it.prim_idx];
            let ior = gpu.primitive_ior(prim.material_index);
            let tr  = gpu.primitive_transmission(prim.material_index);
            ior > 1.0 + REFRACT_IOR_EPSILON || tr > 0.0
        }).collect();
        select_regrab_indices(&refracts, REFRACT_REGRAB_MAX)
    } else {
        Vec::new()
    };

    SortedPlan { items, regrab_before }
}

/// プラン内の 1 アイテム（添字 i）を開いているパスへ描く。draw_sorted と同一の描画経路。
#[allow(clippy::too_many_arguments)]
pub fn draw_planned<'p>(
    pass:      &mut wgpu::RenderPass<'p>,
    plan:      &SortedPlan,
    i:         usize,
    models:    &'p TransparentModels<'p>,
    ...
    draw_one(
        pass, &plan.items[i], models, camera_bg, lights_bg,
        &tp.sorted_mesh, &tp.sorted_skinned, &tp.mesh_empty_bg3,
    );
}

/// scene_hdr → refract ピラミッド mip0 の逐次再グラブ（mip0 のみ更新）。
/// コピー元／先は同サイズ・同フォーマット（HDR）であること。深いミップは初回のまま。
pub fn regrab_mip0(
    encoder:       &mut wgpu::CommandEncoder,
    scene_hdr_tex: &wgpu::Texture,
    pyramid_tex:   &wgpu::Texture,
    w:             u32,
    h:             u32,
) {
...
mod tests {
    ...
    /// 逐次グラブ境界の選定（select_regrab_indices）の回帰テスト。
    /// 描画順は back→front（末尾＝手前）。手前の屈折アイテムを最大 max 件、昇順で返すこと。
    #[test]
    fn select_regrab_indices_picks_nearest_refractors() {
        use super::select_regrab_indices;
        // 屈折フラグ（描画順＝背面→前面）。true が屈折アイテム。
        // indices:         0     1      2     3      4     5
        let refracts = [true, false, true, false, true, true];
        // max=4: 手前（末尾）から屈折を 4 件 → indices 5,4,2,0 → 昇順 [0,2,4,5]。
        assert_eq!(select_regrab_indices(&refracts, 4), vec![0, 2, 4, 5]);
        // max=2: 手前の 2 件 → 5,4 → 昇順 [4,5]。
        assert_eq!(select_regrab_indices(&refracts, 2), vec![4, 5]);
        // max=0: 再グラブなし。
        assert_eq!(select_regrab_indices(&refracts, 0), Vec::<usize>::new());
        // 屈折アイテムが max より少ない: 全件。
        let few = [false, true, false];
        assert_eq!(select_regrab_indices(&few, 4), vec![1]);
        // 屈折アイテムなし: 空。
        assert_eq!(select_regrab_indices(&[false, false], 4), Vec::<usize>::new());
    }

    /// REFRACT_REGRAB_MAX がコスト上限（4）であること、CPU の ior しきい値が
    /// refract_common.wgsl の IOR_EPSILON（1.0e-4）と一致すること（material_refracts の判定を揃える）。
    #[test]
    fn regrab_constants_are_sane() {
        assert_eq!(super::REFRACT_REGRAB_MAX, 4, "逐次グラブは 1 フレーム最大 4 回（コスト上限）");
        assert_eq!(super::REFRACT_IOR_EPSILON, 1.0e-4, "CPU ior しきい値は WGSL IOR_EPSILON と一致");
        let refract_src = include_str!("shaders/refract_common.wgsl");
        assert!(
            refract_src.contains("const IOR_EPSILON: f32 = 1.0e-4;"),
            "refract_common.wgsl の IOR_EPSILON が CPU 値（REFRACT_IOR_EPSILON）と一致すること"
        );
    }
