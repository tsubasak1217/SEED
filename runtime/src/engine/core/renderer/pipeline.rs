    ...
    F: Fn(&str) -> &'static str + Copy,
{
    ...
    (
        pipelines,
        bgls.expect("build_cull_variants: BGL が生成されていない"),
    )
}
...
impl MeshPipeline {
    fn new(
        ...
        let (pipelines, bgls) = build_cull_variants(
            device,
            include_str!("pipelines/mesh.toml"),
            sf,
            df,
            cache,
            "mesh_pbr",
...
impl SkinnedMeshPipeline {
    fn new(
        ...
        let (pipelines, bgls) = build_cull_variants(
            device,
            include_str!("pipelines/skinned_mesh.toml"),
            sf,
            df,
            cache,
            "skinned_mesh_pbr",
...
impl RtMeshPipelines {
    ...
    pub fn new(
        ...
        let (mesh, bgls_m) = build_cull_variants(
            device,
            include_str!("pipelines/mesh_rt.toml"),
            sf,
            df,
            cache,
            "mesh_pbr_rt",
        ...
        let (skinned, _bgls_s) = build_cull_variants(
            device,
            include_str!("pipelines/skinned_mesh_rt.toml"),
            sf,
            df,
            cache,
            "skinned_mesh_pbr_rt",
...
impl DepthPrepassPipelines {
    pub fn new(
        ...
        let (mesh, _) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/depth_prepass_mesh.toml"),
            sf_unused,
            df,
        )
        .with_cache(cache)
        .build(get_shader_source);
        let (skinned, _) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/depth_prepass_skinned.toml"),
            sf_unused,
            df,
        )
        .with_cache(cache)
...
impl ShadowDepthPipelines {
    pub fn new(
        ...
        let (mesh, _) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/shadow_depth_mesh.toml"),
            sf_unused,
            shadow_df,
        )
        ...
        let (skinned, bgls_s) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/shadow_depth_skinned.toml"),
            sf_unused,
            shadow_df,
        )
...
impl IdPassPipeline {
    fn new(
        ...
        let (mesh_pipelines, mut bgls_m) =
            build(include_str!("pipelines/id_pass_mesh.toml"), "id_pass_mesh");
        ...
        let (skinned_pipelines, mut bgls_s) = build(
            include_str!("pipelines/id_pass_skinned.toml"),
            "id_pass_skinned",
        );
...
impl CanvasIdPipeline {
    fn new(
        ...
        let (pipeline, bgls) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/canvas_id.toml"),
            sf_unused,
            df,
        )
        .with_cache(cache)
        ...
        let (pick_depth_pipeline, _bgls_depth) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/collider_pick.toml"),
            sf_unused,
            df,
        )
        .with_cache(cache)
...
impl CameraPreviewBlitPipeline {
    fn new(
        ...
        let (pipeline, bgls) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/camera_preview_blit.toml"),
            sf,
            df,
        )
        .with_cache(cache)
...
impl SpriteOutlinePipeline {
    fn new(
        ...
        let (pipeline, _bgls) = RenderPipelineBuilder::new(
            device,
            include_str!("pipelines/sprite_outline.toml"),
            sf,
            df,
        )
        .with_cache(cache)
...
impl ParticlePipelines {
    ...
    fn new(
        ...
        let mesh: [wgpu::RenderPipeline; PARTICLE_BLEND_COUNT] =
            std::array::from_fn(|i| build(i, false));
        let pixel: [wgpu::RenderPipeline; PARTICLE_BLEND_COUNT] =
            std::array::from_fn(|i| build(i, true));
...
impl DrawPipelines {
    pub fn new(
        ...
        let shadow_depth =
            ShadowDepthPipelines::new(device, super::shadow::SHADOW_DEPTH_FORMAT, cache);
        ...
        let gbuffer =
            super::gbuffer::GBufferPipelines::new(device, &mesh, &skinned_mesh, df, cache);
        // デファードのフルスクリーン・ライティング復元（Phase D3 Phase A）。
        // sf（シーン HDR）へ出力する（PostPipeline 等と同じ HDR オフスクリーン規約）。
        let deferred =
            super::deferred::DeferredLightingPipelines::new(device, queue, sf, df, cache);
