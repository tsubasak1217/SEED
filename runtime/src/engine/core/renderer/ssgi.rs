...
impl SsgiPipelines {
    ...
    pub fn write_params(&self, queue: &wgpu::Queue, ambient: [f32; 3]) {
        queue.write_buffer(
            &self.params_buffer,
            0,
            bytemuck::bytes_of(&SsgiParams::new(ambient)),
        );
    }
...
impl SsgiTargets {
    ...
    pub fn raw_view(&self) -> &wgpu::TextureView {
        &self
            .raw
            .as_ref()
            .expect("SsgiTargets: ensure 未実行（raw）")
            .1
    }
...
mod tests {
    ...
    fn ssgi_params_is_16_bytes() {
        ...
        assert_eq!(
            SSGI_FORMAT, IMOS_BLUR_FORMAT,
            "SSGI と imos の storage フォーマットは一致必須"
        );
    }
    ...
    fn ssgi_gen_shader_parses() {
        ...
        v.validate(&module)
            .unwrap_or_else(|e| panic!("[ssgi_gen] validate 失敗: {e:?}"));
    }
