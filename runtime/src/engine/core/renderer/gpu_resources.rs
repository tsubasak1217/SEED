...
impl InstancedModelBatch {
    ...
    ///
    /// 【なぜ要るか — 世代の 2 系統化】
    ///   このバッチが持つのは「どのインスタンスをどの行列で描くか」という**構成**だけで、
    ///   頂点データそのものは `GpuModel` 側が持つ。地形のカバー焼き込み
    ///   （`apply_cover_to_chunk`）は頂点バッファを直接書き換えるので、構成は 1 ビットも
    ///   変わらない。したがって統合バッチのダーティゲート（`MergeBatchGate`）は正しく
    ///   スキップし続け、`update()` は呼ばれず `content_generation` も据え置かれる。
    ///
    ///   ところが `content_generation` は影の静的カスケードスキップ（`shadow.rs`）の
    ///   シグネチャでもあり、これが据え置かれると**形状が変わったのに影が焼き直されない**。
    ///   そこで「構成の世代（＝ゲートが見るインスタンス入力）」と「内容の世代
    ///   （＝実際に描かれる形）」を分離し、後者だけをここで進める。
    ///
    ///   `update()` を呼ばずに版番号だけ進めるため、
    ///   「世代が同じなら内容も同じ」という片方向保証は保たれる（逆は保証しない）。
