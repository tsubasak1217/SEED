...
use super::pipeline::{MeshletCullPipeline, SkinComputePipeline};
...
use crate::engine::core::loader::model::{
    AlphaMode, CachedTexFormat, CullFace, FilterMode, Material, Model, ModelNode,
    NormalTextureInfo, OcclusionTextureInfo, Primitive, SamplerData, TextureData, TextureInfo,
    TextureSource, Vertex, WrapMode,
};
use rayon::prelude::*;
use std::path::Path;
...
pub fn upload_texture_data(
    ...
    match &tex.source {
        ...
        TextureSource::FilePath(path) => load_from_file(device, queue, path, linear, &tex.sampler),
...
fn upload_ready(
    ...
    for (level, data) in mips.iter().enumerate() {
        ...
        if (bytes_per_row as usize) * (rows as usize) > data.len() {
            eprintln!(
                "[SEED cache] Ready テクスチャのミップ {level} のデータ長が不足。スキップします。"
            );
            continue;
        }
...
fn conv_filter(
    ...
    let (min_f, mip_f) = match min {
        ...
        FilterMode::NearestMipmapLinear => (wgpu::FilterMode::Nearest, wgpu::FilterMode::Linear),
        FilterMode::LinearMipmapNearest => (wgpu::FilterMode::Linear, wgpu::FilterMode::Nearest),
...
pub(crate) fn build_material_uniform(mat: &Material) -> MaterialUniform {
    ...
    MaterialUniform {
        ...
        diffuse_transmission: if DIFFUSE_TRANSMISSION_DISABLED {
            0.0
        } else {
...
pub(crate) fn bake_inline_material(
    base: &Material,
    ...
    if let Some(v) = base_color {
        eff.base_color_factor = *v;
    }
    if let Some(v) = metallic {
        eff.metallic_factor = *v;
    }
    if let Some(v) = roughness {
        eff.roughness_factor = *v;
    }
    if let Some(v) = emissive {
        eff.emissive_factor = *v;
    }
    if let Some(v) = alpha_mode {
        eff.alpha_mode = material_asset::parse_alpha_mode(v);
    }
    if let Some(v) = alpha_cutoff {
        eff.alpha_cutoff = *v;
    }
    ...
    if let Some(v) = diffuse_transmission {
        eff.diffuse_transmission = *v;
    }
    // MR テクスチャ無視トグルの上書き（None なら埋込値を維持）。true で MR テクスチャを無視。
    if let Some(v) = mr_tex_ignore {
        eff.mr_tex_ignore = *v;
    }
    // カリング面の上書き（None なら埋込値＝glTF double_sided 由来を維持）。
    if let Some(v) = cull_face {
        eff.cull_face = material_asset::parse_cull_face(v);
    }
...
impl GpuMaterial {
    pub fn upload(
        ...
        let bc_sampler = base_color_idx
            .map(tex_sampler)
            .unwrap_or(&defaults.white.sampler);
        let nm_view = normal_idx
            .map(tex_view)
            .unwrap_or(&defaults.flat_normal.view);
        let nm_sampler = normal_idx
            .map(tex_sampler)
            .unwrap_or(&defaults.flat_normal.sampler);
        ...
        let ao_sampler = occlusion_idx
            .map(tex_sampler)
            .unwrap_or(&defaults.white.sampler);
        let em_view = emissive_idx.map(tex_view).unwrap_or(&defaults.black.view);
        let em_sampler = emissive_idx
            .map(tex_sampler)
            .unwrap_or(&defaults.black.sampler);
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::Sampler(bc_sampler),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                ...
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: wgpu::BindingResource::Sampler(nm_sampler),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
                ...
                wgpu::BindGroupEntry {
                    binding: 10,
                    resource: wgpu::BindingResource::Sampler(em_sampler),
                },
            ],
        });
...
impl GpuPrimitive {
    ...
    fn upload(device: &wgpu::Device, prim: &Primitive) -> Self {
        ...
        let lod_index_buffers: Vec<wgpu::Buffer> = prim
            .lod_indices
            .iter()
            .map(|lod_idx| {
                device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            .collect();
        let lod_index_counts: Vec<u32> = prim
            .lod_indices
            .iter()
...
impl GpuModel {
    ...
    pub fn primitive_terrain_palette(
        &self,
    ...
    pub fn register_bindless(
        ...
        for (mesh_i, mesh) in model.meshes.iter().enumerate() {
            let Some(gpu_mesh) = self.meshes.get_mut(mesh_i) else {
                break;
            };
            for (prim_i, prim) in mesh.primitives.iter().enumerate() {
                let Some(gp) = gpu_mesh.primitives.get_mut(prim_i) else {
                    break;
                };
                ...
                if !prim.skin_vertices.is_empty() {
                    continue;
                }
                ...
                let normals: Vec<u32> = prim
                    .vertices
                    .iter()
                    .map(|v| crate::engine::core::renderer::bindless::oct_encode_normal(v.normal))
                    .collect();
                let (uv_off, idx_off, nrm_off, elig) = bindless.register_primitive_geometry(
                    queue,
                    &uvs,
                    &prim.indices,
                    &normals,
    ...
    pub fn upload(
        ...
        // ── テクスチャ ─────────────────────────────────────────
        let gpu_textures: Vec<GpuTexture> = model
            .textures
            .iter()
            .enumerate()
            .map(|(i, td)| {
    ...
    pub fn apply_overrides(
        ...
        for ovr in overrides {
            ...
            if ovr.slot >= self.materials.len() {
                continue;
            }
            ...
            match &ovr.kind {
                ...
                MaterialOverrideKind::Inline { .. } => {
                    let Some(base) = model.materials.get(ovr.slot) else {
                        continue;
                    };
                    // 焼き込みは共有関数に委譲（in-place 更新経路と 1 ビットもズレない）。
                    let Some(eff) = bake_inline_material(base, &ovr.kind) else {
                        continue;
                    };
                ...
                MaterialOverrideKind::MatAsset { path } => {
                    ...
                    let Some(asset) = material_asset::load(path) else {
                        eprintln!(
                            "[SEED gpu_resources] MaterialOverride: .mat ロード失敗（path={path}）。オーバーライドをスキップします。"
                        );
                        continue;
                    };
                    ...
                    if !tex_path.albedo.is_empty() {
                        let idx = Self::upload_override_texture(
                            device,
                            queue,
                            &tex_path.albedo,
                            false,
                        ...
                        eff.base_color_texture = Some(TextureInfo {
                            texture_index: idx,
                            tex_coord_set: OVERRIDE_TEX_COORD_SET,
                        });
                    }
                    ...
                    if !tex_path.metallic_roughness.is_empty() {
                        let idx = Self::upload_override_texture(
                            device,
                            queue,
                            &tex_path.metallic_roughness,
                            true,
                            &mut self.textures,
                        );
                        eff.metallic_roughness_texture = Some(TextureInfo {
                            texture_index: idx,
                            tex_coord_set: OVERRIDE_TEX_COORD_SET,
                        });
                    }
                    if !tex_path.occlusion.is_empty() {
                        ...
                        eff.occlusion_texture = Some(OcclusionTextureInfo {
                            texture_index: idx,
                            tex_coord_set: OVERRIDE_TEX_COORD_SET,
                            strength: OVERRIDE_OCCLUSION_STRENGTH,
                        });
                    }
                    if !tex_path.emissive.is_empty() {
                        ...
                        eff.emissive_texture = Some(TextureInfo {
                            texture_index: idx,
                            tex_coord_set: OVERRIDE_TEX_COORD_SET,
                        });
                    }
    ...
    pub fn update_material_inline(
        ...
        if slot >= self.materials.len() {
            return InlineUpdateResult::NotApplicable;
        }
        let Some(base) = model.materials.get(slot) else {
            return InlineUpdateResult::NotApplicable;
        };
        // フル経路と同一の共有関数で実効マテリアルを作る（値解釈が 1 ビットもズレない）。
        let Some(eff) = bake_inline_material(base, kind) else {
            return InlineUpdateResult::NotApplicable;
        };
...
pub fn chunk_culled_by_camera(
    ...
    aabb_outside_frustum(planes, min, max) || aabb_distance_sq(min, max, camera_pos) > cull_dist_sq
}
...
impl InstancedModelBatch {
    pub fn new(
        ...
        // ── メッシュノードの順序テーブル ────────────────────────
        let mesh_node_indices: Vec<usize> = model
            .nodes
            .iter()
            .enumerate()
        ...
        node_prim_list.sort_by_key(|d| {
            (
                d.is_skinned as u8,
                cull_key(d.material_idx),
                d.material_idx.unwrap_or(usize::MAX),
            )
        });
    ...
    pub fn prepare_meshlet_cull(
        ...
        for (i, slot_opt) in self.meshlet_cull.iter_mut().enumerate() {
            ...
            if gpu_model.primitive_alpha_mode(draw.material_idx) == AlphaMode::Blend {
                continue;
            }

            // GPU プリミティブのメッシュレット記述子バッファ。
            let Some(gpu_prim) = gpu_model
                .meshes
                .get(draw.mesh_idx)
                .and_then(|m| m.primitives.get(draw.prim_idx))
            else {
                continue;
            };
            let Some(desc_buf) = gpu_prim.meshlet_desc_buffer.as_ref() else {
                continue;
            };
            // このノードの LOD0 可視インスタンス行列バッファ。
            let Some((inst_buf, _)) = self.lod_node_data[0]
                .get(slot.node_idx)
                .and_then(|o| o.as_ref())
            else {
                continue;
            ...
            let cone_cull_enabled =
                if gpu_model.primitive_cull_face(draw.material_idx) == CullFace::Back {
                    CONE_CULL_ON
                } else {
                    CONE_CULL_OFF
                };
            ...
            let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry {
                        binding: 2,
                        resource: slot.cmd_buf.as_entire_binding(),
                    },
                    wgpu::BindGroupEntry {
                        binding: 3,
                        resource: slot.count_buf.as_entire_binding(),
                    },
                    wgpu::BindGroupEntry {
                        binding: 4,
    ...
    pub fn record_meshlet_cull<'p>(
        ...
        for slot_opt in &self.meshlet_cull {
            ...
            let Some(bg) = slot.bind_group.as_ref() else {
                continue;
            };
            if slot.workgroups == 0 {
                continue;
            }
            if !pipeline_set {
    ...
    pub fn meshlet_draw(&self, idx: usize) -> Option<(&wgpu::Buffer, &wgpu::Buffer, u32)> {
        ...
        if slot.bind_group.is_none() || slot.workgroups == 0 {
            return None;
        }
    ...
    pub fn update(
        ...
        let mut compact: Vec<Vec<Vec<ModelUniform>>> = (0..NUM_LODS)
            .map(|_| {
                (0..n_mesh_nodes)
                    .map(|_| Vec::with_capacity(n_instances / NUM_LODS + 1))
                    .collect()
            })
            .collect();
        ...
        for (inst_idx, aabb) in self.world_aabbs.iter().enumerate() {
            ...
            let lod = if dist_sq < LOD_DIST_SQ[0] {
                0
            } else if dist_sq < LOD_DIST_SQ[1] {
                1
            } else if dist_sq < LOD_DIST_SQ[2] {
                2
            } else {
                3
            };
        ...
        for lod in 0..NUM_LODS {
            ...
            if visible > 0 {
                ...
                let ids: Vec<u32> = self.lod_compact_insts[lod]
                    .iter()
                    .map(|&i| i as u32)
                    .collect();
    ...
    pub fn find_compact_index(&self, inst_idx: u32) -> Option<(usize, u32)> {
        for lod in 0..NUM_LODS {
            if let Some(pos) = self.lod_compact_insts[lod]
                .iter()
                .position(|&i| i == inst_idx as usize)
            {
    ...
    pub fn dispatch_skin(&self, pass: &mut wgpu::ComputePass<'_>, pipeline: &SkinComputePipeline) {
        if let Some(skin) = &self.skin {
            for lod in 0..NUM_LODS {
    ...
    pub fn rt_enumerate<F: FnMut(usize, usize, Option<usize>, [f32; 12])>(&self, mut f: F) {
        ...
        if self.world_mats_cache.is_empty() || self.n_mesh_nodes == 0 {
            return;
        }
        ...
        for inst in 0..n_inst {
            for draw in &self.node_prim_list {
                ...
                let Some(pos) = self.node_pos_map[draw.node_idx] else {
                    continue;
                };
                let cache_idx = inst * self.n_mesh_nodes + pos;
                let Some(mu) = self.world_mats_cache.get(cache_idx) else {
                    continue;
                };
                f(
                    draw.mesh_idx,
...
impl GpuGizmoBatch {
    pub fn new(device: &wgpu::Device, lines: &[GizmoVertex], tris: &[ColorVertex]) -> Self {
        ...
        let tri_buffer = if tris.is_empty() {
            None
        } else {
            Some(
                device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
...
mod meshlet_gpu_tests {
    use super::{
        DRAW_INDEXED_INDIRECT_SIZE, GpuMeshlet, meshlet_cmd_buf_size, meshlet_cull_slot_fits,
    };
    ...
    fn meshlet_cmd_buf_size_is_capacity_times_stride() {
        // 通常ケース。
        assert_eq!(
            meshlet_cmd_buf_size(100, 50),
            100 * 50 * DRAW_INDEXED_INDIRECT_SIZE
        );
    ...
    fn meshlet_cull_slot_fits_threshold() {
        ...
        assert!(
            !meshlet_cull_slot_fits(1, n + 1, LIMIT),
            "上限+1コマンドは不可"
        );
...
mod cull_tests {
    use super::{
        DENSITY_DECAY_FAR_DIVISOR, DENSITY_DECAY_MID_DIVISOR, aabb_distance_sq,
        aabb_outside_frustum, chunk_culled_by_camera, density_kept_count, extract_frustum_planes,
    };
    ...
    fn density_kept_count_bands_and_boundaries() {
        ...
        assert_eq!(
            density_kept_count(total, near_sq, near_sq, mid_sq),
            100,
            "near 境界は全数側"
        );
        // 中距離帯 → 1/2。境界ちょうども 1/2 側。
        assert_eq!(
            density_kept_count(total, near_sq + 1.0, near_sq, mid_sq),
            50
        );
        assert_eq!(
            density_kept_count(total, mid_sq, near_sq, mid_sq),
            50,
            "mid 境界は 1/2 側"
        );
        // 遠距離帯 → 1/4。
...
mod inline_bake_tests {
    ...
    use crate::engine::core::loader::model::{AlphaMode, CullFace, Material};
    ...
    fn bake_overrides_fields_correctly() {
        ...
        assert_eq!(
            eff.diffuse_transmission, 0.6,
            "diffuse_transmission=Some(0.6) が反映されること"
        );
        assert!(
            eff.mr_tex_ignore,
            "mr_tex_ignore=Some(true) が反映されること"
        );
    ...
    fn mat_asset_kind_returns_none() {
        let base = Material::default();
        let kind = MaterialOverrideKind::MatAsset {
            path: "assets://x.mat".to_string(),
        };
    ...
    fn mr_tex_ignore_true_sets_flag_and_preserves_factors() {
        ...
        assert_eq!(
            uni.metallic_factor, 0.2,
            "metallic factor は素通り（実効値＝factor）"
        );
        assert_eq!(
            uni.roughness_factor, 0.9,
            "roughness factor は素通り（実効値＝factor）"
        );
    }
    ...
    fn mr_tex_ignore_false_is_bit_compatible_with_old_layout() {
        ...
        assert_eq!(
            &bytes[68..72],
            &[0u8; 4],
            "offset 68 の 4 バイトは旧 _pad0(0.0) とビット一致"
        );
    }
    ...
    fn eff_avg_albedo_tracks_current_base_color_factor() {
        ...
        assert_eq!(
            [out[0], out[1], out[2]],
            [0.2, 0.1, 0.05],
            ".rgb = tex 平均 × factor.rgb（白 factor なので tex 平均）"
        );
        assert_eq!(
            out[3], 0.3,
            ".a は現在の base_color_factor.a（baked のロード時アルファではない）"
        );
        ...
        assert_eq!(
            out2,
            [0.1, 0.2, 0.3, 0.5],
            "テクスチャ無しは rgb/a とも base_color_factor"
        );
        ...
        assert_eq!(
            out3,
            [0.0, 1.0, 1.0, 1.0],
            "シアンのガラスの実効アルベド＝シアン（影 tint がシアンになる根拠）"
        );
        ...
        assert_eq!(
            t,
            [0.0, 1.0, 1.0],
            "α=1,tr=1 の透過率 T はシアン（R を遮り G/B を通す色影。黒ではない）"
        );
    }
}
