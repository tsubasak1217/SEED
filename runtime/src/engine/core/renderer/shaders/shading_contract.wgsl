...
struct ShadingSurface {
    ...
    /// ゲーム内累計時間（秒）。アニメーションする光応答（脈動する発光・流れる縞・
    /// 明滅など）を書くための唯一の時間入力である。
    ///
    /// 【どの時計か】スクリプトの `SEED.Time.ElapsedTime`（Rust 側 `Clock::anim_time`）と
    /// **同一の値**であり、草の揺れ（grass_gbuffer.wgsl）が使う時刻とも同一。したがって
    /// スクリプトとアセットが同じ位相を計算でき、両者がズレることはない。
    ///
    /// 【進む条件】Play かつ非ポーズのときだけ進む。Edit モード・ポーズ中は値が
    /// 止まったままになる（＝アニメーションが静止する）。これはエンジン全体の
    /// アニメーション時刻の規約であり、シェーディングアセットだけが独自の時計を
    /// 持つことはしない。Play 開始・シーン再ロードで 0 へ戻る。
    ///
    /// 【精度】f32 の秒。長時間の連続 Play では絶対値が大きくなり `sin()` 等の
    /// 位相分解能が落ちる。周期の速い模様は `time * 速度` の速度側を小さく保つか、
    /// 必要なら剰余（`time % 周期`）を取ってから使うこと。
    ///
    /// 【契約バージョン】v1 への**末尾への純粋追加**である（既存アセットは無改造で動く）。
    time:          f32,
}
