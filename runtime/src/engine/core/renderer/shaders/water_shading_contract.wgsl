...
struct WaterShadeInput {
    ...
    /// 反射パスが走らないフレーム（前方描画・水面反射 off）でスカイボックスがあるときは、
    /// 波法線で反射した方向の**空の色**（天球の直接サンプル。物体は映らない）が入る。
    reflection_color: vec3<f32>,
    /// 反射の強度 0..1。反射強度 0 の水域では 0。反射パスが走らないフレームは、スカイボックスがあって
    /// 水上から見ている画素だけ水域の反射強度（空を映す）で、それ以外（水中・スカイボックス無し）は 0。
