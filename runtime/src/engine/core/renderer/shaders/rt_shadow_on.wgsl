...
/// 8 本＝遮蔽率が 9 段階に量子化される。
///
/// 【4→8 に増やした理由（Play 中のチカチカ対策）】
/// 遮蔽率は二値可視性の平均なので、量子化段数が粗いほど「1 本のヒット/ミスが切り替わるだけで
/// 影の濃さが 1/N ジャンプする」。Play 中はアニメーションでスキン BLAS が毎フレーム再構築され、
/// 二値判定の位相が総入れ替えになるため、この量子化段差がフレーム間でバタつき（チカチカ）に見える。
/// 段数を倍にすると 1 段差の振幅が半分になり、後段のバイラテラル＋テンポラル EMA で残差が均される。
/// コスト: マスク経路は半解像度（SHADOW_MASK_RESOLUTION_DIVISOR=2）で評価されるため、
/// フル解像度換算の実効コストは本数 ×1/4。8 本でも従来のフル解像度 2 本相当に収まる。
const RT_SHADOW_SAMPLES_MIN: u32 = 8u;
