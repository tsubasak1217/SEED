...
//                                三角形内で一定＝面境界で不連続。
//                                → 「レイを自分の三角形の平面から確実に逃がす」目的**だけ**:
//                                   ・原点の最小クリアランス（平面から浮かせる）
//                                   ・レイ方向の地平線リフト（平面より下へ潜らせない）
//
...
/// レイ方向を三角形の平面より上へ持ち上げる際の、最小の仰角 cos（**Ng 基準**）。
/// 円錐が広がるとサンプル方向の一部は三角形の平面より下（dot(Ng,dir) <= 0）を向く。
/// そのまま飛ばすと必ず自分の三角形／隣接三角形に当たり、偽の遮蔽（＝黒帯）を返す。
///
/// 旧実装はそれらを「母数から除外」していたが、除外本数がピクセルごとの IGN 回転で
/// ばらつき、平均の分母が暴れてディザノイズの原因になっていた。さらに「有効 0 本 → 0.0」の
/// 分岐が黒潰れを増幅していた。現実装は**除外せず、方向を地平線のわずか上へ射影**する。
///   dir' = normalize(dir + Ng * (MIN_COS - dot(Ng,dir)))   （dot(Ng,dir) < MIN_COS のときのみ）
/// これにより
///   - 分母は常に samples 本で一定（ピクセル間で変動しない＝ノイズ源が消える）
///   - どのレイも自分の平面から離れる向きに進む（偽の自己遮蔽が出ない）
///   - 方向の歪みは高々「地平線付近の数度」で、その領域はターミネータランプと BRDF の
///     ndl でどのみち寄与がほぼ 0（見た目に出ない）
///
/// 値 0.05 の根拠: 仰角 ≈ 2.9°。原点は平面から最低 RT_SHADOW_GEO_CLEARANCE(0.005) 浮いており、
/// この仰角ならレイは 1 ワールド単位進むごとに 0.05 上昇するため、平面へ戻ることはない。
/// 小さすぎる（0 に近い）と面すれすれを長く這って隣接三角形に当たり黒ずむ。大きすぎると
/// サンプル方向の歪みが目に見える範囲まで持ち上がる。
const RT_SHADOW_GEO_HORIZON_MIN_COS: f32 = 0.05;
...
/// レイ方向を**三角形の平面（Ng）より上**へ持ち上げる（地平線リフト）。
/// dot(ng, dir) が RT_SHADOW_GEO_HORIZON_MIN_COS 未満のときだけ、ng 方向成分を足して
/// ちょうどその仰角まで起こす。サンプルを捨てないので平均の分母が一定に保たれる
/// （＝ピクセル間で分母が変動することによるディザノイズが出ない）。
fn rt_shadow_lift_above_horizon(dir: vec3<f32>, ng: vec3<f32>) -> vec3<f32> {
    let c = dot(ng, dir);
    if c >= RT_SHADOW_GEO_HORIZON_MIN_COS {
        return dir;
    }
    // dir + ng*(min_cos - c) は正規化前で dot(ng, ・) = min_cos になる。
    // 正規化後も dot は正（長さは |dir'|^2 = 1 + min_cos^2 - c^2 > 0 で 0 にならない）。
    return normalize(dir + ng * (RT_SHADOW_GEO_HORIZON_MIN_COS - c));
}

/// 表面からライト方向 l へ遮蔽レイを飛ばし、遮蔽率を返す。
/// - 戻り値: 1.0=完全照射 / 0.0=完全遮蔽 / 中間=ペナンブラ。
/// - `origin`     : 表面のワールド座標。
/// - `ng`         : **幾何法線**（フラットな面法線）。自己交差回避専用
///                  （原点の最小クリアランス／レイ方向の地平線リフト）。減衰判定には使わない。
/// - `nv`         : **補間頂点法線**（法線マップ前・滑らか）。減衰判定に使う
///                  （ターミネータランプ／スロープスケールバイアス）。
/// - `l`          : 面から光源への方向（正規化済み）。
...
/// 3. 円錐内へ Vogel ディスク分布でレイを分散し、各方向を地平線リフトしてから平均する。
///    サンプルは 1 本も捨てない（分母は常に samples）。
/// 4. 平均 × ターミネータ係数 を返す。
fn rt_shadow_factor(
    origin:      vec3<f32>,
    ng:          vec3<f32>,
    ...
    if cone_radius <= 0.0 {
        // 中心方向も平面より下を向きうる（nv·l > 0 でも ng·l < 0 になりうる＝ターミネータ帯）。
        // リフトして自己交差を避ける。
        let dir = rt_shadow_lift_above_horizon(l, ng);
        return terminator * rt_trace_occlusion(o, dir, tmax);
    }
    ...
    for (var i: u32 = 0u; i < RT_SHADOW_SAMPLES_MAX; i = i + 1u) {
        ...
        let raw   = normalize(l + t * disk.x + b * disk.y);
        // 平面より下へ潜ったサンプルは**捨てずに**地平線上へ起こす（分母を一定に保つ）。
        let dir   = rt_shadow_lift_above_horizon(raw, ng);

        sum += rt_trace_occlusion(o, dir, tmax);
    }

    // 分母は常に samples（ピクセル間で変動しない＝ディザノイズが出ない）。
