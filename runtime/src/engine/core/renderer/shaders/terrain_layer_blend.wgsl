// ============================================================
// terrain_layer_blend.wgsl — 地形レイヤブレンドの共有モジュール（Terrain T2b / 前方描画の地形）
//
// ## 役割（単一責任）
// 頂点カラーに載って来たスプラット重み（4 スロット）と group3 のレイヤ定義から、
// triplanar 投影でレイヤをブレンドし、**ライティング前の「面の情報」**
// （アルベド・法線・メタリック・ラフネス・オクルージョン）を作る。それだけを持つ。
// 出力先（G-Buffer の MRT か、前方描画のライティングか）は知らない。
//
// ## 誰が連結するか（どちらもこのファイルの直後にエントリポイントを連結する）
//   - デファード（G-Buffer 書き込み）: terrain_gbuffer_write.wgsl の fs_terrain_gbuffer
//       連結順の正典は terrain_gbuffer.rs::terrain_gbuffer_shader_sources
//   - 前方描画（mobile プリセット等の deferred=false）: terrain_forward.wgsl の fs_terrain_forward
//       連結順の正典は terrain_forward.rs::terrain_forward_shader_sources
// 両者が同じ `terrain_blend_surface` を通るので、デファードと前方描画で地表の色・法線・粗さが
// 食い違わない（前方描画だけが頂点カラー＝レイヤ重みを色として塗ってしまう不具合の根治。
// 以前の前方描画は地形を汎用メッシュのシェーダで描いており、重みがそのまま赤・緑に見えていた）。
//
// ## 依存
//   - shader_common.wgsl : VertexOutput（world_pos / world_normal / color / uv0 / uv1 / sharpness）
// group0〜2（camera / model / material）は shader_common.wgsl の宣言をそのまま使う。
// group3（地形レイヤ定義）は本ファイルが宣言する（terrain_gbuffer.rs の layer_bgl と一致必須）。
//
// ## 法線シャープネスペイント（T4）
...
// ============================================================
//  レイヤブレンド本体（G-Buffer 書き込みと前方描画が共有する唯一の実装）
// ============================================================

/// 地形 1 フラグメントのレイヤブレンド結果（ライティング前の「面の情報」）。
///
/// G-Buffer 書き込み（terrain_gbuffer_write.wgsl）はこれを MRT へそのまま焼き、
/// 前方描画（terrain_forward.wgsl）はこれから Surface を組んで evaluate_lighting へ渡す。
struct TerrainBlendResult {
    /// 合成済みのアルベド（リニア）。地表カバー・踏み固めの暗さを反映済み。
    albedo:    vec3<f32>,
    /// 合成済みのシェーディング法線（ワールド空間・正規化済み・見えている側を向く）。
    /// 地形は閉じたハイトフィールドなので、ライティングの幾何法線（geo_gate）にもこれを使う
    /// （TERRAIN_NORMAL_AUTHORED_FLAG の説明を参照）。
    normal:    vec3<f32>,
    /// 合成済みのメタリック（0..1）。
    metallic:  f32,
    /// 合成済みのラフネス（[TERRAIN_ROUGHNESS_MIN, 1] へ丸め済み）。
    roughness: f32,
    /// オクルージョン（1 = 遮蔽なし）。轍のキャビティ（uv1.y）だけが下げる。
    /// 地形は AO テクスチャを持たない（SSAO はデファードの後段のパスが乗せる）。
    occlusion: f32,
}

/// 地形レイヤをブレンドして「面の情報」を作る。
///
/// **フラグメントステージ専用**（dpdx / dpdy を使う）。画面微分は関数の先頭
/// （＝呼び出し側の一様制御フロー）で取るので、エントリポイントの先頭で呼ぶこと。
///
/// - `in`           : 補間済みの頂点出力（頂点カラー＝スプラット重み・uv0/uv1＝カバーと轍）
/// - `front_facing` : `@builtin(front_facing)`（両面描画の裏面で法線を反転する）
fn terrain_blend_surface(in: VertexOutput, front_facing: bool) -> TerrainBlendResult {
    ...
    // ── 結果を詰める ──
    //   G-Buffer 版の出力（albedo_occ.a = 1 - キャビティ・mr.g = 丸めたラフネス）と同じ値。
    var r: TerrainBlendResult;
    r.albedo    = albedo;
    r.normal    = out_n;
    r.metallic  = metallic;
    r.roughness = clamp(roughness, TERRAIN_ROUGHNESS_MIN, 1.0);
    r.occlusion = 1.0 - trample_cavity;
    return r;
}
