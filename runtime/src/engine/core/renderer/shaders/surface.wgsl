...
struct Surface {
    ...
    /// 水中に落ちる影の**屈折オフセット**（ワールド XZ・単位 m。Phase W5.3）。
    ///
    /// このピクセルへ届く平行光（太陽）は、真上ではなく「水面勾配 × 深さ × (1−1/n)」だけ
    /// ずれた位置で水面を通っている。よって遮蔽判定もその位置で行うのが物理的に正しく、
    /// 結果として**水中の影が水面のうねりに同期して揺らぐ**。
    /// `lighting_eval.wgsl` が**平行光の影評価だけ**でサンプル位置へ
    /// `world_pos + vec3(x, 0, y)` として足す（点光源・スポット・rect には掛けない）。
    ///
    /// **中立値は 0（＝ずらさない＝従来と同一の影）で、これはゼロ初期化と一致する。**
    /// 透過率（caustics.rgb）のような乗算項と違い、フォワード系がこのフィールドを
    /// 設定し忘れても絵は壊れない（それでも意図を明示するため surface_gather は 0 を書く）。
    /// 供給元はコースティクス生成パスの 2 枚目の MRT（`caustics.wgsl` location1）で、
    /// deferred が group1 binding13 から `textureLoad` で 1:1 に読む。
    /// RT ソフト影マスク経路だけは**生成パス側**（`shadow_mask.wgsl`）が同じテクスチャを
    /// 読んでレイ原点をずらすため、このフィールドを経由しない。
    shadow_refract_offset: vec2<f32>,
