...
///
/// ## しきい値を 0.0 にする根拠
/// 値は「幾何地平線そのもの」＝ 0.0。ゲートはこの値を**遷移帯の下端**に使うため、
/// `dot(Ng, L) <= 0` の面は smoothstep が厳密に 0 を返し、必ず直接光 0 になる。
...
///
/// ## 副作用（許容）
/// ゲートは三角形ごとに一定の Ng で切り替わるため、シャドウターミネータが面境界で
/// ハード（ファセット状）になる。「見た目が崩れてもよいので絶対漏らさない」という
/// 要件どおりであり、意図的に許容する。なお切り替わり自体は RT_GEO_SHADOW_SOFT_COS の
/// 幅ぶんだけ滑らかにしている（Ng の数値誤差が黒斑点になるのを防ぐため。面境界の
/// ファセット感そのものは Ng が三角形ごとに一定である以上、遷移帯では解消しない）。
const RT_GEO_SHADOW_MIN_COS: f32 = 0.0;

/// 幾何ゲートの**遷移帯の幅**（cos 単位）。ゲートは
/// `smoothstep(RT_GEO_SHADOW_MIN_COS, RT_GEO_SHADOW_MIN_COS + この値, dot(Ng,L))` で効く。
///
/// ## なぜ 0/1 の階段をやめるのか
/// Ng は深度バッファ（deferred）または補間ワールド座標（forward）の画面微分から復元する
/// 数値量であり、必ず有限の誤差を持つ。桁落ち対策（カメラ相対座標での微分）を入れた後でも
/// 残差はおよそ 0.1° 規模＝ cos で 1e-3 規模ある。ここに `dot(Ng,L) > 0` の**二値**ゲートを
/// 置くと、`dot(Ng,L)` がほぼ 0 の領域（＝大きな平面ポリゴンのターミネータ付近では画面上の
/// 広い帯）で誤差がそのまま 0/1 に増幅され、**黒い斑点ノイズ**として可視化される。
/// カメラのサブピクセル移動で誤差の符号が入れ替わるため、静止していても明滅して見える。
/// 狭い遷移帯を与えると、残差はゲート値の連続なわずかな揺らぎに落ち、斑点にならない。
///
/// ## 値の根拠（0.02）
/// - 下端は厳密に RT_GEO_SHADOW_MIN_COS（=0.0）のまま。`dot(Ng,L) <= 0` なら
///   smoothstep は厳密に 0 を返すため、**裏面への光漏れ防止というゲート本来の目的は不変**。
/// - 上端 0.02 は角度にして地平線から約 1.1°。Ng の残差誤差（~0.1°）を十分覆いつつ、
///   遷移帯自体は「幾何的な放射照度が cos で 0〜0.02 倍しかない」＝ほぼ真っ暗な領域なので、
///   見た目の変化はターミネータが 1° ぶん柔らかくなる程度で実質観測されない。
/// - 遷移帯を広げるほど斑点耐性は上がるがターミネータがぼやける。ここを触るときは
///   「Ng の実誤差より広く、放射照度が無視できる範囲より狭く」を守ること。
const RT_GEO_SHADOW_SOFT_COS: f32 = 0.02;
...
fn evaluate_lighting(s: Surface) -> vec3<f32> {
    ...
    for (var it: u32 = 0u; it < iter_count; it = it + 1u) {
        ...
        //
        // ゲートは 0/1 の階段ではなく **幅 RT_GEO_SHADOW_SOFT_COS の狭い遷移帯** を持つ。
        // Ng は画面微分から復元する数値量で必ず残差誤差を持ち、二値ゲートだとその誤差が
        // ターミネータ付近（dot(Ng,L)≒0 の帯）で 0/1 の黒斑点ノイズへ増幅されるため
        // （RT_GEO_SHADOW_SOFT_COS の解説参照）。
        // 下端は RT_GEO_SHADOW_MIN_COS のままなので `dot(Ng, L) <= 0` は smoothstep が
        // 厳密に 0 を返し、「幾何的に裏を向いた面は直接光ゼロ」という光漏れ防止は不変。
        // アンビエント（下の ambient）とエミッシブ（s.emissive）にはこのゲートを掛けない
        // （方向性がなく漏れではないため）。
        let geo_gate = smoothstep(
            RT_GEO_SHADOW_MIN_COS,
            RT_GEO_SHADOW_MIN_COS + RT_GEO_SHADOW_SOFT_COS,
            dot(Ng, L),
        );
