...
pub enum PhysicsCommand {
    ...
    /// キャラクターコントローラーの「希望位置」を差分解決し、補正後位置を reply で返す
    /// （**同期問い合わせ**・スクリプトフェーズ実行後にフレーム 1 回だけ送る）。
    ///
    ...
    /// 前回位置を更新する。接地判定も保持し、物理ワールドのコライダー姿勢も同期する。
    ///
    /// Raycast / CheckKinematicOverlap と同じ reply チャンネルパターンで、補正後位置と接地を
    /// **その場で**返す。メインスレッドは受け取った補正後位置を描画前に ECS へ書き戻すため、
    /// スクリプトの貫通直書きが同フレーム・描画前に押し戻し済み位置で上書きされる。
    ///
    /// 【旧設計（撤去済み）】以前は reply を持たない `StepCharacter` を投げっぱなしで送り、
    /// 補正後位置を次フレームの `PhysicsResult.character_updates` で受けていたが、
    /// 「補正結果が描画前に確定しない・スクリプトの直書きで毎フレーム上書きされる」ため
    /// 実機で全く押し戻されず貫通した。フレーム 1 回の同期解決に一本化してこれを根治した。
    ResolveCharacter {
        ...
        rotation:          [f32; 4],
        /// 結果送信チャンネル。`Some((補正後ワールド位置 [x,y,z], grounded))` を返す。
        /// 対象が未登録・キャラクター指定でない・コライダーが引けない場合は `None`
        /// （メインスレッドは今フレームの補正を諦め、ECS はスクリプト値のまま）。
        reply:             crossbeam_channel::Sender<Option<([f32; 3], bool)>>,
    },
...
pub struct PhysicsResult {
    ...
    // 【撤去】旧 `character_updates`（キャラ補正後位置の非同期ストリーム）は廃止した。
    // キャラクターの衝突解決は `ResolveCharacter` の同期 reply でフレーム 1 回・描画前に
    // 確定させるため、ステップ結果に相乗りさせる非同期チャネルは不要になった。
}
