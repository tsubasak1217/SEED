...
impl Camera for BaseCamera {
    fn view_matrix(&self)          -> Mat4x4<f32> { self.view_matrix() }
    fn projection_matrix(&self)    -> Mat4x4<f32> { self.projection_matrix() }
...
impl Camera for DebugCamera {
    fn view_matrix(&self)          -> Mat4x4<f32> { self.view_matrix() }
    fn projection_matrix(&self)    -> Mat4x4<f32> { self.projection_matrix() }
    fn transform(&self)            -> &Transform  { &self.base.transform }
    fn transform_mut(&mut self)    -> &mut Transform { &mut self.base.transform }
}
