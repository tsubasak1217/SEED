...
/// 4 つのアイテム（帯 2 本・文字 2 つ）は<b>アンカーを全て画面中心 (0.5, 0.5) に揃え</b>、
/// 中心からのオフセットで配置している（2026-09-07 改定）。ルートキャンバスの実効サイズは
/// ウィンドウの実解像度そのものになり <c>CanvasComponent</c> の設計解像度 (1920x1080) は
/// 効かないため、角アンカー（左上・右下）では解像度が変わると構図が崩れるためである。
