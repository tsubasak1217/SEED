...
impl Plugin for SamplePlugin {
    ...
    fn name(&self) -> &str { "SamplePlugin" }

    /// バージョン文字列。
    fn version(&self) -> &str { "0.1.0" }

    /// エディタ UI に表示される説明文。
    fn description(&self) -> &str { "プラグインシステムのテスト用サンプルプラグイン。" }
    ...
    fn field_defs(&self) -> Vec<PluginFieldDef> {
        vec![
            ...
            PluginFieldDef {
                ...
                kind:          PluginFieldKind::Float { min: 0.0, max: 50.0, step: 0.5 },
    ...
    fn on_field_changed(&self, key: &str, old_value: &str, new_value: &str) {
        eprintln!(
            "[SamplePlugin] フィールド変更: key={key}, {old_value} → {new_value}"
        );
    }
}
