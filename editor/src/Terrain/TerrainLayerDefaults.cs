...
internal static class TerrainLayerDefaults
{
    ...
    /// <summary>
    /// 「レイヤ追加」で作られた直後のレイヤに与える優先度。
    ///
    /// 【なぜ既定値（1.0）ではなく 0 なのか — 実害のあった不具合の再発防止】
    ///   レイヤの既定ルールは「斜度 0〜90 度・高度 無制限」＝**地形の全頂点で成立**する。
    ///   そこへ既定の優先度 1.0 が付くと、追加した瞬間にそのレイヤが地形全域の
    ///   自動ペイント重みを 1/N ぶん奪う。テクスチャ未設定の新規レイヤは
    ///   <see cref="BaseColor"/>（中間グレー）の単色なので、
    ///   「自動ペイント部分だけ色が抜けて灰色になる（模様＝他レイヤのテクスチャは残る）」
    ///   という、原因の分かりにくい見た目の劣化になる。
    ///   手動ペイントはルールを迂回して重みを直接書くため、なぞった所だけ色が戻る
    ///   ——これが「手で塗ると直る」症状の正体だった。
    ///
    ///   優先度 0 なら <c>LayerRule::evaluate</c> が常に 0 を返し、そのレイヤは
    ///   自動ペイントへ一切載らない（手動ペイントでは従来どおり塗れる）。
    ///   自動ペイントさせたくなったら UI の「優先度」を 0 より大きくすればよい。
    /// </summary>
    public const double NewLayerPriority = 0.0;
