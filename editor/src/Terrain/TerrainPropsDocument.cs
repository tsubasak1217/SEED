...
internal sealed class TerrainPropsDocument
{
    ...
    ///
    /// <para>
    /// **真のまま保存してはいけない**（フォールバックの 1 プロップで props.json を
    ...
    public static TerrainPropsDocument Load(string assetsRoot)
    {
        ...
        var read = AssetMigrationGateway.ReadFile(path, AssetFormats.TerrainProps);
        if (read.IsBlocked)
        {
            // 未来版・変換失敗。フォールバックで開くと保存で上書きしてしまうので、
            // 「読めなかった」印を付けて返す（呼び出し側が編集させない）。
            var blocked = new TerrainPropsDocument(new JsonObject())
            {
                WasMissingOrInvalid = true,
                IsUnreadable = true,
            };
            blocked.Props.Add(TerrainPropEdit.CreateNew(FallbackPropId, FallbackPropName));
            return blocked;
        }
    ...
    public string ToJsonString()
    {
        ...
        return AssetVersionStamp
            .WithVersionFirst(_root, AssetFormats.TerrainProps)
