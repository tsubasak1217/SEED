...
| `simplify.rs` | その場デシメート（QEM ハーフエッジコラプス。境界頂点ロック。§18） |
| `meta.rs` | 地形フォルダの付随メタデータ（当たり判定 ON/OFF・デシメート強度）の直列化（§8） |
| `tests.rs` | 4 テスト（水密性・法線向き・境界同期・tvox 往復） |
| `tests_simplify.rs` | デシメートの継ぎ目保証・整合・削減率（§18.2 / §18.7） |
| `tests_meta.rs` | メタデータの往復・決定性・後方互換 |
...
| `core/app_base/app/terrain_collision_ops.rs` | チャンク単位の当たり判定 ON/OFF・境界オーバーレイ（§17） |
| `core/app_base/app/terrain_decimate_ops.rs` | その場デシメートの適用（全チャンク再メッシュ・頂点数集計。§18） |
| `core/app_base/app/terrain_meta_ops.rs` | `terrain_meta.json` の読み書き（§8） |
...
### tvox フォーマット（v4・リトルエンディアン）— T2b でレイヤ番号付き、T4 で法線シャープネス追加
...
| 24 | f32 | voxel_size（m） |
| 28 | u32 | **slot_count**（1 サンプルのブレンドスロット数。v3/v4 では `TERRAIN_BLEND_SLOTS`=4） |
...
| … | u8 × N | **法線シャープネス**（0=スムーズ法線〜255=面法線。v4 で追加。§19 参照） |

ヘッダ v4 = 32 バイト（v3 と同一。増えるのは本体末尾のブロックだけ）。ヘッダ v3 = 32 バイト（v2 と同じヘッダ長。28 バイト目の意味が layer_count→slot_count に変わるだけ。
...
**v3 後方互換（T4 で追加）**: v3 は法線シャープネスのブロックを持たない。v3 を読んだ場合は
全サンプルのシャープネスを 0（＝完全スムーズ法線）として復元する。これは法線シャープネス
ペイント導入前の見た目とビット単位で一致する。

保存は常に v4 で行う（v1/v2/v3 の書き手 `write_chunk_v1`/`write_chunk_v2`/`write_chunk_v3` は
テスト・移行用のみ）。
...
#### terrain_meta.json（地形フォルダの付随メタデータ）

地形フォルダ直下に `terrain_meta.json` を 1 枚置き、**チャンク 1 枚ぶんの密度に属さない
地形アセット全体の設定**をまとめて持つ。正典は `runtime/src/engine/terrain/meta.rs`（純関数・
単体テスト付き）で、ファイル IO は `terrain_meta_ops.rs`。

```json
{
  "version": 1,
  "collision_disabled": [[1, 0, 2], [3, 0, 1]],
  "decimate_strength": 0.5
}
```

| キー | 意味 | 既定 |
|---|---|---|
| `collision_disabled` | 物理コライダーを登録しない**チャンク座標の一覧**（§17） | `[]`＝全チャンク有効 |
| `decimate_strength` | その場デシメートの強度 0〜1（§18） | `0.0`＝簡略化なし |

**ファイルが無い＝すべて既定**（全チャンク当たり判定あり・デシメートなし）なので、
この機能より前に保存された地形フォルダはそのまま開ける。`.tvox` のヘッダを拡張しなかったのは、
あちらが固定長バイナリで、項目を足すとバージョンを上げて全チャンクを書き直すことになるからである。
保存は `.tvox` / `.tscatter` / `.tcover` と同じ 3 経路（`TERRAIN_SAVE` / `TERRAIN_SAVE_AS` /
シーン保存の差分フラッシュ）すべてで行う。別名保存で移し忘れると
「移した先だけ当たり判定設定が抜ける」ことになるため、必ず一緒に運ぶ。

無効チャンクの一覧は書き出し前に座標順（x, y, z）へ並べる。並べないと `HashSet` の走査順で
毎回ファイルの中身が変わり、何も変えていないのに差分が湧く。
...
## 17. チャンク単位の当たり判定 ON/OFF（エディタ専用機能）

地形チャンクごとに「物理コライダーを登録するか」を切り替える。**描画には一切影響しない**
（見た目はそのままで衝突だけ消える／戻る）。

### 17.1 何のためにあるか

地形の一部（遠景の飾り・屋根の上・演出用の張り出しなど）はプレイヤーが決して触れないため、
トライメッシュコライダーを持つだけ無駄である。チャンク単位で外せば、Play 開始時の
QBVH 構築コスト（`register_all_terrain_colliders`。§16.3）と物理ワールドのメモリを、
見た目を一切変えずに削れる。

### 17.2 状態と永続化

- 実行時状態は `TerrainState::collision_disabled: HashSet<ChunkCoord>`。
  **「無効なものだけ」を持つ**のがこの設計の要で、チャンクを追加したときに既定が自動で有効になり、
  この機能より前のデータが「全チャンク有効」としてそのまま開ける。
- 永続化は地形フォルダの `terrain_meta.json`（§8）。ファイルが無ければ全チャンク有効。
- ロード時は存在しないチャンク座標のフラグを捨てる（地形を作り直して小さくしたあと、
  ありもしないチャンクの無効フラグが保存され続けるのを防ぐ）。

### 17.3 物理への反映（有効／無効どちらの向きも 1 本の経路）

- **Play 開始時**: `collider_target_coords`（純関数・`terrain_collision_ops.rs`）が
  無効チャンクを除いた座標を決定的な順で返し、`register_all_terrain_colliders` はそれだけを登録する。
- **編集中のトグル**: `sync_terrain_chunk_collider` を呼ぶ。この関数は
  「既存コライダーを必ず `RemoveObject` してから、**無効チャンクなら再登録せずに帰る**」
  という作りなので、有効→無効・無効→有効のどちらの向きでも 1 本呼ぶだけで物理ワールドが正しくなる。
  物理停止中は no-op（次の Play で反映される）。

### 17.4 エディタ操作

1. シーンモードを **terrain** にし、左縦パネルのツール一覧から **「コリジョン」** を選ぶ。
2. 選択中はビューポートに**チャンク境界のワイヤ枠**が出る。
   **薄い緑 = 当たり判定あり／薄い赤 = 当たり判定なし**（`terrain_collision_ops.rs` の色定数）。
   枠は Edit のみ・terrain モードのみで、Play では出ない。
3. 地形をクリックすると、着弾点のチャンクの当たり判定が**反転**する。
   ブラシではないので**ドラッグしても連続反転しない**（1 クリック 1 トグル）。
   押しっぱなしで隣のチャンクまで消える事故を防ぐため、意図的にストローク状態にしない。
4. ステータスバーに `チャンク (x, y, z) → 当たり判定なし` のように出る。
5. **Ctrl+Z（terrain 専用 Undo）で 1 手ずつ戻る**。トグルは `TerrainEdit` の
   `collision_before` / `collision_after`（変更したチャンクだけを持つマップ）に載る。
   密度もカバーも変わらないので、Undo で再メッシュは走らない。

ツール選択中は半径・強度スライダーとブラシプレビュー球を隠す（対象はチャンク 1 枚であり、
どちらも意味を持たないため。「選択に無関係なパラメータは出さない」という UI 方針に従う）。

### 17.5 IPC

| コマンド（送信） | 引数 | 応答（受信） |
|---|---|---|
| `TERRAIN_COLLISION_TOGGLE:{screen_x},{screen_y}` | f32×2。クリック位置 | `TERRAIN_COLLISION_OK:{x},{y},{z},{0\|1}` / `TERRAIN_COLLISION_MISS` |
| `TERRAIN_COLLISION_OVERLAY:{0\|1}` | 境界枠の表示切替（表示状態なので保存も Undo もしない） | なし |

スクリプト API は用意していない（エディタ専用のオーサリング機能であり、
実行時にチャンク単位で当たり判定を切り替える用途が無いため）。

---

## 18. その場デシメート（地形メッシュの頂点数削減）

現在ロード中の地形メッシュを、**再設定・再読込なしにその場で**簡略化して頂点数を減らす。
描画・物理・レイトレ（BLAS）の負荷が同時に下がる。

### 18.1 方式 — QEM の「ハーフエッジ」コラプス

アルゴリズム本体は `runtime/src/engine/terrain/simplify.rs`（エンジン非依存の純粋層）。
Garland–Heckbert の二次誤差計量（Quadric Error Metric）でエッジの潰しやすさを順位づけし、
誤差の小さいものから潰す。ただし**潰し先は必ず既存頂点のどちらか**にする
（最適点を新しく作る「フルエッジ」コラプスは採らない）。この制限には 3 つの理由がある。

1. 生き残る頂点がすべて**元の MC 頂点そのもの**になるため、`TerrainVertexEdge`（頂点の由来辺）を
   そのまま引き継げる。由来辺はレイヤペイント高速パス（§12.6）の唯一の手掛かりであり、
   新しい位置の頂点を作ると由来辺が定義できず高速パスが死ぬ。
2. 同じ理由で法線・スプラットも引き継げる。とくに法線は密度勾配から作った高品位なもので、
   面から作り直すと**隣接チャンクと境界頂点の法線が食い違い、陰影の継ぎ目が出る**。
   元の値を持ち回ればその心配が原理的に無い。
3. 頂点位置が動かないので、境界頂点を潰し先に固定するだけで継ぎ目の位置ずれがゼロになる。

**非多様体を作らないためのガードが 2 つ**入っている。
リンク条件（a と b の 1 近傍が共有する頂点数が、辺 (a,b) を含む三角形数と一致するときだけ潰す）と、
法線反転ガード（潰したあと a に接していた面が 90 度以上倒れるなら潰さない）である。

### 18.2 継ぎ目の保証（最重要）

チャンク境界面（x=0 / x=extent / y=0 / y=extent / z=0 / z=extent）に載る頂点は**ロック**され、
潰される側には決してならない（潰し先にはなれる）。ロック頂点どうしのエッジも潰さない。
MC の境界頂点は隣接チャンクと同じ密度サンプルから同じ位置に生まれるため（§5）、
「境界頂点集合が変わらない」＝「継ぎ目が開かない」がそのまま成り立つ。
開いた縁（1 枚の面しか共有しない辺）の頂点も同じ理由でロックする。

`tests_simplify.rs` の `adjacent_chunks_share_identical_boundary_after_simplify` が、
X 方向に隣接する 2 チャンクを同じ密度関数から作り、共有面上の頂点集合が
デシメート前後・強度を変えても**ビット単位で一致し続ける**ことを検証している。

**LOD との関係**: デシメートは **LOD0（フル解像度）のメッシュにだけ**掛ける。
LOD>=1 は既に密度グリッドを間引いた低ポリメッシュで、さらにスカート（§7.1）が混ざっており、
簡略化のロック条件（境界頂点＝隣と共有）が成り立たない。
遠景の頂点数は LOD が、近景の頂点数はデシメートが受け持つという分担になる。

### 18.3 適用の仕組み — 強度を状態に持ち、再メッシュで反映する

`TerrainState::decimate_strength`（0〜1）を設定して**全チャンクを再メッシュ**する
（`remesh_chunks(RemeshOptions::immediate())`）。再メッシュ経路は既に

- 頂点／インデックスバッファの差し替え
- 統合バッチキャッシュの無効化
- RT BLAS の prune（再構築予約）
- 物理コライダーの Remove→Add（**当たり判定が有効なチャンクのみ**。§17.3）
- カバー場（§I3.1）の派生データ破棄と頂点への焼き直し予約

を漏れなくやるので、経路を 1 本に保てる。頂点バッファを別経路で書き換えると、
この 5 つのどれかを取りこぼす（＝古い当たり判定・古い影が残る）。

強度は状態なので、この後スカルプトで再メッシュが走っても掛かり続ける
（掘った跡だけ急に高精細へ戻る、という不整合が起きない）。
コライダーの MC フォールバック経路（`build_chunk_collider_shape`）にも同じ強度を掛けるため、
「見た目は粗いのに当たり判定だけ細かい」チャンクは生じない。

**カバー焼き込みとの整合**: 再メッシュのたびに `invalidate_cover_for_remesh` が
`cover_base_mesh`（カバー適用前の頂点位置・法線）を捨てて焼き直しを予約するので、
頂点数が変わっても基準メッシュのバッファ長は必ず新しいメッシュに合わせて作り直される。
凍結波（`bake_wave.rs`）に乗っていたチャンクとその 26 近傍も波から追い出され、
次の波で新しいメッシュへ焼き直される。

### 18.4 SDF 密度場は変更しない（非破壊）

簡略化はマーチングキューブスの**出力**に対して行い、密度グリッドは 1 ビットも変わらない。
したがって:

- スカルプトを続ければ、常に**元の密度から作り直したメッシュ**へその時点の強度が掛かる。
- 強度 0 で適用し直せば、**元の頂点数へ完全に戻る**。
- `.tvox` の中身はデシメート前後で同一である。

### 18.5 保存されるもの・されないもの

**デシメート結果のメッシュそのものは保存されない**（メッシュ由来のデータなので `.tvox` に入らない）。
保存されるのは**強度だけ**で、地形フォルダの `terrain_meta.json`（§8）に記録される。
ロード時はメッシュ化（フェーズ 2）より**前**に強度を読み込むので、
保存したときと同じ見た目でシーンが開く（＝ロード後の自動再適用）。

強度を読むのがメッシュ化より前でなければならないのは、強度がメッシュ生成時に効く
パラメータだからである。後から読むと、ロード直後だけフル解像度で描かれ、
次の編集で急に粗くなる。

### 18.6 エディタ操作

terrain モードの**上部バー**に「デシメート強度」スライダー（0〜1）と「デシメート」ボタンがある。

1. スライダーで強度を決める。**値を動かしただけでは何も起きない**
   （掛かる処理が全チャンク再メッシュなので、スライダー追従で走らせてはいけない）。
2. 「デシメート」ボタンで**全チャンク一括**適用。Edit モード限定。
3. 適用後、ステータスバーに `デシメート: 頂点 123,456 → 34,567（72.0% 削減・強度 1.00）` と出る。
4. 強度 0 で押すと簡略化が解除され、フル解像度のメッシュへ戻る（非破壊の確認手段）。

### 18.7 実測削減率

`cargo test -- --ignored --nocapture measure_reduction_rates` で計測できる
（1 チャンク = 16 セル × 0.5m）。

| 地形 | 強度 0.25 | 強度 0.50 | 強度 0.75 | 強度 1.00 |
|---|---|---|---|---|
| 平坦（289 頂点） | 17.3% | 34.9% | 52.2% | 69.9% |
| ゆるい起伏（373 頂点） | 18.0% | 36.2% | 54.4% | 72.4% |
| 強い起伏（573 頂点） | 18.7% | 37.5% | 56.4% | 75.2% |

強度 1.0 でも 100% にならないのは、境界頂点がロックされている（チャンクが小さいほど
境界頂点の割合が高い）ことと、`MAX_REMOVE_FRACTION = 0.9`（潰せる頂点の 9 割までに留める）
の安全弁があるためである。100% まで潰すと平坦チャンクが 1 枚のポリゴンまで落ちて陰影が破綻する。

実運用サイズ（`chunk_cells=32` / voxel 0.5m ＝ 16m 角・2273 頂点、強い起伏）の計測は
`measure_realistic_chunk_cost`（同じく `#[ignore]`）で取れる。release ビルドで:

| 強度 | 頂点 | 削減率 | 所要（1 チャンク） |
|---|---|---|---|
| 0.50 | 2273 → 1339 | 41.1% | 4.4ms |
| 1.00 | 2273 → 405 | 82.2% | 9.6ms |

簡略化は `build_chunk_cpu_model` の内側で走るため、`remesh_chunks` フェーズ 0 の
rayon 並列（チャンク間）にそのまま乗る。ボタン押下の一括適用は数十チャンクでも
1 秒未満で終わる。ただし**強度を掛けたままスカルプトすると、ブラシが触れたチャンクの
再メッシュにこの時間が上乗せされる**（4 チャンク触れて約 10ms／並列）。
造形中は強度 0、仕上げに掛ける、という使い方が軽い。

### 18.8 IPC

| コマンド（送信） | 引数 | 応答（受信） |
|---|---|---|
| `TERRAIN_DECIMATE:{strength}` | f32・0〜1（範囲外はランタイムがクランプ） | `TERRAIN_DECIMATE_OK:{強度},{適用前頂点数},{適用後頂点数}` / `TERRAIN_DECIMATE_ERROR:{理由}` |

スクリプト API は用意していない（オーサリング時の最適化操作であり、
実行時にメッシュ密度を切り替える用途は LOD（§7.1）が担うため）。

---

## 19. 法線シャープネスペイント（スムーズ法線 ⇔ 面法線の塗り分け）

地形の法線は SDF（密度場）の勾配から作られるため、既定では**どこも完全に滑らか**である。
これは砂地・草地には合うが、岩場・崖には合わない（丸い岩に見えてしまう）。
そこで「スムーズ法線と面法線をどの比率で混ぜるか」を **0〜1 の重み `w`** として塗れるようにした。

- `w = 0` … 従来どおり SDF 勾配のスムーズ法線（砂地・なだらかな地面）
- `w = 1` … フラグメントの面法線（ファセット状のパキっとした岩場）
- 中間 … その配合。ブラシで滑らかに遷移させられる

### 19.1 データ — 密度・スプラットと同じ格子に持つ

`w` は**頂点ではなくボクセルサンプル**に持つ（`TerrainChunkData::sharpness`、u8 量子化・
1 チャンクあたり +35.9 KB）。頂点ではなくサンプルに持つのが設計の要点で、これにより
レイヤペイント（T2）とまったく同じ性質が自動的に手に入る。

| 性質 | 成立する理由 |
|---|---|
| 再メッシュで値が残る | 掘削・盛り上げ・LOD 切替は頂点を作り直すが、グリッドは触らない |
| チャンク境界が割れない | 境界サンプルは隣接チャンクが重複所有し、書き込み時に同期される |
| Undo に載る | `ChunkSnapshot` が密度・スプラットと一緒に丸ごと控える（ストローク単位） |
| 永続化が済む | `.tvox` v4 の末尾ブロック。旧データ（v1/v2/v3）は `w=0` で読める |

頂点値はメッシュ生成時に、レイヤ重みと**同じ辺・同じ補間係数 `t`** で線形補間される
（`marching_cubes::interp_vertex_sharpness`）。フル生成とペイント高速パスが同一関数を
使うため、両経路の結果は定義上必ず一致する。

**デシメート（§18）での継承**: QEM ハーフエッジコラプスは生き残る頂点が必ず元の MC 頂点
そのものなので、`w` は値を作り直さずそのまま引き継がれる（`simplify.rs`）。

### 19.2 頂点への載せ方 — `tangent.w`（頂点フォーマットは増やさない）

`Vertex` / mesh_vertex レイアウトはエンジン内の全パイプラインが共有しており、1 成分でも
増やすと 20 以上のパイプラインへ波及する（T2 でレイヤ重みを頂点カラーへ載せたのと同じ制約）。
地形は法線マップを持たないため接線が完全に未使用であり、ここが唯一残っていた空き成分である。

符号化は **`tangent.w = 1.0 + w`**（`terrain_mesh_build::encode_sharpness_tangent`）。
0〜1 の生値ではなく 1〜2 に載せるのは、共有頂点シェーダが従法線を
`normalize(cross(N, T) * tangent.w)` で作るためである。`w=0` を生で入れると
`normalize(0ベクトル)` になり従法線が NaN になる。1 起点なら `tangent.w` は常に正で、
`normalize` が長さを捨てるので**従来（w=+1）とビット単位で同じ従法線**が得られる。

復号は WGSL の `decode_vertex_sharpness(tangent_w) = clamp(tangent_w - 1, 0, 1)`。
通常メッシュの `tangent.w`（±1 のハンドネス）はこの式で必ず 0 に落ちるため、
**地形以外は常にスムーズ法線＝この機能の導入前と同じ絵**になる。
Rust ↔ WGSL の定数一致は `sharpness_tangent_bias_matches_shader` テストが文字列で固定する。

運搬は `VertexOutput.sharpness`（`@location(10)`）。値を埋めるのは
`shader_static_vertex` / `gbuffer_static_vertex` / `shader_skinned_vertex` /
`gbuffer_skinned_vertex` の 4 本。
...
`terrain_shaded_normal` は `w <= 0.0009` なら `smooth_n` をそのまま返す（＝塗っていない
地形は分岐へ入らず、導入前と完全に同じ出力）。それ以外では

1. `face_n = normalize(cross(dpdx_p, dpdy_p))` — 三角形の平面に垂直なベクトル
2. 向きが定まらない（画面上の巻きに依存する）ので `dot(face_n, smooth_n) < 0` なら反転
3. `n = normalize(mix(smooth_n, face_n, w))`

縮退（画面上で潰れた面 → 外積が 0 ベクトル）では `smooth_n` へフォールバックする。
これは NaN 法線を G-Buffer へ書かないための必須のガードである。

`geo_n` は**それ以降の全処理**が使う。すなわち triplanar の面選択・法線マップの whiteout
合成も面法線に追従するので、岩場では投影面の切り替わりまでファセットに揃う。
ライティング・影・SSR は G-Buffer を読むだけなので、地形専用の追加処理は一切ない。

`dpdx_p` / `dpdy_p` は triplanar が既に一様制御フローで取っていた値をそのまま使い回す
（新しい derivative を増やしていない）。

### 19.4 制約 — RT / GI 経路はスムーズのまま

**レイトレース経路（RT 影・RT 反射・DDGI・水面反射）と、頂点法線を直接読むあらゆる経路は
シャープネスを見ない。** これらはラスタライズしないので `dFdx/dFdy` が存在せず、面法線を
同じ方法では作れないためである。BLAS に載るのは頂点位置と（必要なら）頂点法線＝
SDF 勾配のスムーズ法線だけ。
...
| 経路 | シャープネスの反映 |
|---|---|
| Deferred の直接光・影・SSAO・SSR | **反映される**（G-Buffer 法線を読むため） |
| RT 影（遮蔽判定） | 反映されない（形状は同じなので影の輪郭は変わらない。半影の柔らかさに微差） |
| RT 反射・水面反射・DDGI のバウンス色 | 反映されない（ヒット面の陰影がスムーズ法線基準になる） |
| チャンク平均アルベド（縮退色） | 無関係（色は変えない機能なので影響なし） |

岩場を鏡面の水辺に映すと「映り込みだけ滑らか」になるが、色は同じなので実用上は目立たない。
これを解消するには面法線を頂点側へ焼く（＝頂点を分割する）必要があり、水密メッシュ・
コライダー・デシメートの前提が全部崩れるため採らなかった。

### 19.5 エディタ操作

左の縦パネルで **ペイント**ツールを選ぶと「塗り対象」コンボが出る。

| 塗り対象 | 出るパラメータ | 送る IPC |
|---|---|---|
| レイヤ | レイヤ選択コンボ（layers.json 由来） | `TERRAIN_PAINT` |
| 法線 | 目標値スライダー（0〜1）＋「消去」チェック | `TERRAIN_SHARPNESS` |

ブラシ半径・強度スライダーは共通。なぞった場所は
`w ← w + (目標値 - w) × (強度 × フォールオフ)` で目標値へ**収束**する
（上げも下げも同じ式なので、目標値 0 でなぞれば消去になる。「消去」チェックは
スライダーの値によらず目標値 0 を送るショートカット）。

Undo は密度ブラシ・レイヤペイントと同じ地形専用ストローク単位に載る
（`TERRAIN_STROKE_END` で 1 エントリ確定。混在したストロークも 1 手で巻き戻る）。

### 19.6 性能 — レイヤペイントと同じ高速パス

シャープネスは密度を変えないので、マーチングキューブスは走らない。
`apply_terrain_paint_colors`（レイヤペイントの高速パス）が、由来辺キャッシュから
`w` を引き直して頂点の接線 w を差し替え、GPU 頂点バッファを 1 回の `write_buffer` で
丸ごと更新する。BLAS 再構築も統合バッチ再構築も起きない。

### 19.7 IPC

| コマンド（送信） | 引数 | 応答（受信） |
|---|---|---|
| `TERRAIN_SHARPNESS:{target},{screen_x},{screen_y},{radius},{strength}` | f32×5。`target` は目標シャープネス（0〜1） | `TERRAIN_SHARPNESS_OK:{target},{x},{y},{z}` / `TERRAIN_SHARPNESS_MISS` |

スクリプト API は用意していない（オーサリング時の見た目調整であり、
実行時に配合率を動かす用途が現状無いため）。
