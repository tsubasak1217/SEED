...
- **時間で動く光応答が書ける**。`ShadingSurface.time`（秒）で脈動する発光・
  流れる縞・明滅などをアセットだけで表現できる（7.5 節）。`time` は
  **Play（非ポーズ）中はゲーム内累計時間、Edit・ポーズ中は常時進む壁時計**で、
  どちらのモードでも止まらない（＝編集中もアニメーションが動く）。
...
| `time` | `f32` | 累計時間（秒） | アニメーションする光応答（脈動・流れる縞・明滅）の唯一の時間入力。**Play（非ポーズ）中はゲーム内累計時間**（スクリプトの `SEED.Time.ElapsedTime`・草の揺れと同一の値。Play 開始とシーン再ロードで 0 に戻る）、**Edit・ポーズ中は常時進む壁時計**（`Clock::ambient_time`。水面の波と共通）。どちらでも止まらないので編集中も動きが見える。モード切替時に位相は跳ぶ。→ 7.5 節に例 |
