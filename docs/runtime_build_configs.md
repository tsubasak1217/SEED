...
Android 向け（`libSEED.so` と APK）のビルドもこの構成とは独立していて、SeedAndroid（`editor/tools/SeedAndroid`。cargo ndk）の
`--release`（`runtime/android/build_and_run.ps1` の `-Release`）の有無で dev / release を選ぶ。構成と手順の正典は [android.md](android.md)。
