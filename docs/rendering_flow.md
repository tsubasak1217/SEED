...
#### 幾何法線 Ng の復元は「カメラ相対座標」で微分する（f32 桁落ち対策）

deferred は幾何法線 `Ng` を G-Buffer に焼かず、深度から復元した座標の画面微分
`cross(dpdx(p), dpdy(p))` で作る。このとき **`p` に絶対ワールド座標を使ってはならない**。

- **症状**: 原点から離れた位置（実機で確認したのは約 45m）にシーンを置くと、スキンメッシュや
  大きな平面ポリゴンに黒い斑点ノイズが乗る。静止していても明滅し、ライティングを OFF にすると
  消え、法線デバッグ表示では正常に見える。
- **原因**: f32 の刻み幅は絶対値に比例する。45m 地点では ulp(45) ≒ 3.8e-6 m あり、これが
  **画素ごとにランダムな**丸め誤差として乗る。1 画素ぶんの世界差分は近距離・狭 FOV では
  1e-3 m 程度しかないため、微分に対して無視できない比率になる。外積はほぼ平行な 2 本の
  ベクトルの差を取る演算なのでこの誤差が角度誤差へ増幅され、数値実験（原点から 64m・
  被写体まで 2m・FOV 20°）では **Ng の角度誤差が 4.5°** に達する。これが
  `lighting_eval.wgsl` の幾何ゲートで 0/1 の斑点へ変換されていた。
- **対策**: 逆 ViewProjection 行列から**平行移動成分（カメラ位置）を落とした行列**で復元し、
  得られた「カメラ相対座標」で微分する。外積は平行移動不変なので **Ng の意味は完全に不変**で、
  変わるのは精度だけ。行列の減算誤差は全画素で共通の定数なので、隣接画素の差である微分では
  ほぼ相殺される。上の数値実験で誤差は 4.5° → 0.08° になる。
- **やってはいけない対策**: 復元**後**の `world_pos` から `camera_position` を引く形。
  `world_pos` はその時点で既に ulp(45) へ丸められており、引き算は失われたビットを取り戻さない
  （実測でも角度誤差は 4.4694° → 4.4694° と完全に不変）。直感に反するので注意。

適用箇所（いずれも同一の式。`ivp` の各列について `xyz -= cam * w`、`w` 行は不変）:

| シェーダ | 関数 | Ng の用途 |
|---|---|---|
| `deferred_lighting.wgsl` | `deferred_camera_relative_ivp` | 幾何ゲート（`Surface.geo_normal`） |
| `ao_common.wgsl` / `ao_rt.wgsl` | `ao_cam_rel_pos` | RT-AO のレイ原点クリアランス方向 |
| `shadow_mask.wgsl` | `mask_cam_rel_pos` | RT ソフト影マスクのレイ原点クリアランス方向 |

微分は必ず **discard / 早期 return より前**（一様制御フロー）で評価する。WGSL の `discard` は
ヘルパー化（demote）であり後続コードも実行されるため理屈の上では後でも取れるが、早期終了として
実装しうるドライバでの微分破損を原理的に避けるため、先頭側で済ませる規約とする。
（`deferred_lighting.wgsl` だけは下記「シルエット境界は…」のとおりハードウェア微分を
使わなくなったため、この制約から外れている。`ao_rt.wgsl` / `shadow_mask.wgsl` は従来どおり。）

回帰テストは `renderer/tests_geo_normal_precision.rs`（シェーダと同一式の CPU 写しで
桁落ち量を実測し、絶対ワールド座標への退行と「復元後の減算」への差し替えを止める）。

**未対応（既知の残件）**: フォワード経路の `surface_gather.wgsl:geometric_normal` と
`terrain_gbuffer_write.wgsl` は、深度復元ではなく**補間ワールド座標の varying**を微分している。
同じ桁落ちを受けるが、修正には `VertexOutput` にカメラ相対の varying を追加する必要があり
（頂点シェーダ側で引けば有効。フラグメント側で引いても無意味）、影響範囲が広いため別件とした。

#### シルエット境界は「深度差の小さい側の片側差分」で微分する

ハードウェアの `dpdx/dpdy` は **2x2 クアッド固定**の差分なので、シルエット境界の画素では
必ず片側が別サーフェス（奥の物体）へ飛び、その画素だけ Ng が大きく傾く。
`geo_gate` はその Ng で直接光を遮断するため、**輪郭に沿った 1px の黒い縁**として出る
（小屋の梁の上端・キャラクターの輪郭で実機確認。2026-09-13）。

対策として `deferred_lighting.wgsl` は Ng 用の微分をハードウェア微分ではなく
**明示的な近傍 `textureLoad` による片側差分**で取る:

1. 中心画素の深度と、左右・上下の深度を読む（`t_depth` を 4 回 `textureLoad`）。
2. 左右・上下それぞれで **深度差が中心に近い側**を選ぶ（同一サーフェスである可能性が高い側）。
3. 選んだ側の画素を `deferred_rel_pos_at`（中心と同じ写像・カメラ相対）で復元し、
   その差分を `dpdx/dpdy` と同じ向き（+x 右 / +y 下）へ符号を揃えて外積する。

これで手前側のサーフェスは自分の面の隣接画素だけで微分でき、境界でも正しい面法線になる。
`textureLoad` は範囲外で 0（＝最も手前）を返すため、画面端では自動的に反対側が選ばれる。

- 微分がハードウェア依存でなくなったため、**一様制御フロー（discard より前）の制約は消えた**。
- 1 画素幅の細い物体など「両側とも別サーフェス」の画素は依然として正しく復元できない
  （2x2 クアッド微分でも同じ）。その場合でも誤差は 1px に留まる。
- コスト: 深度テクスチャの `textureLoad` ×4 と mat4 積 ×2 の増加。800x600・debug の実測では
  同一環境で交互に測った対照（ハードウェア微分）との差が計測ノイズ以下だった。

#### 幾何ゲートは 0/1 の階段ではなく狭い遷移帯

`lighting_eval.wgsl` の `geo_gate` は
`smoothstep(RT_GEO_SHADOW_MIN_COS, RT_GEO_SHADOW_MIN_COS + RT_GEO_SHADOW_SOFT_COS, dot(Ng, L))`。

`Ng` は画面微分から復元する数値量なので、桁落ち対策後も 0.1° 規模の残差誤差が残る。
`dot(Ng, L) > 0` の二値ゲートだとこの残差が、ターミネータ付近（`dot(Ng,L)` ≒ 0 の帯。
大きな平面ポリゴンでは画面上の広い面積）でそのまま 0/1 の斑点へ増幅される。
幅 `RT_GEO_SHADOW_SOFT_COS = 0.02`（地平線から約 1.1°）の遷移帯を与えると、残差はゲート値の
連続なわずかな揺らぎに落ちる。下端は `RT_GEO_SHADOW_MIN_COS = 0.0` のままなので
`dot(Ng, L) <= 0` は smoothstep が厳密に 0 を返し、**裏面への光漏れ防止というゲート本来の
目的は不変**。見た目の変化はターミネータが 1° ぶん柔らかくなる程度（遷移帯は放射照度が
cos で 0〜0.02 倍しかないほぼ真っ暗な領域）。

#### 遠景では Ng を捨てて G-Buffer 法線 N へ倒す（深度量子化によるドット状ノイズ対策）

上の遷移帯（0.02＝1.1°）で吸収できるのは **1° 規模**の残差までである。deferred の Ng には
もう 1 つ、距離とともに増える誤差源がある: **深度バッファそのものの量子化**。
非線形な `[0,1]` 深度では 1 ulp（Depth32Float で相対 2⁻²³）が遠方で大きなワールド距離に化ける。

| カメラ距離 | 深度 1 ulp のワールド幅 | 1 画素のワールド幅 | Ng の角度誤差 |
|---|---|---|---|
| 5m | 0.03mm | 3.7mm | 0.008（0.5°） |
| 150m | 2.6cm | 11cm | 0.24（14°） |
| 440m | 22cm | 61cm | 0.37（21°） |

（`near=0.1 / far=1000`, `fov_y=25°`, 600px 高さでの実測式。解像度が上がるほど 1 画素の
ワールド幅が縮むので誤差は**大きくなる**。）

誤差が遷移帯を大きく超えると `geo_gate` はターミネータ付近で 0/1 に振れ、
**点状・ハッチ状の黒いドット**になる（遠景の島の斜面で実機確認。2026-09-13）。
そこで `deferred_lighting.wgsl` は「深度を 1 ulp 動かしたときの復元位置の移動量 ÷ 1 画素ぶんの
移動量」で誤差を毎ピクセル見積もり、`DEFERRED_NG_ERR_TRUST_LO(0.02)`〜`_HI(0.06)` で
`Ng` を `N`（G-Buffer 法線）へ**滑らかに倒す**。

- 近距離（誤差 ≤ 0.02）では従来と 1 ビットも変わらない。
- 遠距離（誤差 ≥ 0.06。典型的なカメラで 30m 以上）では `Ng = N` となり、
  草・地形の `authored` フラグ（RT1.w=1）と同じ状態になる。
  幾何ゲート本来の「法線マップが表と言い張る薄い面の裏面光漏れ防止」はその距離では効かないが、
  漏れ自体が数ピクセル未満に縮む距離なので割り切っている。
- 深度**不連続**（草の刃・地形のシルエット）による暴れは従来どおり RT1.w の authored フラグが担当。
  こちらは連続した 1 枚の面の内部でも起きる**別要因**である。
