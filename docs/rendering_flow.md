...
マテリアル（G-Buffer 書き込み）が担うのは **表面の自己申告** だけである。
...
一方エンジン側のライティング段（`deferred_lighting.wgsl` + `lighting_eval.wgsl` + `shadow.wgsl` +
`rt_shadow_*.wgsl` + `ddgi_common.wgsl` + クラスタ / AO / シャドウマスク / SSGI / 反射）が担うのは
**不変の光の物理** ——どのライトが届き、何に遮られ、どう間接的に回り込み、どう反射するか——であり、
これはマテリアルの種類に依存しない共通処理として 1 箇所に集約されている。
この分界のおかげで、新しいマテリアル表現を足すときはマテリアル段の G-Buffer 出力だけを考えればよく、
新しい光の機能（RT 影・DDGI・クラスタ等）を足すときはライティング段だけを触ればよい。
