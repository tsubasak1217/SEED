...
- [ ] **ポーズメニューのスプライトが FishingUI のダイアログより奥に描かれる** — 2026-09-08。動的生成した PauseMenu キャンバスは root の末尾に付くのに、暗幕（PauseOverlay）と選択の下敷き（PauseHighlight）が FishingUI のダイアログ枠より奥に出る。一方でテキストは手前に出るため、「文字だけダイアログの上に浮く」見た目になる（`fix_menu_open.png` で実測）。スプライトとテキストで別パスを通っているか、キャンバス間の描画順が生成順ではなく別の基準で決まっている疑いがある。選択行の下敷きは 2 行目・3 行目ではダイアログ枠に完全に隠れて見えない。
...
- [ ] **ポーズ中でもチュートリアルの吹き出しがメニューより手前に出る** — 2026-09-08。ヘッドレス確認（`Esc` でポーズを開いた画面）で、チュートリアルの会話ウィンドウが `PauseItem1`（図鑑）の上へ重なって描かれていた。ポーズメニューの Sprite/Text は layer 2000〜2004、チュートリアル側はそれより手前のレイヤーを使っている。ポーズ中は会話ウィンドウを隠すか、ポーズメニューのレイヤーを最前面帯（例 3000 台）へ上げるかの方針決めが要る。今回の PauseMenu 改修とは別件のためスコープ外。
