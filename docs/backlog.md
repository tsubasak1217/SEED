...
- [ ] **ロックの保存ゲート（強制）は未実装** — 2026-09-18。保持者が実名になったので、次の段階で
  「他の人がロック中のファイルは保存・送信させない」「シーンやアクターを開いたら自動ロック」を入れる。
- [ ] **ダイアログの配色・寸法が二重管理** — 2026-09-18。既存の `TextInputWindow` / `AudioSilenceTrimWindow` と新設の `AccountDialogTheme`。
