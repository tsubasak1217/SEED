...
- [ ] **Android の実行の一時停止が無い（段階D で IPC を TCP にするときに扱う）** — 2026-09-25（段階C-3 で利用者の要望を受けて記載）。
  PC の Play は実行ボタンで一時停止できるが、Android は実行中の実行ボタンが「一時停止の絵柄で無効」のまま。端末のランタイムとエディタをつなぐ
...
- [ ] **シーンマネージャに未登録のシーンは、開いていても Android では開始シーンで起動する** — 2026-09-25（段階C-3 の確認で判明）。
