...
- [ ] **開いているシーン自体を解決しても再読込されない** — 2026-09-19。確定でディスクの `.scene` は
  変わるが、エディタが開いているシーンはそのまま。`SceneAutoReloader` に繋ぐか、
  確定後に「開き直しますか？」を出す。現状は手で開き直す必要がある。
- [ ] **競合の一覧を作るたびにファイルを読んで差分まで取る** — 2026-09-19。「両方を取り込む」の
  可否判定のため、`ConflictRowItem` が未解決の競合 1 件につき 1 回読み、さらに
  **ブロックごとに LCS を 2 回**回す（可否が「両側とも挿入だけ」になったため。当初の
  「元が空」なら O(1) だった）。UI スレッドで走るので、競合が多い／ブロックが大きいと
  状態の取り直しが重くなる。判定を非同期にするか、ファイルの更新時刻で結果を覚える。
- [ ] **印の残存検査が `=======` 単体も弾く** — 2026-09-19。Markdown の見出し下線（`=` が 7 個ちょうど）を
  印と誤判定して「編集した内容で解決」が使えなくなる。JSON アセットでは起きないが、
...
- [ ] **実機での確認の残り** — 2026-09-19。使い捨て loreserver ＋ 競合状態の使い捨てプロジェクトを
  ...
  **未確認**: ブランチのマージ由来（見出しのブランチ名・両方の並び順の入れ替わり）、
  左マージンのチェックのクリック（ヒット判定）、上段の同期スクロールの体感、
  行のダブルクリック、狭い幅での 5 ボタンの収まり、押せない側（同じアクターの
  パラメータを 2 人が変えた等）のツールチップの見え方。
