...
- [ ] **HIT 演出の帯の角度を変えるにはクリップの作り直しが必要** — `hit_banner.anim` の位置キーは θ=−12° を展開した実座標。回転トラックだけ変えても位置は追従しない。関連: `runtime/assets/mainGame/animations/hit_banner.anim`、`scripts/HitBanner.cs`。
