...
- [ ] **サムネイル生成中、エディタのビューポートに被写体が映り込む** — 2026-09-12。
  撮影は隔離ワールド線へ切り替えて**提示中のカラーターゲットに描き、それを読み戻す**方式
  （図鑑サムネイルから引き継いだ仕組み）。そのためモデルの入ったフォルダを開くと、
  ユーザーのシーンではなく撮影中のモデルが 1 瞬ビューポートに出る。
  さらに連続生成の途切れを待つセッション（`SESSION_IDLE_SECONDS` = 2 秒）があるので、
  最後の 1 枚を撮り終えてから元のシーンに戻るまで最大 2 秒かかる。
  図鑑はユーザーが明示的に叩く一括処理なので許容されていたが、
  **プロジェクトパネルはフォルダを開くだけで起きる**ぶん目に付く。
  根治するには「専用のオフスクリーンターゲットへ描いて読み戻す（スワップチェーンを使わない）」
  必要があり、ID パス・カラー読み戻しの両方を別ターゲット向けに用意する作業になる。
  関連: `runtime/src/engine/core/app_base/app/thumbnail_ops.rs`、
  `runtime/src/engine/core/renderer/screenshot.rs`、`docs/editor_project_panel.md` §5。
