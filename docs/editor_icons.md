...
| 定義場所 | `editor/src/Resources/Icons.xaml`（**自動生成。手で編集しない**） |
| 生成器 | `editor/gen_icons.py` |
| 整合チェッカ | `editor/check_icons.py`（キー参照の突き合わせ） |
| 読み込み検証 | `editor/verify_icons.ps1`（WPF で実際に読めるかの確認） |

ラスタ PNG（旧 `editor/resources/icons/**`）は使わない。ベクターなので
高 DPI でも滲まず、色は Foreground の継承で切り替わる。
...
image.Source       = IconImages.Get(FileTypeIcons.GetIconKey(ext));
```
...
| ファイル拡張子 → キー | `editor/src/Controls/FileTypeIcons.cs` | プロジェクトパネル、新規作成ウィンドウ |
...
`FileTypeIcons` が唯一の対応表で、フォールバックは 2 段構え。

1. **未知の拡張子** → `Icon.File.Generic`（汎用ファイル）。
   `GetIconKey()` は表に無いキーで必ずこれを返すので、新しい拡張子が
   増えてもアイコンが欠けることはない。フォルダは `GetFolderIconKey(isEmpty)`
   で中身あり／空を出し分ける。
2. **サムネイルを持てる形式** → プレビューの**生成前・生成中・生成失敗**の
   すべての期間、形式アイコンを表示したままにする。
   `ProjectPanel.BuildFileItem()` が実装例で、
   - まず `FileTypeIcons.GetIconKey(ext)` の形式アイコンで `Image` を作り、
...
現在の拡張子対応（`FileTypeIcons.IconKeyByExtension`）:

| 拡張子 | アイコンキー |
|---|---|
| `.scene` | `Icon.File.Scene` |
| `.actor` / `.actor2d` | `Icon.File.Actor` / `Icon.File.Actor2D` |
...
| `.lua` / `.py` / `.rs` | `Icon.File.ScriptGeneric` |
| `.glb` / `.gltf` / `.obj` / `.fbx` | `Icon.File.Model` |
| `.png` `.jpg` `.jpeg` `.bmp` `.gif` `.tga` `.hdr` `.exr` `.webp` | `Icon.File.Image`（サムネイル対象） |
| `.wav` / `.ogg` / `.mp3` / `.flac` | `Icon.File.Audio` |
| `.json` | `Icon.File.Json` |
| `.toml` / `.yaml` / `.yml` / `.ini` / `.cfg` / `.lock` | `Icon.File.Config` |
| `.txt` / `.md` / `.log` | `Icon.File.Text` |
| 上記以外 | `Icon.File.Generic`（フォールバック） |
