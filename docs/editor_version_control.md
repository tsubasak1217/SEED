...
| `editor/src/Panels/VersionControl/ConflictRowItem.cs` | 競合節の行（ファイル名・フォルダ・アイコンだけ。ボタンは持たない） |
...
| `editor/src/Dialogs/ConflictSidePickerWindow.cs` | 並べて表示できない競合の 2 択モーダル（4.7.8） |
| `editor/src/VersionControl/Presentation/ConflictSidePick.cs` | その 2 択 → `ConflictResolutionChoice`（純関数・単体テスト済み） |
...
- **競合** … 常に最上部の独立した節。**0 件のときは節ごと隠す**（本当に競合したときの目立ち方を鈍らせない）。
  **行にボタンは置かない**（ファイル名が見えなくなるため。4.7.8）。開くのは
  ダブルクリックか Enter で、解決はマージエディタの中だけで行う。
...
- 上段 2 面は**ブロック単位で行数を揃えて**並べる（`MergeAlignedView`）。
  短い側には斜線の詰め物が入る。
- **上段 2 面の縦スクロールは連動させない**（2026-09-19 の指摘で撤去）。
  行が揃っているのでオフセットを写すだけで同期はできたが、**操作しづらかった**:
  片方を読みながらもう片方を別の位置で見たいときに、動かした側につられて飛んでしまう。
  行揃えは残してあるので、**「前の競合 / 次の競合」は両面とも**同じ行番号へ寄せる
  （＝並べ直したいときはこのボタンを押す）。
...
マージエディタのツールバーの「すべて両方」は押せない状態になる（ツールチップに理由）。
ただし**利用者が手でブロックごとに両方へチェックすることはできる**。
その場合だけ従来どおりの連結へ落ちる（元を残しようが無いため）。
壊れていれば 4.7.5 の検査が書き戻す前に止める。

マージエディタの既定チェック（この形のブロックは最初から両方を採る）と
ツールバーの「すべて両方」の有効条件は、**同じ `IsUnionable` を使う**。
別々の条件にすると「既定で入っているのにボタンは押せない」が起きる。
（パネル側の「両方を取り込む」ボタンは 4.7.8 のとおり廃止した。
`ResolveConflictsTakingBothAsync` は境界として残っている。）
...
**競合の節は「ファイルの一覧」だけ。行にボタンは無い**（2026-09-19 の指摘で全部外した）。
以前は 1 行に「比較… / 自分の変更を残す / リモートを採用 / 両方を取り込む /
編集した内容で解決」の 5 つを並べていたが、パネルは細いので**ボタンが幅を食い切り、
肝心のファイル名が 1 文字も見えなかった**。一覧の仕事を「何が競合しているか」に絞り、
どちらを残すかの選択は**マージエディタの中だけ**で行う。

| 場所 | 中身 |
|---|---|
| 見出しの下の帯 | 競合アイコン ＋ 薄い案内 1 行「ダブルクリックで開いて解決します。」 |
| 1 行 | `[競合アイコン][ファイル種別アイコン] ファイル名` ＋ そのうしろに薄い色でフォルダ |
| 行のツールチップ | フルパス（＋移動・改名なら移動前のパス）＋ 開き方の案内 |

- **ファイル名の列を `Auto`、フォルダの列を `*`** にしてある。幅が足りなくなると
  先に削られるのはフォルダ側で、細いパネルでも**ファイル名が必ず先に見える**。
- 開く手段は **行のダブルクリック**と **Enter** の 2 つだけ
  （一覧はキーボードでも辿れるので、開く手段もキーボードに用意する）。
- 行の生成は**文字列操作だけ**。以前は「両方を取り込む」の可否を決めるため
  1 件ずつファイルを読んでブロックごとに LCS まで回していたが、そのボタンが無くなった。

#### 開けないファイル（バイナリ・印が無い）の受け皿

ボタンを外した以上、`.png` や `.blend` の競合を解決する手段が無くなってはいけない。
`MergeEditorWindows.TryOpen` が失敗したら、そのまま理由を出して終わるのではなく
**小さな 2 択ダイアログ**（`Dialogs/ConflictSidePickerWindow`）を出す。

| 要素 | 中身 |
|---|---|
| 本文 | 「このファイルは中身を並べて表示できません（`理由`）。どちらの内容を残しますか？」＋ パス |
| ボタン | 「`現在の呼び名` を残す」（主操作）／「`取り込み元の呼び名` を採用」／「キャンセル」 |
| 呼び名 | `MergeEditorWindows.CurrentName` / `IncomingName`（sync なら「自分の変更／リモート」、ブランチのマージならブランチ名） |

対応は `Presentation/ConflictSidePick.cs` の純関数 1 か所:
**「現在を残す」= `KeepMine`／「取り込み元を採用」= `TakeRemote`**。
そこから先の mine / theirs の入れ替わり（4.1）は `LoreConflictResolutionMap` が吸収するので、
ダイアログ側は出どころを知らない。この 2 段の突き合わせは
`PureLogicTests.PickReachesIntendedSide` が sync / ブランチのマージの両方で固定している。
「取り込み元を採用」は自分側の内容が消えるので、既存の確認（`ConfirmTakeRemote`）を通す。
ヘッドレスでは `EditorDialogs` がログへ流して「キャンセル」を返す（勝手に片側を捨てない）。

#### 残してあるプロバイダ境界

`ResolveConflictsAsync`（2 択）／ `ResolveConflictsTakingBothAsync`（両方を取り込む）／
`ResolveConflictsWithContentAsync`（中身を指定）は**そのまま残っている**。
パネルのボタンは無くなったが、マージエディタの確定・上のダイアログ・単体テストが使う。
`ResolveConflictsTakingBothAsync` は 1 件でも弾かれたら**何も書き込まない**
（半端に書き換わった作業コピーを作らない）。
...
★この記録のうち『比較…』は**同日に廃止した**（4.7.8）。いまマージエディタを開く手順は
**競合の節の行をダブルクリック（または選んで Enter）**に読み替えること。
