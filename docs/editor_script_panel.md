...
型名・メソッド名・フィールド名・ローカル変数・**属性名**などの色は、正規表現ハイライトの上へ
Roslyn の分類結果を重ねて付けている（`SemanticColorizer`）。重ねる範囲は文字数で決まる。

| 文字数 | 方式 |
|---|---|
| `SemanticFullMaxTextLength`（400,000）以下 | **全文**を分類する。スクロールしても色は抜けない |
| それを超える | **窓方式**: 見えている範囲 ± `SemanticWindowMarginLines`（400 行）だけを分類し、スクロールで窓の端（残り 100 行）へ近づいたらデバウンス（120 ms）つきで分類し直す |

**着色を無効にする上限は無い。** 以前は 200,000 文字を超えると着色そのものを無効化しており、
大きなスクリプト（`FishingController.cs` = 219,457 文字）だけ、属性が正規表現の「メソッド呼び出し」色
（`SerializeField(` が `名前(` の形に見えるため）のまま、型・メソッド・フィールドも無着色になっていた
（2026-09-20 の報告「Attribute の色がスクリプトによって揺れている」）。
実測（同ファイル・5,994 行・プロジェクト 86 ファイル）: 編集後の全文分類 430〜600 ms
（バックグラウンド・UI は止まらない）、600 行の窓なら 60〜110 ms、分類スパン約 33,000 件
（行の描画は二分探索なので件数に比例しない）。
選択単語の強調（`OccurrenceMaxTextLength`）も同じ理由で 200,000 → 1,000,000 へ引き上げた。
