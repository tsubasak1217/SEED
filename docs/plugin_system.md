...
- `game_tools`（このゲーム専用）: `<project>/plugins/GameTools/{game_tools.dll, plugin.json}`
...
有効化は `<project>/assets/project_settings.json` の `plugins` 配列:
...
- キー定義: `<project>/assets/common/scripts/GameProgressKeys.cs` の `TutorialDone = "tutorial_done"`
- 読み出し: `<project>/assets/mainGame/scripts/Tutorial/TutorialDirector.cs`
