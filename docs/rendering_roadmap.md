...
- WBOIT 方式（**色付き透過率＋屈折歪み / 曲げと色フィルタの分離**）: accum=Rgba16Float
  （One/One 加算, クリア0, premult×w を積む＝**生の曲がった背景 bent_bg + 自色**。tint 非乗算）＋
  reveal=**Rgba16Float**（(Dst, Zero)＝dst*=src の乗算累積, クリア1, Π T_frag を rgb・Π(1-a) を a）の
  2RT（RtPool `wboit_accum`/`wboit_reveal`）。デュアル MRT パイプラインは TOML ビルダー非対応のため
  手動構築（ソート用ビルドのリフレクション BGL を再利用、グループ数 5 維持）。深度はメインパス
  深度を Load・書込なしで不透明に隠される。
  - **色付き透過率の狙い**: 同色の半透明が N 枚重なると足し算的に明るくなる問題（赤カーテンの
    シワ）を解消する。各フラグメントは per-channel 色フィルタ
    `T_frag = clamp((1-a) + a·tr·albedo, 0, 1)`（色付き影・refract_layer_tint と同セマンティクス）を
    reveal.rgb へ**乗算累積**する（乗算は可換＝順序不問）。純赤 T=(1,0,0) は T^N=T で飽和し「1赤+1赤≈1赤」。
  - **屈折歪みは保つ**（実機フィードバック反映）: 背景を落とすとガラスの存在感が消え平坦化したため、
    フラグメントは `glass_composite` と同じ屈折幾何で「曲がった背景」を焼き込む（RT 版は TLAS 屈折レイ、
    SS 版は grab-pass）。ただし WBOIT 経路では**ガラス色 tint を premult に掛けない**（色フィルタは reveal
    の ΠT が持つ）。式は refract_common.wgsl の `glass_composite_wboit`。
  - **合成は「曲げ」と「色フィルタ」を分離**: `C = 1 − Π(1-a)`（reveal.a）＝スカラーのカバレッジ、
    `avg = accum.rgb/max(accum.a,ε)`（生の曲がった背景+自色）として
    `final = lerp(scene, avg, C) × ΠT`（`bent = scene·(1-C)+avg·C` を per-channel 色フィルタで着色）。
    **曲げをスカラー C（色に依らない幾何）で行い、色フィルタ ΠT を bent 全体へ掛ける**のが要点。
    これで前案（チャンネルごとの凸結合 `scene_c·ΠT_c+avg_c·(1-ΠT_c)`）の欠陥＝透過の高いチャンネルで
    avg 重み (1-ΠT)≈0 となり scene（直進背景）が出て「よく通す色ほど歪まない」矛盾を解消し、
    シアンガラスの G/B でも曲がった背景が見える。N 枚で ΠT 飽和・avg 重み平均で足し算的増強も無し。
  - **自色（スペキュラ等）は avg に束ね ×ΠT が掛かる**（自色専用 3 枚目 MRT を避ける判断。ガラス色で
    わずかに色付くがスペキュラは微小で許容）。**半透明 a<1 のフェード端は C と ΠT の双方に (1-a) が効き
    軽微に過減衰**（scene は (1-a)²）するが有界・単調で、屈折歪みを正しく出すためのトレードオフとして許容。
  - **黒ガラス回帰の根治（RT 屈折ミス時フォールバック）**: 高屈折（ior=2.5）ガラス 2 枚の重なりでは、
    手前ガラスの屈折レイが 2 枚目を貫いて大きく曲がり、最終不透明トレースがミス／画面外射影になりやすい。
    従来ここで DDGI/アンビエント（暗所ほぼ 0）を返していたため、bg≈0 → premult≈自色（暗）→ 手前が深度重みで
    avg を支配 → 合成 C=1 で scene も消え **ピュアブラックの塊**になっていた（WBOIT・半透明=RT の実機症状）。
    修正: `refract_rt.wgsl` の不透明ミス／画面外フォールバックを **フラグメント直進背景**
    （`refract_bg_sample(frag_xy/res, roughness)`）へ変更（SS 屈折の穏当な縮退と同方針）。屈折レイが解けない
    領域でも「素通しの背景」が見え、色フィルタ ΠT が乗って `背景×フィルタ` に収まる。距離ソート RT も同経路で
    改善（挙動変更: RT 屈折レイのミスが暗アンビエントではなく直進背景になる）。
  - **合成は 2 パス**（合成先シーン HDR は dst でありサンプル不可＝1 パス不可、dual-source feature も未要求）:
    パス1 背景の曲げ濾過（blend=WboitBgMultiply, `dst = scene·(1-C)·ΠT`。(1-C)=reveal.a なので
    src.rgb=reveal.a·reveal.rgb）→ パス2 曲がった透明色の加算（blend=Additive, `dst += avg·C·ΠT`）。
    合計 = `[scene·(1-C)+avg·C]·ΠT`。同一 HDR（LoadOp::Load, **ブルーム前＝トーンマップ前**）へ
    順に描く（パス1→パス2 順序必須）。パス1 は reveal.a≈1、パス2 は accum.a≈0 のピクセルを discard。
...
さらに **水面反射の粗さぼかし（W5.2）へ転用**し `.rgb`→**`.rgba`** に広げた。水面反射 RT は
「rgb = 反射色 × 強度／a = 強度」の**プリマルチプライド**で、読み手が `rgb / max(a, ε)` と割り戻すことで
**α 重み付き平均**になり、反射のない画素（クリア値 0）を巻き込んだ黒い縁取りが構造的に出ない。
α まで平均する必要があるのはこのため。**既存 3 用途（AO=`.r` / SSGI=`.rgb` / 屈折ピラミッド=`.rgb`）の結果はビット単位で不変**
...
- ~~Hi-Zオクルージョンの接続~~ → **Phase HZ で配線完了（opt-in・下記参照）**。
...
- **拡散透過（diffuse transmission）〔実装済み・2026-07-20 時点で一時無効化中〕**: パラメータ数の割に制御が
  ...
  - **半透明遮蔽物の色付き遮蔽対応済み**（本コミット。戻り値を f32→**vec3** へ拡張）: 上の厚み減衰は不透明（0x01）
    ...
    vec3(1) へ同期。
  - **一枚シート（片面クロス）を裏面から遮蔽する対応済み**（追補コミット。`rt_trace_translucent_tint` に
    `pair_back: bool` 引数を追加）: 上記の色付き影は「閉じたガラスの入射/出射での二重色付け防止」のため **front-face
    ヒットのみ tint を累積**する規則だった。ところが拡散透過の遮蔽レイは「赤カーテン裏側の点→光源」方向で、**表側に
    掛かった青カーテン（片面クロス）に背面から当たる**ため、全ヒットが back-face になり front-only 規則で tint が
    スキップ＝素通しになっていた（ユーザー報告: 青カーテンの重なり領域が裏から見て明るいまま）。是正として拡散透過の
    トレース**だけ** `pair_back=true` を渡し、t 順ループに「直前 front を通過した半透明 instance を 1 組」保持する
    最小ペアリングを追加: front ヒットは常に tint＋その instance を記録、back ヒットは直前 front と**同一 instance**なら
    閉メッシュ出射面としてスキップ（二重計上回避）、対の無い back は**裏から当たった一枚シート**として tint 適用。
    色付き影の既存 4 呼び出し点は `pair_back=false`（front-only 不変）で挙動を保つ。avg 版・bindless 版の両方へ同規則を
    実装し、bindless の Mask 分岐は従来どおり表裏問わず α テスト（`pair_back` 非依存）。多重入れ子の完全追跡はしない
    （front は 1 組のみ保持。カーテン/ガラス板の実用範囲では十分）。naga 連結検証（deferred rt_on/bindless・forward RT
    透明・shadow_mask avg/bindless の 6 連結）と `cargo test`（183）で全通過。
...
- **B4 相当（2026-08-04 実装）**: **水面反射（W5.2）の画面外ヒット**も B2 と同じ方式へ移行。
  W5.2 実装時は「binding_array と uniform は同一 bind group に同居できない」制約で見送られ、
  画面外の物体が**一様な灰色の塊**として水面に映っていた（スキン・非スキンを問わず発生）。
  group3 の唯一の uniform だった `wr_sky` を storage へ変えて uniform ゼロにし、
  そこへバインドレス資源 5 本を同居させて解決した（グループ数は 5 のまま）。
  併せて `RenderPipelineBuilder::with_binding_array_count()` を新設し、
...
### マテリアル情報の紐付け（平均アルベド／バインドレスレコード）

スキンインスタンスは **非スキンとまったく同じ流儀** でマテリアル情報を持つ
（`rt_shadow.rs` の 4.5「スキンインスタンスを同じスロット列へ続けて詰める」）:

- `inst_count` を非スキンと**共有**するため、`TlasInstance::new(..., inst_count, mask)` の
  `custom_data` は全インスタンスで一意。`albedos[inst_count]` / `records[inst_count]` は
  同じ添字で埋まる＝**インデックス空間の衝突は構造的に起こらない**。
- 平均アルベド（`primitive_avg_albedo`＝テクスチャ平均 × `base_color_factor`）と、
  色付き影用の `.a` パック（α × transmission）も非スキンと同一式で詰める。
  よって `albedo_buffer` の全読み手（DDGI のバウンス色／RT 反射・水面反射の画面外ヒット／
  RT 色付き影／RT 屈折の着色）にスキン分がそのまま効く。
- `BindlessInstanceRecord` も同様に詰める。**唯一の違いは `BINDLESS_FLAG_GEOM` を立てないこと**
  （理由は下記「既知の制限」の法線の項）。UV とインデックスは変形で不変なので
  `ELIGIBLE`（＝ヒット点のベースカラーテクスチャ実サンプル）は有効であり、
  D6 の RT 反射・**水面反射のバインドレス変種**（2026-08-04 追加）はスキンでも実テクスチャ色で映る。
- スキン BLAS のインデックスバッファは**元プリミティブのものをそのまま**使うため、
  ヒットの `primitive_index` はメガバッファへ登録した index 列と一致する
  （＝UV 復元がスキンでも正しく引ける）。

> **注意（誤診しやすい点）**: 「スキンだけが反射で一様な灰色になる」症状が出た場合、
> ここの紐付けを疑う前に**画面外ヒットのアルベド解決がバインドレス変種か**を確認すること。
> 平均色経路ではテクスチャ平均が灰色に寄るため、**非スキンモデルでも同じく灰色の塊になる**
> （2026-08-04 の実例。原因は水面反射の平均色近似で、スキン側の紐付けは正しかった）。

### BLAS のインスタンス単位統合（2026-08 修正）

**症状**: `CameraTestScene` に BrainStem（1 メッシュ / 59 プリミティブ）を 20 体並べると、
オリジナル 1 体だけが目に見えて暗くなる。

**原因**: スキン BLAS のエントリ単位が `(batch_key, mesh, prim, inst)` の
**プリミティブ×インスタンス**だった。列挙はインスタンス major なので、20×59 = 1180 件が
上限 64 を **体の途中で**使い切る（inst0 が 59 件、inst1 が 5 件、inst2 以降は全滅）。
結果 TLAS にはオリジナルしか載らず、RTAO と RT 影の自己遮蔽がその 1 体にだけ乗っていた。

**修正**: **1 スキンメッシュノードインスタンス = 1 BLAS** に統合した。
`BlasGeometries::TriangleGeometries` は 1 BLAS に複数ジオメトリを持てるので、
そのノードの全プリミティブをジオメトリ配列として 1 本の BLAS へ入れる。

- キー: `(batch_key, node_idx, inst_idx, class_id)`。
  - **ノード**単位であって「メッシュ」単位ではない。1 BLAS は TLAS インスタンス変換を
    1 個しか持てないため、同じ mesh を参照する別ノード（別ワールド変換）は混ぜられない。
  - `class_id` は **TLAS インスタンスマスク**（不透明 0x01 / 非不透明 0x02）。マスクは
    TLAS インスタンス単位の属性なので、混ぜると半透明部位が不透明の影を落とす退行になる。
    したがって 1 体は最大 2 エントリへ割れる（枠は体単位で原子的に確保する）。
- 変形出力バッファは従来どおり **プリミティブごと**に分けたまま、ジオメトリごとに
  別の `vertex_buffer` として参照する（1 本へコピー統合はしない＝追加のコピーコストなし）。
- ポーズ署名の `prim_generation` は、エントリを構成する全プリミティブの
  （mesh/prim/世代/頂点数/インデックス数）を畳み込んだ **`layout_sig`** に置き換えた。
  構成が変われば必ずエントリ再生成＋BLAS 再構築へ倒れる。
- 効果: BLAS 本数が 59×N → N（＋マスク分割ぶん）に減り、VRAM とビルド時間も改善する。

### マテリアルレコードの索引規約（`custom_data + geometry_index`）

統合により **1 TLAS インスタンスが複数マテリアルを持つ**ようになったため、
`instance_custom_data` とマテリアルレコードの 1:1 対応は成り立たなくなった。新しい規約:

```
record = hit.instance_custom_data + hit.geometry_index
```

- TLAS の `custom_data` には「そのインスタンスの **先頭レコード番号**」を入れる。
- レコード（平均アルベド storage ＝ `gi_albedo` / `rt_albedo`、および bindless
  `instance_table`）は **ジオメトリ順に連続配置**する。
- **非スキンも同じ式**で引く（1 インスタンス = 1 ジオメトリなので `geometry_index` は常に 0 で、
  従来と同一の値になる）。経路ごとに意味を変えないので判別フラグは不要。
- 容量は `MAX_RT_RECORDS`（= `MAX_RT_INSTANCES` = 4096）。TLAS スロットとレコードの
  **両方**に空きがある場合のみ登録し、スキンは「1 インスタンス＋n ジオメトリ」がまとめて
  入らなければ 1 件も登録しない（ジオメトリとレコードの対応が途中で切れないため）。
- `hit.primitive_index` は **ジオメトリ内**の三角形番号なので、`rec.index_offset` との
  組み合わせは統合後も従来どおり正しい。
- 読み手（全部この式に更新済み・`wgsl_reads_record_index_with_geometry_index` が
  ソースレベルで縛る）: `ddgi_probe_update.wgsl` / `reflection_rt.wgsl` /
  `refract_rt.wgsl` / `rt_shadow_tint_avg.wgsl` / `rt_shadow_tint_bindless.wgsl` /
  `water_reflection_rt.wgsl`。
...
`(batch_key, node, inst, class)` なので別プロップの別インスタンスが同一キーへ衝突し、
...
- **毎フレーム BLAS フル再構築**。`AccelerationStructureUpdateMode::PreferUpdate`（refit）は使っていない。
  ...
  実測でボトルネックになったら再検討する。
- **上限 `MAX_RT_SKIN_BLAS = 64`**。単位は **1 体（1 メッシュノードインスタンス）= 1 件**
  （後述の「BLAS のインスタンス単位統合」以降。プリミティブ数には依存しない）。判定は
  ...
  枠の確保は **体単位で原子的**（`can_accept_group`）なので、上限到達時に「体の途中で切れる」
  部分受理は起こらない。
...
**地形チャンク**（`terrain://` の統合バッチ）に AABB→Hi-Z オクルージョンテストを追加した。既存のフラスタム／距離
カリング（827bfa9）に **AND** で重ねる（フラスタム内でも山の裏なら描かない）。

- **有効化**: 環境変数 `SEED_OCCLUSION_CULL=1`（**既定 OFF・opt-in**）。未設定なら `HiZSystem` は構築すらされず
  オーバーヘッド 0。誤棄却をユーザーが強く嫌うため、まず opt-in で実機検証する方針。
- **方式（AABB→Hi-Z, 1 フレーム遅延・GPU→CPU 読み戻し）**:
  1. G-Buffer 深度が確定した直後（deferred のみ）に `build_pyramid`（深度→R32F Hi-Z ミップ, 2×2 MAX＝保守）。
  2. 各地形チャンクの world AABB を `dispatch_occlusion` で投影・比較（`result_buf` に 0/1 を書く）。
  3. `schedule_readback`→submit 後 `map_after_submit`→**次フレーム** `try_read_results` で受領（1 フレーム遅延）。
  4. 受領した可視性を `hiz_prev_keys`（ディスパッチ時の path 列）と同順で対応づけ、遮蔽 path を `terrain_culled` へ union。
- **保守性（誤棄却／消え残り防止）**: シェーダーは「AABB が少しでも手前／カメラ背後／画面外なら必ず可視」。
  完全遮蔽（AABB 最近接 z > オクルーダ最大深度）だけ落とす。結果が無い／未対応チャンクは常に描く。
  遮蔽が解けた領域は背景深度=遠になり即可視（1 フレームで自己修復）。CPU ミラーの単体テスト 7 本で固定。
- **影は不変**: `terrain_culled` はメインカメラのパスのみ（G-Buffer 3854 / フォワード 4427）。影パスはライト視錐台で
  独立に地形を描くため、オクルージョン棄却で影が欠けることは無い（既存フラスタムカリングと同じ安全性を継承）。
- **効果範囲**: ラスタ（G-Buffer 描画）のドロー/三角形削減のみ。RT 影／RT-GI 等のフルスクリーンパスは別コストで減らない。
  起伏の多い地形で「大きな遮蔽物が奥のチャンクの画面矩形を丸ごと覆う」構図で効く。
- **実機（RTX 3060 / Vulkan, SEED_TERRAIN_SMOKE）**: build→dispatch→GPU→CPU 読み戻しの全経路が毎フレーム動作
  （`[PERF hiz] dispatch: instances=24` / `occlusion read: occluded=N tested_prev=24`）。スモーク地形（24 の粗い
  ハイトフィールドチャンク＋~10m の丘）では **どの視点でも occluded=0**＝チャンク単位の完全遮蔽が発生しない
  （AABB 粒度では隣接チャンクが互いを丸ごと隠さない）。結果、オクルージョン ON/OFF で **スクリーンショットは
  バイト一致（誤棄却ゼロ）**。drawn 数の実削減は「大きな山が奥チャンクを完全に覆う」実プロジェクト規模シーンでの
  確認が必要（スモークの粗いチャンクでは再現できない）。
- **残課題（申し送り）**:
  - GPU 駆動 indirect 化で CPU 読み戻し（`try_read_results` の poll）を撤去し、同フレーム 2 パス化して 1 フレーム
    遅延の reveal ホールも消す（本命の最適化）。
  - 散布（草・散布モデル）への適用: 現状は地形チャンクのみ。散布はバッチ統合時に可視インスタンスだけを載せる
    設計のため、チャンクスパン単位のオクルージョンは `cull_and_update_scatter_models` への統合が要り、静的地形の
