...
## フェーズ RT-Translucency 近似改善（半透明表面反射／半透明同士の屈折／透過0の裏面カリング） 【状況: 実装済み（実機検証待ち）】

2026-07 追加。RT-Translucency／ガラス表現の土台上に、ガラス面の見えを底上げする 3 つの近似を入れた。
いずれも**既にバインド済みの資源だけ**を使い（新グループ増設なし・`max_bind_groups=5` 厳守）、
距離ソート `fs_transparent_sorted` と WBOIT `fs_wboit` が共有する `refract_common.wgsl::glass_composite`
の 1 か所で効かせる（片方漏れを防ぐ）。

### A. 半透明表面の反射（ミニ SSR ＋ プローブ／アンビエント フォールバック）
- 課題: 不透明用の反射（SSR/RT, Phase D6）は G-Buffer（不透明のみ）依存でガラス面に景色が映らない。
- 実装（`refract_common.wgsl::glass_reflection`。ガラス＝material_refracts のフラグメントで、反射ビット時のみ）:
  1. `reflect(-V, N)` の反射方向をワールドで**深度レス**に短くマーチ（`MINI_SSR_STEPS=10`, `MINI_SSR_MAX_DIST=20.0`）。
     各ステップを画面へ射影し、**画面内に留まる最遠点をヒット**とみなす。深度が透明パスにバインド
     されていない（メイン深度は同パスのアタッチメントで読めない）ため、遮蔽テスト付きの厳密 SSR は行わない。
  2. ヒット → **屈折背景ピラミッド**（binding15/16, ミップ付き）を roughness でミップ選択してサンプル（＝ぼけた反射）。
  3. ミス／画面外／SSR 無効 → **DDGI プローブ照度**（binding10-13, 反射方向）→ GI 無効なら**環境光**
     （`ambient_color*ambient_intensity`。真っ黒にしない）。
- **フォールバック階層**: ミニ SSR（refract_bit＝屈折ピラミッド存在時のみ）→ DDGI（GI 有効時）→ アンビエント。
- 重み: **フレネル（誘電体 F0）× 粗さフェード（0.30→0.55 で消える, reflection_common と同値）× reflection_intensity**。
- ゲート: `ReflectionMode!=Off` で `LightMeta.translucency_rt` の **bit2（`TRANSLUCENCY_RT_REFLECTION=4`）** を立てる
  （屈折 bit1・色付き影 bit0 と同じビットマスク。frame_renderer が反射モード確定後に write_buffer で追記）。
  `reflection_intensity` は `LightMeta` を **32→48 バイトへ拡張**して offset32 に載せた（layout_tests 更新）。
  `ReflectionMode=Off` は bit2＝0＋intensity=0 で**完全スキップ**（ガラス面でも反射項が乗らない）。
- **限界（実機で要確認）**: 遮蔽テストが無いため、実際には隠れている景色が反射に映る**ゴースト反射**が出うる。
  深度レス近似のためステップ数を増やしても精度は上がらない（射影可能な最遠点を選ぶだけ）。深いミップ（すりガラス）は
  refract_background と同じチェーンを共有。

### B. 半透明同士の屈折（距離ソート限定の逐次グラブ）
- 課題: 屈折背景は「不透明のみのコピー 1 回」で、手前のガラス越しに奥のガラスが見えない。
- 実装（`transparency.rs::plan_sorted` / `regrab_mip0` ＋ `frame_renderer` のパス分割）:
  1. 距離ソートは奥→手前順に描くため、**屈折するオブジェクト（`material_refracts` = ior>1 || transmission>0）を
     描く直前に scene_hdr → refract mip0 を再コピー**する。手前のガラスが「既に描いた奥のガラス」を屈折で拾える。
  2. コスト上限: 再グラブは 1 フレーム最大 **`REFRACT_REGRAB_MAX=4` 回**・**手前（カメラに近い）優先**
     （`select_regrab_indices` が back→front 描画順の末尾から屈折アイテムを 4 件選ぶ。超過分は初回グラブのまま）。
  3. **mip0 のみ更新**（すりガラスの深いミップは初回のまま妥協。粗い屈折は細部が見えず 1 フレーム内の差は知覚困難。
     深いミップの再生成＝ダウンサンプル＋ブラーを毎回回すとコストが跳ねるため）。
  4. **距離ソートモード限定**（WBOIT は順序が無いため対象外）。
  5. パス分割: scene_hdr が現レンダーターゲットのため、再グラブ前にメインパスを閉じる必要がある。`frame_renderer` が
     regrab_before 境界でメインパスを `drop → copy_texture_to_texture → begin_scene_pass_load_to`（深度/HDR を Load・
     Play ビューポート/シザー再適用）で分割し、最終セグメントを開いたまま後続描画（グリッド/ギズモ）へ引き継ぐ。
     再グラブ不要なフレーム（refract_active 偽 or 屈折アイテムなし）は**従来の単一パス**（分割ゼロ＝コスト増なし）。
- CPU 判定に `GpuModel.iors`（＋`primitive_ior()`）を transmissions と同順で追加（override 追従も同様に）。
- **限界**: 深いミップのすりガラスは 1 フレーム内で奥ガラスの更新が反映されない（意図的な妥協）。

### C. 透過0の動的裏面カリング（シェーダ discard）
- 実装（`glass_composite` 冒頭）: `if !front_facing && u_material.transmission <= 0.0 { discard; }`。
  透過率 0（＝光を透過しない実質不透明な Blend）のガラスの**自分の裏面**は原理的に前面に隠れて見えないため破棄する。
  「透過しないガラスの裏面が自分越しに透けて見える」不具合が消える。透過率>0 のガラスは両面を残す（厚み感維持）。
- マテリアルの `cull_face=None`（両面）より優先されるのは**意図的**（透過 0 なら裏面は見えないという物理的根拠）。
- **限界**: 単層両面 Blend で透過 0 のもの（例: 透過しないカーテン/デカール）は裏面が消えて片面化する。
  透過するファブリック（`transmission>0`）は対象外で安全。

### 検証（実装時点）
- `cargo build` 0 エラー / `cargo test` **150 passed**（+2: `select_regrab_indices_picks_nearest_refractors`・
  `regrab_constants_are_sane`。既存の grab-pass ゲート／屈折オフ パリティ／naga 全バリアント検証は維持）。
  `cd editor && dotnet build` 0 エラー（UI 変更なし）。
- naga 検証: `transparency_shaders_parse_and_validate`（sorted/wboit mesh/skinned）が反射（`glass_reflection`）・
  discard・48B LightMeta を含む連結を parse+validate。deferred/rt 系の LightMeta 参照も 48B で再検証済み。
- **未検証（実機 GPU 無し）**: (A) ガラスに景色の映り込みが出る／ゴースト反射の程度、(B) 手前のガラス越しに
  奥のガラスが見える／パス分割の実行時整合（深度 Load・ビューポート再適用・pass 再代入）、
  (C) 透過 0 で裏面が消える／両面カーテンの片面化。実機スモークで確認すること。
