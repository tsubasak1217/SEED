...
- **I2: 水の波紋・航跡** — ✅ **実装済み（2026-07-28）**
  ...
  **I2 で確定した設計判断**
  ...
  - **伝播バッファは増やさない（チャンネル同居）**。波動方程式の陽解法
    `h_(t+1) = h_t + (h_t − h_(t−1))·inertia + k·∇²h_t`（全体に減衰を掛ける）は
  ...
  - **時間刻みは CFL 飽和で固定化**。伝播係数 `k = (c·dt/dx)²` を CPU 側で
    2 次元の安定条件 1/2（`INTERACTION_WAVE_MAX_COURANT_SQ`）に飽和させる。
    通常のフレームレート域（約 42fps 以上）では波速が実時間に一致し、それを下回ると
    波の広がりだけが遅くなる（**発散して水面が壊れるより良い**という判断）。
    さらに慣性項へ `inertia = 今 dt / 前 dt`（上限 2 倍）を掛けて可変フレームレートを
    正規化する。これが無いと dt が急変したフレームで波にエネルギーが出入りし、
    dt=0 のフレームでは `2h−h_prev` が線形に発散する。
