...
- **W5: 水の最終形（北極星: Horizon Forbidden West 級）** — **W5.1 / W5.3 実装済み（2026-08-03）／残りは未着手**。
  ...
  | **W5.2 反射の本命化** | 現状は「浅い角度で単色へ寄せる」簡易フレネル（`reflection_color` 固定色。シーン内オブジェクトは映らない）。**方式は合意済み（2026-08）**: D6 で実装済みの**ハイブリッド RT 反射**（`reflection_rt.wgsl` — レイを TLAS へ飛ばし、ヒット点が画面内なら scene_hdr を採用＝SSR 品質、画面外・遮蔽裏は RT シェーディング、ミス時 GI プローブ）**をそのまま水面へ流用する**。実装形態は、水面シェーダーへの直接組み込みではなく**水面用の独立反射パス**（バインドグループ上限 5 を水面パイプラインが既に圧迫しているため）: water_id パスの位置・波法線を入力に水面反射 RT へ書き、水面シェーダーはそれを 1 枚サンプルする（不透明版と同じ流れ）。反射強度・波による反射のぼけ（粗さ）は WaterVolume のパラメータにする。**実装時に既存の「反射の色」（`reflection_color`）パラメータは削除してよい（2026-08-03 ユーザー指示。インスペクタ UI・シリアライズ・シェーダーの全経路から撤去し、serde は旧フィールド無視で後方互換を保つ）** | **D6（実装済み・流用）** / water_id |
  ...
  **順序の考え方**: W5.2 は D フェーズ（Deferred+Clustered）に従属するので**単独で先行させない**。
  既に持っている高さ場を使い回せるものから積む方針で、**W5.1 → W5.3 まで完了**。
  残りは W5.2（D6 の RT 反射流用）→ W5.5 → W5.4 の順。
  ...
  **W7.2: 水中から見上げても外の景色が見えない**
  ...
  - **修正**: カメラがそのフラグメントの水面高さより下なら（`u_camera.position.y < in.world_pos.y`）
    ...
    - **法線を反転**して視線側へ向け、フレネルをその法線で評価する。水中では浅い角度ほど
      `reflection_color` へ寄る＝**全反射の粗い代用**になる（真上は外が見え、水平方向ほど鏡になる）。
...
W0 → W1 → I1 → **I2（済）** → **W1.5（済）** → **W4（済）** → **W5.1（済）** → **W5.3（済）** → **W2.5（済）** → **W5.2** → I3
