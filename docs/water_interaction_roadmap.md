...
- 永続変形をシーン保存に含めるかは I3 実装時の設計判断として保留
  → **I3.1 で決着: 保存する**。地形チャンク紐づけの `.tcover` として `.tvox` / `.tscatter` の
  隣に置く（`docs/cover_field.md` §3.4）。ただし **Edit で作った状態だけ**が保存対象で、
  Play 中の積算は揮発する（Stop で保存状態へ戻る）。
...
- **W5.2: 反射の本命化** — ✅ **実装済み（2026-08-04）**
  ...
  **既知の制限**
  ...
  - **deferred 有効時のみ動作する**（既定 ON なので通常は常時有効）。フォワード描画では
    反射 RT が確保されない。W5.2 以前は固定色が出ていたので、フォワードでは**見た目が変わる**点に注意。
    **2026-09-26 から、反射パスが走らないフレーム（フォワード・描画品質の `water_reflection=false`。
    Android の既定 `mobile` がこれ）は「空の映り込み」のフォールバックを使う**: CPU が代表スカイボックス
    （反射パスのミス経路と同じ `reflection_sky_source`）を水面パスの group1 binding8〜10 に挿し、
    `water_surface.wgsl` が波法線で反射した方向の空を天球から 1 回サンプルして反射像の代わりにする
    （強度は水域の `reflection_intensity`、合成は同じフレネル。物体の映り込み・粗さのぼけ・水中の内部反射は無い）。
    反射パスが走るフレームは無効値が挿さって分岐に入らないので、デファードの見た目は画素単位で従来どおり。
    判断は `renderer/water/sky_fallback.rs`（`fallback_source`）。以前は平坦な深場の色一色になっていた
    （[android.md](android.md) §22.9）。
...
- **W5.3: 水中コースティクス** — ✅ **実装済み（2026-08-03）**
  ...
  **パラメータ（既定値）**
  ...
  | `shadow_refraction_strength` | `0.0`（**既定オフ**。旧既定 1.0） | **水中に落ちる影を水面の屈折に合わせて揺らがせる誇張倍率**。1.0 = 物理どおり／**0 で完全無効（従来と 1 ビットも変わらない影）**。物理的なずれは水深 2m・勾配 0.05 で 2〜3cm と目に見えないため、演出としては値を上げて使う（クランプ上限 5cm を超えるずれは出ない） |
  ...
  **影の屈折オフセットの水中判定規則（追補。Play 中のチカチカの真因対策）**

  症状: Play 中だけキャラクタの影がチカチカする。真因は**この影屈折オフセットが陸上のキャラクタに
  掛かっていた**こと。オフセットは `u_camera.time` 駆動なので毎フレーム動くが、旧実装の水中判定は
  「水域クアッドの XZ 内かつ水面 Y より下」だけだったため、**Ocean（カメラ追従の巨大クアッド）の
  範囲内で海面 Y より低い陸地**に立つキャラクタまで水中扱いになり、その影のレイ原点が毎フレーム
  最大 ±2m（旧 `CAUSTICS_SHADOW_OFFSET_MAX_M`）振り回されていた。既定値が 1.0（有効）だったため
  何も設定していないシーンでも発火する。対策は次の 3 点:

  1. **既定を 0（オプトイン）へ**。明示的に > 0 を設定した水域だけ揺らぐ。
     既存シーンで値を書いていない水域は影の揺らぎがオフになる（**意図的な挙動変更**）。
  2. **クランプ上限 2.0m → 0.05m**。実屈折ずれ（2〜3cm）の 2 倍弱を逃げ幅とする。
     誤判定が残っても影のずれは知覚できない大きさに収まる。
  3. **水中判定の厳密化**。判定規則は次のとおり（`shaders/caustics.wgsl`）:

  | 段 | 規則 | 効く対象 |
  |---|---|---|
  | (a) | `caustics_intensity > 0` かつ 水域クアッドの XZ 矩形内 かつ 水面 Y より下 | 光・影オフセット両方（従来どおり） |
  | (b) | **水域の下端より上**（`wave_axis.w` = 水面から下端までの深さ。負＝下端なし） | 光・影オフセット両方（新規） |
  | (c) | **水深 ≥ `CAUSTICS_SHADOW_MIN_DEPTH_M`（0.5m）** | **影オフセットのみ**（新規） |

  - (b) は CPU 側の正典（`engine::water::query::volume_contains` の Region 規則
    「`y <= surface_y` かつ `y >= center_y − half_extents_y`」）と同じ式を GPU へ持ち込んだもの。
    値は `WaterParams::submerge_depth_limit`（`renderer/water/params.rs`）が計算し、
    **旧「予約(0)」だった `wave_axis.w`** へ焼く（vec4 の本数もストライドも不変）。
    Ocean と川は下端を持たないので `WATER_SUBMERGE_DEPTH_UNBOUNDED = -1.0` を入れ、判定をスキップする。
    これで「池の真下にある洞窟・地下室」が水中扱いされる不具合も同時に消える。
  - (c) を**影オフセットにだけ**掛けるのが要点。光（集光模様）にまで最小水深を課すと
    30cm の水たまりで模様が消えてしまう。一方オフセットのずれは深さに比例するので、
    浅いところを捨てても演出上の損失はない。
  - **Ocean の残課題**: 「カメラとピクセルの間に本当に水面があるか」は G-Buffer だけでは判定できない
    （水面より低い**屋内・堤防の内側**などは依然として水中扱いになる）。(c) の 0.5m と (2) の 5cm
    クランプで実害を潰す割り切りであり、厳密解は水面パスの深度を別途焼く必要がある。未着手。
...
- **I1: InteractionSource＋瞬発場＋草の揺れ** — ✅ **実装済み（2026-07-28）**
  ...
  **I1 の既知の制限**（後続フェーズで解消）
  ...
  - ~~消費側は草だけ~~ → **I2 で水面が加わった**。雪泥の積もり（永続の場）は
    **I3.1 で加わった**（地表カバー場。ただし瞬発場ではなくチャンク紐づけの別の場である）。
    轍（動く物が場を凹ませる書き込み）は **I3.2 で実装済み**（カバー場側の
    踏み固めチャネルへ書く。瞬発場そのものは触っていない）。
  - 窓（64m）の外に出た草は影響を受けない。広域の積もり・轍表現は
    **地表カバー場（I3.1／I3.2）**の担当で、こちらはカメラ追従の窓を持たない。
...
- **I3: 雪・泥** — 地表カバー場（永続テクスチャ）＋地形シェーダの変位・素材ブレンド（最重量）
  - **I3.1: 地表カバー場（済）** — 正典は **`docs/cover_field.md`**。
    地形チャンク紐づけの低解像度永続テクスチャ（既定 32×32／チャンク、`.tcover`）へ
    「素材 ID ＋ 量」を 1 層持ち、`CoverEmitterComponent`（Global / Region / TextureMask）が
    傾斜ルール込みで積算する。素材（色・粗さ・変位高さ）は
    `assets/terrain/cover_materials.json` のデータ差し替えだけで作れる。

    **本フェーズで確定した設計判断**（詳細と根拠は `docs/cover_field.md` §2）:
    - **積算は CPU**。1 チャンク 1024 テクセルしか無く、GPU 化すると保存のたびに
      テクスチャ読み戻し（非同期マップ）が要り、「N 秒ぶんを即時計算」がフレームを跨ぐ。
      積算・保存・テストを 1 本の同期コードで完結させるほうがこの規模では明確に有利。
    - **変位は CPU が頂点位置へ焼く**。頂点シェーダで動かすと G-Buffer パス専用になり、
      シャドウマップ・深度・ID パス・RT の形状とズレる（雪が影を落とさない）。
      位置そのものへ焼けば、頂点バッファを共有する全ラスタパスが自動的に一致する。
    - **色・粗さはフラグメントで合成**。頂点が運ぶのは量と素材番号だけで、
      地形で未使用だった `uv0` を転用した（T2 が `Vertex.color` へレイヤ重みを
      smuggle したのと同じ判断。72 バイトの共有頂点レイアウトを増やさない）。
    - **素材表は地形レイヤ uniform（group3）へ同居**。全チャンク共通のデータなので、
      チャンクごとのバインドグループもテクスチャも増えない（`max_bind_groups = 5` に触れない）。
    - **量 0 のチャンク・カバー場が無いチャンクは従来と完全同一**（`uv0` が 0 なので
      シェーダの分岐に入らない）。契約テストで固定してある。
    - **Play 中の積算は揮発**。Stop で Edit の保存状態へ戻る（水位 `sim_level_y` と同じ考え方）。

    **I3.1 の既知の制限**（`docs/cover_field.md` §7 が正典）:
    - 同一チャンク内の上下 2 面は XZ 投影のため区別できない（上の面が勝つ）。
      地表と洞窟の床は多くの場合そもそも別チャンクなので、
      チャンク紐づけの時点で大半のケースは既に分かれている。
    - RT（BLAS）とコライダーは変位前の地形を見る。変位は数センチなので実害は無い。
    - 1 チャンクに 1 層（雪の下に落ち葉は残らない。後から積もる素材が古い素材を
      削りながら置き換わる）。
  - **I3.2: 轍・変形（済）** — 正典は **`docs/cover_field.md` §3.5**。
    カバー場へ「踏み固め（trample）」と「面の基準 Y（base_y）」の 2 チャネルを足し
    （`.tcover` は TCOV **v2** へ。v1 は読み込み互換）、Play 中に動く
    `InteractionSource` が接地点へ痕をスタンプする。

    **本フェーズで確定した設計判断**:
    - **実効の量 = `max(量 − trample, 0)`**。引き算を CPU 側で済ませることで、
      変位も色も I3.1 の 1 本の経路がそのまま使える。量を超えた所は下地が露出する。
    - **踏み固めは最大値更新**（加算ではない）。立ち続けても穴は掘れず、
      複数ソースが同じテクセルを踏んでも結果が順序に依らない。
    - **Y 照合＝「面はワールド位置の純関数として選ぶ」**。スタンプ時は接地 Y と
      テクセルの基準 Y を照合し（洞窟の床の轍が頭上の地表へ写らない）、
      焼き込み時は `CoverNeighborhood` を **3×3×3**（上下 3 段）へ広げて
      頂点のワールド Y に最も近い段の面を選ぶ。
      **これが I3.1 §7-8「Y チャンク境界の段差」の恒久対策でもある**
      （回帰テスト `y_boundary_sample_is_bit_identical_between_stacked_chunks`）。
    - **踏み固めの暗さは `uv1.x` へ 1 スカラーで畳む**。GPU の素材 uniform は不変で、
      カバーを持たない地形は轍導入前とビット単位で同じ出力になる。
    - **埋め戻しは「降った分＋素材の `refill_rate`」だけ**（自然減衰は無い）。
      エミッタを止めれば轍は永久に残る。
    - **スタンプ形状は Circle / Texture の 2 種**。Texture は進行方向へ回して押す
      （タイヤのトレッド・靴底）。**草なびき・水の波紋は従来どおり等方の円**で、
      形状は轍だけに効く（場のフォーマットを変えずに済ませる判断）。
    - **Play 揮発**（積算と同じ扱い）。**Edit での轍オーサリングはスコープ外**。
    - 性能: スタンプは移動時のみ・AABB 交差チャンクのみ（既定 48 のうち 1〜4 個）。
      計測は `SEED_PERF_LOG=1` の `[PERF terrain] cover stamp ...`。

    **I3.2 の既知の制限**: 轍は物理コライダー・RT に反映されない（I3.1 と同じ理由）。
    痕の細かさはカバー場の解像度（既定 0.5m 刻み）で頭打ちになる。
  - **I3.3: 天候 UI とスクリプト API（未着手）** — エミッタ強度の読み書きを
    スクリプトへ公開し、天候制御と合わせる。

推奨順（2026-08-02 改訂: W2/W3 はユーザー側責務へ切り出し）:
W0 → W1 → I1 → **I2（済）** → **W1.5（済）** → **W4（済）** → **W5.1（済）** → **W5.3（済）** → **W2.5（済）** → **W5.2（済）** → **I2.1（済）** → **I3.1（済）** → **I3.2（済）** → I3.3
