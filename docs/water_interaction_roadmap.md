...
- **W6: 水面の見た目の詰め（タイル感・川の継ぎ目・波の方向）** — ✅ **実装済み（2026-07-29）**
  ...
  **実装参照**
  ...
  | 役割 | 場所 |
  |---|---|
  | 解析波の層テーブル・空間包絡・回転 | `shaders/water_surface.wgsl` の `wave_layer()` / `water_wave_envelope()` / `water_wave_rotate_point()` / `water_wave_rotate_gradient()` |
  ...
  | コンポーネント | `components/water_volume_component.rs` の `wave_direction_deg` |
