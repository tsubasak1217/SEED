...
| 名前 | 1〜32 文字。英数字・`_`・`-`・`.` と日本語可、空白と記号は不可。**サーバ内で一意・作成後は変更不可**（JWT の `sub` とロック所有者の表示に使う）。一意判定は **ASCII の大文字小文字を無視**する（`alice` と `Alice` は同じ人として扱い、後から来たほうを `name_taken` で断る。表示は入力どおり） |
...
- `expires_at`（`/v1/invites` と `/v1/login/complete`）はどちらも **Unix ミリ秒**。
