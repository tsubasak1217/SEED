...
- `seed_save_scene` は Edit 状態のみ。新規（未保存）シーンは保存先が決まらないためエラー。
  ...
  Android の一時停止中に端末のシーンの写しをシーンパネルへ出している（閲覧専用。[android.md](android.md) §20.17）。
- 端末の写しを出している間（閲覧専用）は、ランタイムが保存・シーンの編集・シーンの切り替えの IPC を捨てて
  `SNAPSHOT_VIEW_REFUSED:<命令の名前>` を返す（`seed_*` の編集系のツールも効かない。見る・撮るツールは使える）。
  写しは再開・停止で元の編集中のシーンへ戻る（[android.md](android.md) §21.13）。
...
0. 撮影フレームは**専用のオフスクリーンターゲットへ描き、present しない**
   （`runtime/src/engine/core/renderer/thumbnail/target.rs`、
   出力先の切り替えは `Renderer::begin_offscreen_frame`）。
   **提示フレーム（スワップチェーン）は一切使わない**ので、生成のあいだ
   エディタのビューポートに映る絵は 1 ピクセルも変わらない
   （セッションが続く間は画面が静止する）。
   環境変数のフレームダンプと `SCREENSHOT:` も撮影フレームを対象にしない。
...
- サムネイルの解像度は**描画解像度**の短辺が上限（オフスクリーンターゲットは
  描画解像度ちょうどで確保し、中央の正方形を切り出して縮小するため）。
  ヘッドレスは 1920x1080 なので 1080px まで劣化なしで出せる。
- シーンの保存は一切行わない。読み取り専用で開いたシーンでも実行できる。
- 生成中はフレームを present しないため、**エディタのビューポートは静止画のまま**になる
  （最後の要求から 2 秒でセッションを畳み、そこで 1 枚提示して元の絵へ戻る）。
  同じ理由で Play 中は処理しない（ゲーム画面が数秒固まって見えるため）。
