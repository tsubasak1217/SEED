...
use seed_plugin_api::{PLUGIN_ENTRY_FN, Plugin, PluginCreateFn};
...
fn test_load_sample_plugin() {
    ...
    if !dll_path.exists() {
        eprintln!(
            "スキップ: {} が見つかりません（先に cargo build -p sample_plugin を実行してください）",
            dll_path.display()
        );
        return;
    }
    ...
    let lib = unsafe { libloading::Library::new(&dll_path) }.expect("DLL ロード失敗");

    let create_fn: libloading::Symbol<PluginCreateFn> =
        unsafe { lib.get(PLUGIN_ENTRY_FN) }.expect("エントリ関数取得失敗");
    ...
    for d in &defs {
        println!(
            "  - {} ({:?}): デフォルト={}",
            d.label, d.kind, d.default_value
        );
    }
